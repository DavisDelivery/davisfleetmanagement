...
Always match to the closest fleet number. Use the TOTAL line (including tax) for the total amount. For the date, use DATE INVOICE (not DATE SHIPPED). Return ONLY the JSON array.`}
  ...
  const sameAsYesterday=(targetDayIdx)=>{
    ...
    // v2.10.50: Safety net for the "repaired truck re-marked OOS" bug.
    // A truck can carry a stale OOS status from a previous day even after its
    // repair was closed (e.g. closed in the portal, or pre-marked OOS for days
    // that never got cleared). When copying forward, find every truck whose
    // status board shows it DOWN on the target day but which has NO open repair
    // ticket — that truck has been repaired. Clear its stale OOS for the target
    // day so it doesn't show as OOS (and doesn't trigger a phantom new ticket),
    // then tell the dispatcher which trucks were cleared so they can reassign.
    const nextStat={...tStat};
    const clearedTrucks=[];
    trucks.forEach(t=>{
      const key=`${t.id}-${targetDay}`;
      const cur=(nextStat[key]||"").toLowerCase().trim();
      const isDown=cur==="oos"||cur==="bd"||cur.includes("repair")||cur.includes("interstate")||cur.includes("shop")||cur.includes("mech");
      if(!isDown)return;
      const hasOpenRepair=repairs.some(r=>r.truckId===t.id&&r.status==="open");
      if(!hasOpenRepair){
        // Repaired — clear the stale down-status for the target day
        nextStat[key]="HERE";
        clearedTrucks.push(t.id);
      }
    });

    if(copied>0)saveAsgn(next);
    if(clearedTrucks.length>0)saveTStat(nextStat);
    ...
    if(clearedTrucks.length>0){
      msg+=`\n\n⚠ ${clearedTrucks.length} truck${clearedTrucks.length===1?"":"s"} ${clearedTrucks.length===1?"was":"were"} marked OOS but no longer ${clearedTrucks.length===1?"has":"have"} an open repair ticket — ${clearedTrucks.length===1?"it has":"they've"} been repaired and ${clearedTrucks.length===1?"is":"are"} now cleared for service:\n\n  #${clearedTrucks.join(", #")}\n\nThese were NOT carried over as OOS. Assign drivers to them as needed.`;
    }
    alert(msg);
  };
