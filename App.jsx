...
function App(){
  ...
  const drvIcon=(role)=>{
    ...
    if(r.includes("load")||r.includes("shift"))return"🏗️";
    return"🚛";
  };
  ...
  const fetchGmailInvoices=async(vendor)=>{
    ...
    try{
      ...
      // Always set to a safe array shape
      const setSafe=(val)=>{
        ...
        else setGmailResults([{error:String(val).substring(0,200)}]);
      };
      if(data.error){setGmailResults([{error:String(data.error).substring(0,300)}]);setGmailLoading(false);return;}
      if(data.results&&Array.isArray(data.results)&&data.results.length>0){setSafe(data.results);}
      else if(data.message){setGmailResults([{error:String(data.message).substring(0,300)}]);}
      else if(data.rawText){
        const m=String(data.rawText).match(/\[[\s\S]*\]/);
        if(m){try{setSafe(JSON.parse(m[0]));}catch(e){setGmailResults([{error:"Found data but could not parse: "+String(data.rawText).substring(0,200)}]);}}
        else setGmailResults([{error:"No invoices found in response"}]);
      }else{
            ...
            {gmailResults&&Array.isArray(gmailResults)&&<div style={{marginTop:10}}>
              ...
              {gmailResults.map((r,i)=>{
                ...
                return <div key={i} style={{padding:"8px 10px",background:safeR.error?"#fef2f2":"#f0f9ff",borderRadius:6,marginBottom:4,border:"1px solid #e2e8f0"}}>
                  {safeR.error?<div style={{fontSize:11,color:C.red}}>{String(safeR.error)}</div>:<div>
