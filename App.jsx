    ...
    try{
      ...
      if(isPdf){
        ...
        for(let p=1;p<=pdf.numPages;p++){
          ...
          // Compute scale so longest side doesn't exceed 1400px (Anthropic API has a 5MB base64 image limit).
          const rawVp=page.getViewport({scale:1});
          const maxSide=Math.max(rawVp.width,rawVp.height);
          const targetScale=Math.min(2,1400/maxSide);
  ...
  const handleFileUpload=async(e)=>{
    ...
    for(const file of files){
      if(file.type==="application/pdf"){
        ...
        for(let p=1;p<=pdf.numPages;p++){
          const page=await pdf.getPage(p);
          // Cap longest side at 1400px to avoid Anthropic 5MB image limit, use JPEG for smaller payload.
          const rawVp=page.getViewport({scale:1});
          const maxSide=Math.max(rawVp.width,rawVp.height);
  ...
  const scanInvoices=async()=>{
    ...
    for(let i=0;i<updated.length;i++){
      ...
      try{
        ...
        let base64Data=updated[i].dataUrl.split(",")[1];

        // ─── PRE-FLIGHT: detect blank/near-blank pages ───
        // Quick Fuel and Peach State invoices often have cover pages and legal/terms
        // pages that are mostly whitespace. Anthropic's image validator rejects these
        // with "The string did not match the expected pattern" — skip them proactively.
        ...
        if(mediaType&&mediaType.startsWith("image/")){
          try{
            ...
            if(probeImg.width<50||probeImg.height<50){
              ...
              if(contentRatio<0.01)blankReason=`page is ${(contentRatio*100).toFixed(2)}% content (appears blank)`;
            }
          }catch(probeErr){/* if probe fails, continue and let the scan handle it */}
        }
        if(blankReason){
          updated[i].status="error";
          updated[i].parsed={error:`Skipped — ${blankReason}. No invoice data on this page.`};
          setScanQueue([...updated]);
          continue;
        }

        // Anthropic's image limit is 5MB decoded (~6.67M base64 chars).
        // If we're over 4MB decoded (5.33M chars), re-encode at lower quality to avoid
        // "string did not match the expected pattern" errors from their validator.
        ...
        if(base64Data&&base64Data.length>MAX_B64_CHARS&&mediaType&&mediaType.startsWith("image/")){
          try{
            ...
            // Progressively lower quality/size until under limit
          ...
          }catch(recErr){/* fall through — send original and let Anthropic error */}
        }
...
- These are WEEKLY fuel card invoices covering MULTIPLE trucks on ONE statement (typically $10K+ total per invoice).
- You MUST return ONE OBJECT PER TRUCK in the array (NOT one object for the whole invoice). A single Quick Fuel PDF typically yields 20-40 truck rows.
...
- For EVERY truck row (INCLUDING the INVENTORY/unassigned bucket), set:
    - truckId: the truck # from that section (e.g. "0294", "3977") — keep leading zeros. For the "0" bucket use "INVENTORY".
    - vendor: "Quick Fuel"
    - category: "Fuel"
    - total: the "Total" (with tax) for that specific truck (REQUIRED — must be > 0)
    - gallons: the "Units" (gallons) for that truck (REQUIRED — must be > 0, even for INVENTORY bucket)
    - pricePerGallon: total ÷ gallons rounded to 4 decimals (REQUIRED — even for INVENTORY bucket)
    - invoiceNum: CRITICAL — you MUST make every row's invoiceNum UNIQUE by appending the truckId. Format: "<base invoice #>-<truckId>". Example: if the invoice is "CFS-4582698" and the truck is "0294", use "CFS-4582698-0294". For INVENTORY use "CFS-4582698-UNASSIGNED". If the same truckId appears twice, use "-0294-1" and "-0294-2". This is required so each truck-row saves separately — if two rows share an invoiceNum the second will be skipped as a duplicate.
    - date: the Invoice Date at top
    - lineItems: include one item like [{"desc": "X.XX gallons @ $Y.YYYY/gal (true cost incl tax)", "amount": total}]
    - notes: "Weekly fuel card - Truck X" or for INVENTORY: "Unassigned fuel card purchases (no unit # entered)".
- The sum of all truck entry totals should equal the Invoice Amount at top (within cents). If the sum is off by more than a few dollars, you missed trucks — re-check the Recap section.
            ...
            <div style={{...s.secT,marginTop:20,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                ...
                {quickFuelAnalytics&&<button onClick={()=>{
                  ...
                  saveCosts(remaining);
                  // Also clear any dismissed Gmail messages from Quick Fuel so the scan UI
                  // won't treat old emails as "already dismissed" after purge
