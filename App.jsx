  ...
  const scanInvoices=async()=>{
    ...
    for(let i=0;i<updated.length;i++){
      ...
      try{
        ...
        // v2.10.4: FLIPPED STRATEGY — document mode is now PRIMARY for all PDFs.
        // Reasoning: text-extraction via pdf.js has had unexplained failures on
        // specific FuelFox PDFs (e.g. 04-07, 04-09, 04-16 Service Logs with -002-
        // in filename) that were clearly readable native-text PDFs. Rather than
        // continue to chase the subtle cause, we just use Anthropic's PDF mode as
        // the primary path — it reliably handles ALL PDF types (native text,
        // scanned, hybrid) server-side. Text extraction is kept as a fallback
        // ONLY for PDFs too large for the 5MB Netlify function payload.
        const DOC_MODE_MAX_B64=4_000_000; // ~3MB raw PDF, comfortable margin
        const MAX_B64_CHARS=5_300_000;
        const pdfB64=isPdfDoc?(updated[i].dataUrl||"").split(",")[1]:null;
        const pdfSize=pdfB64?pdfB64.length:0;
        const hasPdfText=isPdfDoc&&updated[i].pdfText&&updated[i].pdfText.trim().length>50;
        // Prefer document mode when the PDF is small enough. Only fall back to
        // text mode for oversized PDFs that would exceed the serverless limit.
        const useDocMode=isPdfDoc&&pdfB64&&pdfSize<=DOC_MODE_MAX_B64;
        const useTextMode=isPdfDoc&&!useDocMode&&hasPdfText;
        const isPdfText=useTextMode; // legacy alias used in promptIntro below
        let contentBlock;
        let base64Data=null;
        let mediaType=null;

        if(useDocMode){
          // v2.10.4 PRIMARY: send whole PDF to Claude as document (reliable, handles all PDF types)
          console.log(`[scan] using DOCUMENT mode for "${updated[i].file}" (${Math.round(pdfSize/1024)}KB)`);
          contentBlock={type:"document",source:{type:"base64",media_type:"application/pdf",data:pdfB64}};
        }else if(useTextMode){
          // v2.10.4 FALLBACK: large PDF (>4MB) with extractable text — use text-only path to fit payload
          console.log(`[scan] using TEXT mode for "${updated[i].file}" (${Math.round(pdfSize/1024)}KB too large for doc mode, ${updated[i].pdfText.length} chars extracted)`);
          contentBlock=null;
        }else if(isPdfDoc){
          // Oversized PDF with no extractable text — can't scan
          if(!pdfB64){
            throw new Error(`PDF has no data URL — cannot scan "${updated[i].file}". Delete this entry and re-upload.`);
          }
          throw new Error(`PDF is ${Math.round(pdfSize/1024/1024*10)/10}MB with no extractable text layer and exceeds the ${Math.round(DOC_MODE_MAX_B64/1024/1024)}MB document-mode limit. Download from Gmail manually and upload a smaller version via "Choose Files", or enter manually.`);
        }else{
...
Always match to the closest fleet number. Use the TOTAL line (including tax) for the total amount. For the date, use DATE INVOICE (not DATE SHIPPED). Return ONLY the JSON array.`}
        ...
        // v2.10.6: If document mode was rejected by Anthropic, render the PDF
        // to images via pdf.js client-side and retry with image blocks. This is the
        // bulletproof fallback for PDFs that Anthropic's PDF parser chokes on (e.g.
        // the FuelFox -002- Service Logs) — pdf.js can always render them because
        // that's the same engine the app's View modal uses to display them.
        if(data.error&&useDocMode&&pdfB64&&/string did not match|expected pattern|could not (process|decode)|invalid.*(base64|document|pdf)/i.test(String(data.error)+String(data.details||""))){
          console.log(`[scan] Document mode rejected for "${updated[i].file}". Anthropic said: ${String(data.error)} ${String(data.details||"").substring(0,200)}`);
          console.log(`[scan] Rendering PDF to images as fallback...`);
          updated[i].parsed={_waitMsg:"Document mode failed — rendering to images and retrying..."};
          setScanQueue([...updated]);
          try{
            if(!window.pdfjsLib){
              await new Promise((res,rej)=>{const s=document.createElement("script");s.src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js";s.onload=res;s.onerror=rej;document.head.appendChild(s);});
            ...
            for(let p=1;p<=pdf.numPages;p++){
              const page=await pdf.getPage(p);
              // Scale 1.5x gives ~1275×1650 for US Letter — good readability, fits in payload
              const viewport=page.getViewport({scale:1.5});
              const canvas=document.createElement("canvas");
              canvas.width=viewport.width;
              canvas.height=viewport.height;
              await page.render({canvasContext:canvas.getContext("2d"),viewport}).promise;
              const jpegB64=canvas.toDataURL("image/jpeg",0.85).split(",")[1];
              console.log(`[scan] Rendered page ${p}/${pdf.numPages} → ${Math.round(jpegB64.length/1024)}KB JPEG`);
              imageBlocks.push({type:"image",source:{type:"base64",media_type:"image/jpeg",data:jpegB64}});
            }
            console.log(`[scan] Retrying with ${imageBlocks.length} image block(s)...`);
            const retryBody={...msgBody,messages:[{...msgBody.messages[0],content:[
              ...imageBlocks,
              ...msgBody.messages[0].content.filter(c=>c.type==="text"),
            ]}]};
            resp=await fetch("/api/scan-invoice",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(retryBody)});
            data=await resp.json();
            if(!data.error){
              console.log(`[scan] ✓ Image fallback succeeded for "${updated[i].file}"`);
            }else{
              console.log(`[scan] ✗ Image fallback also failed:`,data.error,data.details?.substring(0,200));
            }
          }catch(renderErr){
            console.log(`[scan] PDF-to-image render failed:`,renderErr.message);
          }
          updated[i].parsed=null;
          setScanQueue([...updated]);
        }
