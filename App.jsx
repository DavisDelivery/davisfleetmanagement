    ...
    <div style={s.wrap}>
      ...
      <div style={s.body}>
        ...
        {tab==="fleet"&&<div>
          ...
          {fleetView==="list"&&(()=>{
            // v2.10.37: build per-truck per-day driver lookup from asgn data
            const di=Math.min(todayDI(),4);
            ...
            return <div style={s.tWrap}><table style={{...s.table,fontSize:12}}><thead><tr>{["Truck #","Type","Make","Trans","Axle","Status",...DAYS,"Repairs",""].map((h,i)=>{
              ...
              {filteredTrucks.map(t=>{const st2=gTS(t.id,dk);const openR=repairs.filter(r=>r.truckId===t.id&&r.status==="open").length;
                ...
                return <tr key={t.id} style={s.tr}>
                  ...
                  <td style={s.ltd}>{t.type==="straight"?"Box":"Tractor"}</td>
                  <td style={s.ltd}>
                  ...
                  {/* v2.10.39: Mon–Fri columns show driver assignment + historical OOS.
                      OOS marker reflects the truck's status on that specific day from tStat
                      (which is a per-day record). Past-day OOS stamps are preserved as
                      history — when a truck was in the shop earlier this week, those days
                      stay marked OOS even after the repair ticket is closed. */}
                  {DAYS.map((d,i)=>{
