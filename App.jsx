  ...
  const scanInvoices=async()=>{
    ...
    for(let i=0;i<updated.length;i++){
      ...
      try{
        ...
        // v2.10.2: Distinguish text-extracted PDFs from image-based PDFs
        // pdfText being non-empty = native text PDF, extracted successfully (cheap path)
        // pdfText being empty on a pdf-doc = image-based/scanned PDF, send as document (Anthropic OCR)
        // Threshold of 50 chars catches cases where pdf.js returned a few junk chars
        const hasPdfText=isPdfDoc&&updated[i].pdfText&&updated[i].pdfText.trim().length>50;
        const isPdfText=hasPdfText; // legacy alias used in promptIntro below
        ...
        if(hasPdfText){
          // v2.9.2: send extracted PDF TEXT instead of raw bytes (cheapest path)
          contentBlock=null;
        }else if(isPdfDoc){
          // v2.10.2: image-based PDF (no text layer) or extraction failed.
          // Send whole PDF to Claude as document — Anthropic's PDF beta handles OCR server-side.
          // This is the path that was BROKEN in v2.9.2-v2.10.1: those versions fell through
          // to the image branch with media_type="application/pdf" which is invalid and caused
          // "The string did not match the expected pattern" errors on FuelFox -002- (scanned) PDFs.
          const pdfB64=(updated[i].dataUrl||"").split(",")[1];
          if(!pdfB64){
            throw new Error(`PDF has no data URL and no extractable text — cannot scan "${updated[i].file}". Delete this entry and re-upload.`);
          }
          if(pdfB64.length>MAX_B64_CHARS){
            throw new Error(`PDF is ${Math.round(pdfB64.length/1024/1024*10)/10}MB with no extractable text layer (likely a scanned image). Exceeds 5MB serverless limit. Try downloading from Gmail and uploading a smaller version via "Choose Files", or enter manually.`);
          }
