    ...
    <div style={s.wrap}>
      ...
      <div style={s.body}>
        ...
        {tab==="board"&&<div>
          ...
          <div style={s.tWrap}><table style={s.table}><thead><tr><th style={{...s.th,...s.thS}}>Driver</th>{DAYS.map((d,i)=><th key={d} style={s.th}>{d}<button onClick={()=>sameAsYesterday(i)} title={i>0?`Copy ${DAYS[i-1]} → ${d}`:`Copy last Friday → ${d}`} style={s.sayMini}>←</button></th>)}</tr></thead><tbody>
            {filteredRG.map(g=>{
              ...
              return[
                ...
                ...gd.map(dr=><tr key={dr.name} style={s.tr}><td style={{...s.td,...s.tdN,cursor:"pointer",color:C.brand,textDecoration:"underline",textDecorationColor:C.brand+"44"}} onClick={()=>setDriverReport(dr.name)}>{drvIcon(dr.role)} {dr.name}</td>
                  {DAYS.map(day=>{const ck=`${dr.name}-${day}`;const val=asgn[ck]||"";const isE=editCell===ck;const isOff=OFF_OPTS.includes(val);const ti=val&&!isOff?trucks.find(t=>t.id===val):null;const dup=val&&!isOff&&dupByDay[day]&&dupByDay[day].has(val);const oos=val&&!isOff&&!dup&&oosTrucks.has(val);
                    ...
                    return <td key={day} style={dup?{...s.td,background:"#fef2f2",boxShadow:`inset 0 0 0 1.5px ${C.red}`}:oos?{...s.td,background:"#fff7ed",boxShadow:`inset 0 0 0 1.5px ${C.accent}`}:s.td} title={dup?`Truck ${val} is double-booked on ${day}`:oos?`Truck ${val} is out of service (open repair) — reassign`:undefined} onClick={()=>startEdit(ck,val)}>{val?<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
                      <span style={{fontWeight:700,fontFamily:isOff?"inherit":"monospace",fontSize:isOff?9:12,color:dup?C.red:oos?C.accent:(isOff?C.accent:"#1e293b")}}>{dup?"⚠ ":oos?"🔧 ":""}{val}</span>
