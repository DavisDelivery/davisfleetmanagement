  ...
  const printOpenRepairs=()=>{
    ...
    const cardHTML=r=>{
      ...
      const notes=notesLog.map(entry=>{
        ...
        return `<div class="note"><span class="nt">${esc(label)}</span> ${esc(entry.text)}</div>`;
      }).join("");
        ...
        {tab==="maint"&&(()=>{
          ...
          {(()=>{
            ...
            const addNoteToRepair=async(id)=>{
              ...
              if(!existing)return;
              // Migrate legacy r.notes string into notesLog on first append
              const prevLog=existing.notesLog||[];
              const legacyNote=existing.notes&&!existing.notesLog?[{ts:existing.dateIn||new Date().toISOString(),text:existing.notes}]:[];
              updateRepair(id,{notesLog:[...legacyNote,...prevLog,entry],notes:""});
              inp.value="";
            };
            const renderCard=(r)=>{
              const t=trucks.find(x=>x.id===r.truckId);
              ...
              return <div key={r.id} style={{...s.repairCard,marginBottom:10}}>
                ...
                {/* Notes log — timestamped entries */}
                {notesLog.length>0&&<div style={{marginTop:8,background:"#f8fafc",borderRadius:6,padding:"8px 10px",display:"flex",flexDirection:"column",gap:6}}>
                  {notesLog.map((entry,idx)=>{
                    ...
                    return <div key={idx} style={{display:"flex",gap:8,alignItems:"flex-start"}}>
                      <span style={{fontSize:10,color:"#94a3b8",whiteSpace:"nowrap",marginTop:2,minWidth:90}}>{label}</span>
                      <span style={{fontSize:12,color:"#334155",flex:1}}>{entry.text}</span>
                      ...
                      }} title="Delete note" style={{fontSize:10,color:"#94a3b8",background:"none",border:"none",cursor:"pointer",padding:"0 2px",lineHeight:1,flexShrink:0,marginTop:1}}>✕</button>
                    </div>;
                  })}
                </div>}
                ...
                <div style={{marginTop:8,display:"flex",gap:8}}>
                  <button onClick={async()=>{const cost=await uiPrompt("Final cost (or leave blank):",r.cost||"");if(cost===null)return;const clean=cost.replace(/[$,\s]/g,"");const n=clean===""?r.cost:Number(clean);if(clean!==""&&!Number.isFinite(n)){toast(`"${cost}" isn't a valid number — enter digits only (e.g. 1200.50).`);return;}await closeRepair(r.id,n);}} style={s.closeRepBtn}>Repair Completed</button>
                </div>
              </div>;
            };
