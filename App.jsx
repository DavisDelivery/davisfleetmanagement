    ...
    try{
      ...
      if(isPdf){
        ...
        for(let p=1;p<=pdf.numPages;p++){
          ...
          newItems.push({id:Date.now()+Math.random()+p,file:`${att.filename} (pg ${p}/${pdf.numPages})`,dataUrl,status:"ready",parsed:null,type:"image"});
        }
      }else if(att.mimeType&&att.mimeType.startsWith("image/")){
        const dataUrl=`data:${att.mimeType};base64,${b64}`;
        newItems.push({id:Date.now()+Math.random(),file:att.filename,dataUrl,status:"ready",parsed:null,type:"image"});
      }
          ...
          <div style={{...s.addForm,marginBottom:16}}>
            ...
            {gmailResults&&Array.isArray(gmailResults)&&<div style={{marginTop:10}}>
              ...
              {gmailResults.map((r,i)=>{
                ...
                const pdfAtts=atts.filter(a=>a&&(a.mimeType==="application/pdf"||(a.filename||"").toLowerCase().endsWith(".pdf")||(a.mimeType||"").startsWith("image/")));
                ...
                return <div key={i} style={{padding:"8px 10px",background:safeR.error?"#fef2f2":"#f0f9ff",borderRadius:6,marginBottom:4,border:"1px solid #e2e8f0"}}>
                  {safeR.error?<div style={{fontSize:11,color:C.red,wordBreak:"break-word"}}>{typeof safeR.error==="string"?safeR.error:JSON.stringify(safeR.error).substring(0,500)}</div>:<div>
                    ...
                    {atts.length>0&&<div style={{marginTop:6,display:"flex",flexDirection:"column",gap:4}}>
                      {atts.map((a,ai)=><div key={ai} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:6}}>
                        ...
                        {a.attachmentId&&(a.mimeType==="application/pdf"||(a.filename||"").toLowerCase().endsWith(".pdf")||(a.mimeType||"").startsWith("image/"))&&<button onClick={()=>processGmailAttachment(safeR,a)} disabled={isProcessing} style={{fontSize:9,fontWeight:700,padding:"3px 8px",background:C.green,color:"#fff",border:"none",borderRadius:4,cursor:isProcessing?"wait":"pointer",opacity:isProcessing?0.5:1,whiteSpace:"nowrap"}}>{isProcessing?"...":"→ Scan"}</button>}
                      </div>)}
                    </div>}
                  </div>}
                </div>;
