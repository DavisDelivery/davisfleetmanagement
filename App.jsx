...
function App(){
  ...
  const fetchGmailInvoices=async(vendor)=>{
    ...
    try{
      ...
      if(data.error){setGmailResults([{error:data.error}]);setGmailLoading(false);return;}
      if(data.results&&data.results.length>0){
        setGmailResults(data.results);
      }else if(data.message){
        setGmailResults([{error:data.message}]);
      }else if(data.rawText){
        // Try to extract JSON from raw text
        const m=data.rawText.match(/\[[\s\S]*\]/);
        if(m){try{setGmailResults(JSON.parse(m[0]));}catch(e){setGmailResults([{error:"Found data but could not parse: "+data.rawText.substring(0,200)}]);}}
        else setGmailResults([{error:"No invoices found in response"}]);
      }else{
        setGmailResults([{error:"No results returned"}]);
      }
    }catch(err){
      setGmailResults([{error:err.message||"Failed to fetch from Gmail"}]);
    }
          ...
          <div style={{...s.addForm,marginBottom:16}}>
            ...
            <input type="file" accept="application/json,.json" onChange={async(e)=>{
              ...
              try{
                ...
                alert(`Imported ${newEntries.length} invoices. Skipped ${skipped} duplicates.`);
                e.target.value="";
