...
Format your response as clear sections with headers using ** for bold. Use specific numbers, names, and amounts. Be direct and actionable — this is for the owner who makes operational decisions daily. Keep it concise but thorough.`}
  ...
  const catchUpBacklog=async(daysBack=365)=>{
    ...
    let pass=0,imported=0,queued=0,scanned=0,stalled=0,stuck=0;
    const MAX_PASSES=60;
    ...
    while(pass<MAX_PASSES){
      ...
      setBacklog({running:true,pass,imported,queued,scanned});
      await refreshSyncStatus();
      /* v2.16.18: a pass that processed nothing only means "caught up" if the server also
         got through its queue. When a run hits its time budget before finishing the first
         batch it returns processed:0 with timedOut:true — the queue is still full, it just
         couldn't scan anything in the time it had. Breaking there declared "Caught up ✓"
         while the entire backlog remained (seen as "Pass 1 · scanned 0 · imported 0" with
         152 attachments still queued). Keep sweeping; the server resumes where it stopped.
         Two timed-out passes in a row with nothing processed means it isn't getting
         anywhere — one attachment too slow to ever clear a batch — so stop and say that
         plainly rather than spinning silently to MAX_PASSES. */
      if(!data.processed){
        if(!data.timedOut)break; // nothing left to scan — genuinely caught up
        if(++stalled>=2){
          const left=data.total||0;
          setBacklog({running:false,pass,imported,queued,scanned,note:`Stalled — the server couldn't scan anything inside its time budget${left?` (${left} still queued)`:""}. Try again later.`});
          await loadCostsFromShards(false);
          toast(`Backlog sweep stalled after ${pass} passes — imported ${imported}, queued ${queued}.${left?` ${left} attachment(s) still pending.`:""}`);
          return;
        }
      }else stalled=0;
    }
    ...
    setBacklog({running:false,pass,imported,queued,scanned,note:(hitCap?`Stopped after ${MAX_PASSES} passes — run again to continue.`:"Caught up ✓")+stuckNote});
    toast((hitCap
            ...
            <div style={{background:"#fff",borderRadius:10,padding:0,maxWidth:600,width:"100%",maxHeight:"90vh",overflow:"auto"}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{padding:"16px 20px"}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:16,paddingTop:12,borderTop:"1px solid #e2e8f0"}}>
                  ...
                  <button onClick={async()=>{if(await uiConfirm("Delete this invoice?")){saveCosts(costEntries.filter(c=>c.id!==showCostDetail.id));setShowCostDetail(null);}}} style={{padding:"10px 14px",background:"#fff",color:C.red,border:`1px solid ${C.red}`,borderRadius:6,cursor:"pointer",fontWeight:700,fontSize:13}}>Delete</button>
                </div>
              </div>
            </div>
          </div>}
