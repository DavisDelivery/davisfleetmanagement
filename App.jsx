...
Format your response as clear sections with headers using ** for bold. Use specific numbers, names, and amounts. Be direct and actionable — this is for the owner who makes operational decisions daily. Keep it concise but thorough.`}
  ...
  const fetchGmailInvoices=async(vendor)=>{
    ...
    try{
      ...
      if(data.error){setGmailResults([{error:String(data.error).substring(0,300)}]);}
        ...
        if(data.results.length===0)setGmailResults([{error:"No matching emails found in the last 90 days."}]);
  ...
  const processGmailAttachment=async(emailResult,att)=>{
    ...
    try{
      ...
      const resp=await fetch("/api/gmail-attachment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({refresh_token:gmailConn.refresh_token,messageId:emailResult.emailId,attachmentId:att.attachmentId})});
      const data=await resp.json();
      ...
      if(isPdf){
        // Convert PDF pages to images via pdf.js
        if(!window.pdfjsLib){
        ...
        // Decode base64 to bytes. Gmail returns base64url which may be missing = padding —
        // atob() throws "The string did not match the expected pattern" on unpadded input.
        // Add padding and strip any whitespace/newlines before decoding.
        let b64Clean=b64.replace(/\s/g,"").replace(/-/g,"+").replace(/_/g,"/");
        const padLen=(4-(b64Clean.length%4))%4;
        b64Clean=b64Clean+"=".repeat(padLen);
        let binary;
        try{binary=atob(b64Clean);}catch(decodeErr){throw new Error(`Base64 decode failed on attachment "${att.filename}". Gmail may have returned corrupted data. Try again or scan this PDF manually.`);}
        ...
        for(let p=1;p<=pdf.numPages;p++){
          ...
          newItems.push({id:Date.now()+Math.random()+p,file:`${att.filename} (pg ${p}/${pdf.numPages})`,dataUrl,status:"ready",parsed:null,type:"pdf",origFileName:att.filename,origMimeType:"application/pdf",origDataUrl,sharedFileId,gmailRef:gmailTag});
        }
  ...
  const scanInvoices=async()=>{
    ...
    // ── Detect FuelFox document pairs (Service Log + Invoice) from filenames ──
    // Service logs can span multiple pages (all with same sharedFileId). Match invoices to the service log
    // from the SAME delivery date, not just the first available.
    const isFuelFox=(q)=>{
      const f=(q.file||"").toLowerCase();
      return f.includes("fuelfox")||f.includes("fuel fox");
    };
    const isFuelFoxServiceLog=(q)=>{
      const f=(q.file||"").toLowerCase();
      return f.includes("servicelog")||f.includes("service-log")||f.includes("service_log");
    };
    const isFuelFoxInvoice=(q)=>{
      const f=(q.file||"").toLowerCase();
      return isFuelFox(q)&&(f.match(/invoice[_-]?dd\d+/i)||f.match(/^invoice_dd\d+/i));
    };
    // Extract date from filename (e.g. "04-16-2026" or "2026-04-16")
    const extractDate=(q)=>{
      const f=q.file||"";
    ...
    // Group service log pages by sharedFileId (multi-page PDFs keep their pages together)
    const serviceLogGroups={}; // sharedFileId -> {date, indices: [i1,i2...]}
    for(let i=0;i<updated.length;i++){
      if(updated[i].status!=="ready")continue;
      if(!isFuelFoxServiceLog(updated[i]))continue;
      const sid=updated[i].sharedFileId||updated[i].id;
      if(!serviceLogGroups[sid])serviceLogGroups[sid]={date:extractDate(updated[i]),indices:[]};
      serviceLogGroups[sid].indices.push(i);
    }
    // Find invoice that matches each service log by date
    const pairedIndices=new Set();
    const pairs=[]; // [{serviceLogIndices: [], invoiceIdx}]
    Object.values(serviceLogGroups).forEach(group=>{
      let invoiceIdx=-1;
      // First: try to find an invoice with matching date
      if(group.date){
        for(let j=0;j<updated.length;j++){
          if(pairedIndices.has(j)||updated[j].status!=="ready")continue;
          if(isFuelFoxInvoice(updated[j])&&extractDate(updated[j])===group.date){invoiceIdx=j;break;}
        }
      }
      // Fallback: any unmatched invoice
      if(invoiceIdx===-1){
        for(let j=0;j<updated.length;j++){
          if(pairedIndices.has(j)||updated[j].status!=="ready")continue;
          if(isFuelFoxInvoice(updated[j])){invoiceIdx=j;break;}
        }
      }
      if(invoiceIdx>=0){
        group.indices.forEach(idx=>pairedIndices.add(idx));
        pairedIndices.add(invoiceIdx);
        pairs.push({serviceLogIndices:group.indices,invoiceIdx});
      }
    });

    // ── Process FuelFox pairs first (send ALL service log pages + invoice in one API call) ──
    for(const {serviceLogIndices,invoiceIdx} of pairs){
      serviceLogIndices.forEach(i=>{updated[i].status="scanning";});
      updated[invoiceIdx].status="scanning";
      setScanQueue([...updated]);
      try{
        const slPages=serviceLogIndices.map(i=>updated[i]);
        const inv=updated[invoiceIdx];

        // Build content: alternating text labels + images for each service log page, then invoice
        const content=[];
        content.push({type:"text",text:`IMAGE 1${slPages.length>1?"-"+slPages.length:""} of ${slPages.length+1}: FuelFox Service Log${slPages.length>1?" ("+slPages.length+" pages continuous)":""} — per-truck gallons:`});
        slPages.forEach((sl,idx)=>{
          if(idx>0)content.push({type:"text",text:`(Service Log continued — page ${idx+1}/${slPages.length}):`});
          const slMedia=sl.dataUrl.split(";")[0].split(":")[1];
          const slData=sl.dataUrl.split(",")[1];
          content.push({type:"image",source:{type:"base64",media_type:slMedia,data:slData}});
        });
        content.push({type:"text",text:`IMAGE ${slPages.length+1} of ${slPages.length+1}: FuelFox Invoice (taxes + delivery fee + balance due):`});
        const invMedia=inv.dataUrl.split(";")[0].split(":")[1];
        const invData=inv.dataUrl.split(",")[1];
        content.push({type:"image",source:{type:"base64",media_type:invMedia,data:invData}});
        content.push({type:"text",text:`All ${slPages.length+1} images above represent ONE FuelFox fuel delivery. Follow the SPECIAL RULE FOR FUELFOX ATLANTA:

1. From the Service Log page(s): list EVERY truck's Unit Number + Gallons. The service log may span multiple pages — combine trucks from ALL pages. Stop at the "Total:" row.
2. From the Invoice: get Balance Due (total incl tax+fee) and Total Gallons (from Diesel Sales QTY).
3. Compute TRUE price per gallon = Balance Due ÷ Total Gallons.
4. VERIFY your Service Log total gallons matches the Invoice Diesel Sales QTY (within 0.1 gal). If they don't match you missed a truck.
5. Return a JSON ARRAY with ONE OBJECT PER TRUCK, each with:
{
  "truckId": "unit number (keep leading zeros)",
  "vendor": "FuelFox Atlanta",
  "category": "Fuel",
  "date": "YYYY-MM-DD from Invoice DATE field",
  "invoiceNum": "FuelFox invoice# (e.g. DD404) + '-' + truck# (e.g. DD404-0424)",
  "total": gallons × truePricePerGallon (2 decimals),
  "gallons": this truck's gallons,
  "pricePerGallon": the TRUE price per gallon (Balance Due ÷ Total Gallons, 4 decimals),
  "lineItems": [{"desc": "X.X gallons diesel @ $Y.YYYY/gal (true cost incl tax+fee)", "amount": total}],
  "notes": "FuelFox delivery DDxxx - Truck X (Y gal)"
}

Known fleet trucks: ${truckIds}
...
Return ONLY the JSON array, no markdown, no backticks, no explanation.`});
        ...
        const data=await resp.json();
        if(data.error){throw new Error(data.error+(data.details?": "+data.details.substring(0,200):""));}
        const text=data.content?.find(c=>c.type==="text")?.text||"";
        if(!text)throw new Error("AI returned empty response (may have hit token limit)");
        const clean=text.replace(/```json|```/g,"").trim();
        // Try to extract JSON array even if AI added extra text
        ...
        invoices.forEach(inv=>{if(!inv.truckId||inv.truckId==="null"||inv.truckId==="UNKNOWN")inv.truckId="INVENTORY";});

        // Assign results: first → first service log page, rest → new rows. Mark all other slots as merged.
        const primaryIdx=serviceLogIndices[0];
        if(invoices.length>0){
          updated[primaryIdx].status="parsed";
          updated[primaryIdx].parsed=invoices[0];
          updated[primaryIdx].file=`${slPages[0].file} + ${inv.file} (${slPages.length} SL pages merged)`;
          for(let j=1;j<invoices.length;j++){
            updated.push({
              id:Date.now()+Math.random()+j,
              file:`FuelFox ${invoices[j].invoiceNum||""} Truck ${invoices[j].truckId}`,
              dataUrl:slPages[0].dataUrl,
              status:"parsed",
              parsed:invoices[j],
              type:"pdf",
              origFileName:slPages[0].origFileName,
              origMimeType:slPages[0].origMimeType,
              origDataUrl:slPages[0].origDataUrl,
              sharedFileId:slPages[0].sharedFileId,
              gmailRef:slPages[0].gmailRef,
            });
          }
          // Mark additional service log pages + the invoice slot as merged (hidden)
          for(let idx=1;idx<serviceLogIndices.length;idx++){
            updated[serviceLogIndices[idx]].status="merged";
            updated[serviceLogIndices[idx]].parsed={error:"Merged with primary Service Log"};
          }
          updated[invoiceIdx].status="merged";
          updated[invoiceIdx].parsed={error:"Merged with Service Log"};
        }
      }catch(err){
        serviceLogIndices.forEach(i=>{
          updated[i].status="error";
          updated[i].parsed={error:err.message||"FuelFox pair parse failed"};
        });
        updated[invoiceIdx].status="error";
        updated[invoiceIdx].parsed={error:"Pair scan failed: "+(err.message||"")};
      }
      setScanQueue([...updated]);
    }

    // ── After pair-processing: skip any remaining FuelFox service logs that didn't get paired ──
    // (They have no matching invoice — mark as error so user knows)
    for(let i=0;i<updated.length;i++){
      if(updated[i].status!=="ready")continue;
      if(isFuelFoxServiceLog(updated[i])){
        updated[i].status="error";
        updated[i].parsed={error:"FuelFox Service Log needs matching Invoice PDF to compute true cost per gallon. Please add the corresponding Invoice_DDxxx.pdf and scan again."};
      }
      if(isFuelFoxInvoice(updated[i])&&!isFuelFoxServiceLog(updated[i])){
        updated[i].status="error";
        updated[i].parsed={error:"FuelFox Invoice needs matching Service Log PDF. Please add the corresponding FuelFox-ServiceLog PDF and scan again."};
      }
    }
    setScanQueue([...updated]);

    // ── Process remaining single-doc items ──
    for(let i=0;i<updated.length;i++){
      if(updated[i].status!=="ready")continue;
      ...
      try{
        const mediaType=updated[i].dataUrl.split(";")[0].split(":")[1];
        const base64Data=updated[i].dataUrl.split(",")[1];

        const msgBody={messages:[{role:"user",content:[
              {type:"image",source:{type:"base64",media_type:mediaType,data:base64Data}},
              {type:"text",text:`You are parsing vendor invoices for Davis Delivery Service, a trucking company. This document may contain MULTIPLE invoices (one per page). Extract EVERY invoice found and return a JSON ARRAY of objects (no markdown, no backticks, no explanation). Even if there is only one invoice, return it inside an array.

Each invoice object should have:
{
  "truckId": "4-digit truck/unit number, or 'INVENTORY' if no specific truck",
...
SPECIAL RULE FOR QUICK FUEL / FLYERS ENERGY INVOICES (vendor: "Quick Fuel" or from "ebilling@4flyers.com"):
- These are WEEKLY fuel card invoices covering MULTIPLE trucks on ONE statement.
- You MUST return ONE OBJECT PER TRUCK in the array (NOT one object for the whole invoice).
- Look for the section "Recap by Additional Info 2" — each row there is a separate truck total. The first column is the truck number, then Units (gallons), Amount (pre-tax), Taxes, Total (with tax).
- Use the "Total" column (with tax) as the total for each truck entry.
- Truck numbers that look like "0" or "0 Total" mean drivers did NOT enter a unit number — these MUST use truckId "INVENTORY" and notes should say "Unassigned fuel - needs allocation (Driver forgot to enter unit #)".
- For each truck row, set:
    - truckId: the truck # from that section (e.g. "0294", "3977") — keep leading zeros. For the "0" bucket use "INVENTORY".
    - vendor: "Quick Fuel"
    - category: "Fuel"
    - total: the "Total" (with tax) for that specific truck
    - gallons: the "Units" (gallons) for that truck
    - pricePerGallon: total ÷ gallons (4 decimals) — this is the TRUE cost per gallon including taxes for this truck
    - invoiceNum: the SAME invoice # for all rows (e.g. "CFS-4582698") — but append "-TRUCK" (e.g. "CFS-4582698-0294") so duplicate check doesn't skip them. EXCEPT for the INVENTORY bucket, use "CFS-4582698-UNASSIGNED".
    - date: the Invoice Date at top
    - lineItems: you can leave empty [] OR include a brief summary like [{"desc": "X.XX gallons @ $Y.YYYY/gal (true cost)", "amount": total}]
    - notes: "Weekly fuel card - Truck X" or for INVENTORY: "Unassigned fuel card purchases (no unit # entered)".
- The TOTAL of all truck entries should equal the Invoice Amount at top (within cents).
- Ignore the "Recap by Card" section — that's driver-level info, we want truck-level.

SPECIAL RULE FOR FUELFOX ATLANTA (vendor: "FuelFox" / "FuelFox Atlanta" / from "accounting@fuelfox.net" / "fuelfox.net"):
- FuelFox sends TWO documents per delivery that must be combined:
    (A) "Service Log" — has per-truck breakdown: Unit Number, Gallons, Price Per Gallon, Total Charge. Note: the "Price Per Gallon" shown is BASE diesel price only (excludes taxes and delivery fee).
    (B) "Invoice" (e.g. "Invoice DD404") — has line items: "Diesel Sales" (base total matching Service Log), "Diesel Taxes", "Delivery Fee". BALANCE DUE = true total cost.
- TRUE COST PER GALLON = Balance Due ÷ Total Gallons. Example: $6,457.88 ÷ 1,381.7 gal = $4.6738/gal (not $4.317 shown on Service Log).
- If you see ONLY the Service Log (no tax/fee info), you cannot compute true cost — in that case use the Total Charge column as-is and add a note "⚠ Taxes+delivery not included — match with FuelFox Invoice DDxxx for true cost".
- If you see ONLY the Invoice (no truck breakdown), return ONE entry with truckId="INVENTORY", total=Balance Due, notes="Match with FuelFox Service Log to distribute across trucks".
- If BOTH documents are visible in this scan (multi-page PDF, or user pasted both images):
    1. From the Service Log: get each truck's gallons.
    2. From the Invoice: get Balance Due (total with tax+fee) and Total Gallons.
    3. True price per gallon = Balance Due ÷ Total Gallons.
    4. For EACH truck, return an object with:
         - truckId: the Unit Number (e.g. "0424", "5042")
         - vendor: "FuelFox Atlanta"
         - category: "Fuel"
         - total: truck's gallons × true price per gallon (rounded to 2 decimals)
         - gallons: the number of gallons for this truck
         - pricePerGallon: the TRUE price per gallon (Balance Due ÷ Total Gallons, to 4 decimals)
         - invoiceNum: the FuelFox invoice # from the Invoice doc (e.g. "DD404") + "-" + truck # (e.g. "DD404-0424")
         - date: Invoice DATE from the Invoice doc
         - lineItems: [{"desc": "X.X gallons diesel @ $Y/gal (true cost incl tax+fee)", "amount": total}]
         - notes: "FuelFox delivery DDxxx - Truck X (Y gal)"
- The sum of all truck totals should equal (within rounding) the Invoice Balance Due.
...
Always match to the closest fleet number. Use the TOTAL line (including tax) for the total amount. For the date, use DATE INVOICE (not DATE SHIPPED). Return ONLY the JSON array.`}
        ...
        if(invoices.length===1){
          ...
          updated[i].parsed=invoices[0];
          for(let j=1;j<invoices.length;j++){
            updated.push({id:Date.now()+Math.random()+j,file:updated[i].file+` (pg ${j+1})`,dataUrl:null,status:"parsed",parsed:invoices[j],type:updated[i].type});
          }
        }
      }catch(err){
        updated[i].status="error";
  ...
  const confirmScannedInvoices=async()=>{
    ...
    if(skipped>0)msgs.push(`Skipped ${skipped} duplicate${skipped>1?"s":""}.`);
        ...
        {tab==="costs"&&<div>
          ...
          <div style={{...s.addForm,marginBottom:16}}>
            {!gmailConn?<div>
              ...
              <div style={{fontSize:12,color:"#6b7785",marginBottom:8}}>Search your email for recent vendor invoices (last 90 days). Results include attachment info.</div>
            ...
            {gmailResults&&Array.isArray(gmailResults)&&(()=>{
              ...
              const emailStatus=gmailResults.map(r=>{
                if(!r||r.error)return {fullyImported:false};
                ...
                if(scannableAtts.length===0)return {fullyImported:false};
                const allImported=scannableAtts.every(a=>costEntries.some(c=>c.gmailRef===`gmail:${r.emailId}:${a.attachmentId}`));
                return {fullyImported:allImported};
              });
              const fullyImportedCount=emailStatus.filter(s=>s.fullyImported).length;
              const visibleResults=showImportedEmails?gmailResults:gmailResults.filter((_,i)=>!emailStatus[i].fullyImported);
              return <div style={{marginTop:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6,flexWrap:"wrap",gap:6}}>
                <div style={{fontSize:12,fontWeight:700,color:"#1e293b"}}>
                  {visibleResults.length} email{visibleResults.length!==1?"s":""} {showImportedEmails?"total":"new"}
                  {fullyImportedCount>0&&!showImportedEmails&&<span style={{fontSize:10,color:"#94a3b8",fontWeight:400,marginLeft:6}}>({fullyImportedCount} already imported hidden)</span>}
                </div>
                {fullyImportedCount>0&&<button onClick={()=>setShowImportedEmails(!showImportedEmails)} style={{fontSize:10,fontWeight:600,padding:"3px 8px",background:"#fff",color:C.brand,border:`1px solid ${C.brand}55`,borderRadius:4,cursor:"pointer"}}>{showImportedEmails?"Hide imported":`Show all (${gmailResults.length})`}</button>}
                {(()=>{
                  // Count how many attachments are scannable and NOT already imported/queued
                  const queueable=[];
                  visibleResults.forEach(r=>{
                    ...
                    atts.forEach(a=>{
                      ...
                      if(costEntries.some(c=>c.gmailRef===tag))return;
                      if(scanQueue.some(q=>q.gmailRef===tag))return;
                      queueable.push({r,a});
                    });
                  });
              ...
              {visibleResults.map((r,i)=>{
                ...
                const removeFromList=()=>setGmailResults(prev=>prev.filter(item=>item.emailId!==safeR.emailId));
                return <div key={safeR.emailId||i} style={{padding:"8px 10px",background:safeR.error?"#fef2f2":"#f0f9ff",borderRadius:6,marginBottom:4,border:"1px solid #e2e8f0"}}>
                  {safeR.error?<div style={{fontSize:11,color:C.red,wordBreak:"break-word"}}>{typeof safeR.error==="string"?safeR.error:JSON.stringify(safeR.error).substring(0,500)}</div>:<div>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:6}}>
                      ...
                      <button onClick={removeFromList} title="Remove from list" style={{fontSize:14,padding:"2px 8px",background:"none",color:"#94a3b8",border:"none",cursor:"pointer",fontWeight:700,lineHeight:1}}>×</button>
                    </div>
                    {atts.length>0&&<div style={{marginTop:6,display:"flex",flexDirection:"column",gap:4}}>
                      {atts.map((a,ai)=>{
                        ...
                        const alreadyImported=costEntries.some(c=>c.gmailRef===gmailTag);
              ...
              {visibleResults.length===0&&<div style={{padding:20,textAlign:"center",color:"#94a3b8",fontSize:12,fontStyle:"italic"}}>All emails from this search have already been imported. ✓</div>}
              <button onClick={()=>setGmailResults(null)} style={{...s.canBtn,marginTop:6,fontSize:11}}>Dismiss All Results</button>
            </div>;
            })()}
          </div>
