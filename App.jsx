  ...
  return(
    <div style={s.wrap}>
      ...
      <div style={s.body}>
        ...
        {/* Live Motive drift banner — appears the moment a today assignment differs from Motive */}
        {(()=>{
          const SM={mismatch:{c:C.red,label:"Mismatch"},motive_only:{c:C.accent,label:"Motive only"},board_only:{c:C.brand,label:"Not logged in yet"}};
          const diffs=Object.entries(motiveLiveFlags).filter(([,f])=>SM[f.status]&&f.ts>motiveDismissAt);
          if(!diffs.length)return null;
          const hard=diffs.filter(([,f])=>f.status!=="board_only");
          return <div style={{margin:"0 0 12px",background:hard.length?"#fef2f2":"#fff7ed",border:`1px solid ${(hard.length?C.red:C.accent)}55`,borderRadius:8,padding:"10px 12px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
              <div style={{fontSize:12,fontWeight:800,color:hard.length?C.red:C.accent}}>⚠️ {diffs.length} truck{diffs.length===1?"":"s"} differ{diffs.length===1?"s":""} from Motive (today)</div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:3,marginTop:6}}>
              {diffs.sort((a,b)=>(a[1].status==="board_only"?1:0)-(b[1].status==="board_only"?1:0)).map(([tid,f])=>{const sm=SM[f.status];return(
        ...
        {tab==="board"&&<div>
          <div style={s.weekNav}>
            ...
            {(()=>{
              const td=motiveVehicles.length?buildMotiveComparison(DAYS[Math.min(todayDI(),4)]).filter(r=>r.status!=="match").length:0;
            ...
            <div style={s.modal} onClick={()=>setShowMotiveSync(false)}>
              <div style={{...s.modalBox,maxWidth:580}} onClick={e=>e.stopPropagation()}>
                ...
                {(()=>{
                  ...
                  const diffs=rows.filter(r=>r.status!=="match");
                  const SM={mismatch:{c:C.red,bg:"#fef2f2",label:"Mismatch"},motive_only:{c:C.accent,bg:"#fff7ed",label:"Motive only"},board_only:{c:C.brand,bg:"#eff6ff",label:"Board only"},match:{c:C.green,bg:"#f0fdf4",label:"Match"}};
                  return <div style={{marginTop:4}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                      <div style={{fontWeight:700,fontSize:12,color:C.dark}}>{motiveSyncDay}: {diffs.length} difference{diffs.length===1?"":"s"}</div>
                      {rows.length>0&&<span style={{fontSize:10,color:"#94a3b8"}}>{rows.length-diffs.length}/{rows.length} match</span>}
                    </div>
                    <div style={{maxHeight:230,overflowY:"auto",marginTop:6,display:"flex",flexDirection:"column",gap:4}}>
                      ...
                      {motiveVehicles.length>0&&diffs.length===0&&rows.length>0&&<div style={{fontSize:11,color:C.green,fontWeight:600}}>✓ Board and Motive agree on every mapped truck for {motiveSyncDay}.</div>}
                      {diffs.map(r=>{const sm=SM[r.status];return(
