            ...
            return <div style={s.tWrap}><table style={{...s.table,fontSize:12}}><thead><tr>{["Truck #","Type","Make","Trans","Axle","Status",...DAYS,"Repairs",""].map((h,i)=>{
              ...
              {filteredTrucks.map(t=>{const st2=gTS(t.id,dk);const openR=repairs.filter(r=>r.truckId===t.id&&r.status==="open").length;
                ...
                return <tr key={t.id} style={s.tr}>
                  ...
                  {/* v2.10.38: day cells show driver only — no OOS overlay.
                      Status column on the left already shows current truck status.
                      Historical tStat["truckId-Mon"]="oos" from old/closed repairs
                      was bleeding through here as stale red "OOS" markers. */}
                  {DAYS.map((d,i)=>{
                    const driver=truckDayDriver[`${t.id}-${d}`]||"";
                    const isToday=i===di;
                    return <td key={d} style={{...s.ltd,textAlign:"center",background:isToday?"#f0f9ff":"transparent",fontSize:11,whiteSpace:"nowrap",padding:"4px 6px"}}>
                      {driver
