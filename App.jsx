...
// Resolves to {ok,rec} on success, or {ok:false,notFound} — never rejects, so one
// absent document cannot take the whole load down.
function readDoc(key){
  return window.storage.get(key)
    .then(rec=>({ok:true,rec}))
    .catch(e=>({ok:false,notFound:NOT_FOUND_RE.test(String((e&&e.message)||e)),err:e}));
}
const recOf=r=>(r&&r.ok)?r.rec:null;
// Every read failing for a reason that is NOT "this document does not exist" means the
// database is unreachable, not that this is a fresh install.
function looksOffline(results){
  const reads=results.filter(Boolean);
  return reads.length>0&&reads.every(r=>r&&r.ok===false&&!r.notFound);
}
...
function App(){
  ...
  const[loadError,setLoadError]=useState(null);   // {kind:"timeout"|"offline"} or null
  ...
  useEffect(()=>{let cancelled=false;(async()=>{
    ...
    let docs,rqR;
    try{
      // The review queue is fetched alongside but judged separately: loadReviewQueue
      // catches its own read failure and returns [], so it reports success even when
      // the database is unreachable. Deciding "offline" from the plain document reads
      // keeps that from masking an outage.
      [docs,rqR]=await withTimeout(Promise.all([
        Promise.all(["fl-trucks","fl-drivers","fl-repairs","fl-retired","fl-cores",
                     "fl-retired-drivers","fl-motive-map","fl-miles"].map(readDoc)),
        loadReviewQueue().then(v=>({ok:true,rec:v})).catch(e=>({ok:false,notFound:false,err:e})), // v2.10.48, sharded since v2.17.2
      ]),LOAD_TIMEOUT_MS,"initial load");
    }catch(e){
      // Timed out. Do NOT fall through: the fallbacks below would seed the DEFAULT
      // truck and driver rosters, and the first save after that would write them over
      // a real fleet. Say so and offer a retry instead.
      if(!cancelled)setLoadError({kind:"timeout"});
      return;
    }
    if(cancelled)return;
    if(looksOffline(docs)){setLoadError({kind:"offline"});return;}
...
Always match to the closest fleet number. Use the TOTAL line (including tax) for the total amount. For the date, use DATE INVOICE (not DATE SHIPPED). Return ONLY the JSON array.`}
  ...
  if(loadError)return <div style={{display:"flex",justifyContent:"center",alignItems:"center",height:"100vh",background:"#f5f7fa",padding:20}}>
    <div style={{maxWidth:380,textAlign:"center",color:"#243447"}}>
      ...
      <div style={{fontSize:14,lineHeight:1.5,color:"#5b6b7a",marginBottom:18}}>
        {loadError.kind==="timeout"
          ? "The connection stalled while loading your fleet. This is usually a weak signal rather than a problem with your data."
          : "The database did not answer. Check your connection and try again."}
        <br/><br/>Nothing has been changed or lost — the app stopped here on purpose rather than starting up with blank data.
      </div>
