  ...
  const processGmailAttachment=async(emailResult,att)=>{
    ...
    try{
      ...
      if(isPdf){
        ...
        // Defensive base64 cleanup — strip EVERYTHING that isn't a valid base64 char,
        // then re-pad. This prevents "String did not match the expected pattern" from atob().
        let b64Clean=b64Raw.replace(/-/g,"+").replace(/_/g,"/").replace(/[^A-Za-z0-9+/]/g,"");
        const padLen=(4-(b64Clean.length%4))%4;
        b64Clean=b64Clean+"=".repeat(padLen);
        let binary;
        try{binary=atob(b64Clean);}catch(decodeErr){
          // Surface actual diagnostics so next time we know exactly what's wrong
          const first=b64Clean.substring(0,20);
          const last=b64Clean.substring(Math.max(0,b64Clean.length-20));
          throw new Error(`Base64 decode failed on "${att.filename}" (${b64Clean.length} chars, starts "${first}", ends "${last}"). The file may be corrupted in Gmail — try downloading manually and uploading via "Choose Files".`);
        }
        ...
        const dataUrl=`data:${att.mimeType};base64,${b64Raw}`;
