...
function App(){
  ...
  const[newT,setNewT]=useState({id:"",mk:"Freightliner",md:"",tr:"A",ax:"Single",type:"straight",year:null});
  // Repair form
...
Always match to the closest fleet number. Use the TOTAL line (including tax) for the total amount. For the date, use DATE INVOICE (not DATE SHIPPED). Return ONLY the JSON array.`}
  ...
  const addTruck=()=>{if(!newT.id.trim()||trucks.find(t=>t.id===newT.id))return;saveTrucks([...trucks,{...newT,id:newT.id.trim(),md:(newT.md||"").trim()}]);setNewT({id:"",mk:"Freightliner",md:"",tr:"A",ax:"Single",type:"straight",year:null});setShowAddT(false);};
  ...
  const commitTruckEdit=async()=>{
    ...
    try{
      saveTrucks(trucks.map(t=>t.id===oldId?{...t,id:newId,mk,md,tr,ax}:t));
      if(renumber){
        ...
        toast(`Saved #${newId}${mk?` · ${normalizeMake(mk)}`:""}${md?` ${md}`:""} · ${tr==="A"?"Auto":"Manual"} · ${ax}.`,"success");
      }
      setTruckEdit(null);
    }catch(e){
  ...
  return(
    <div style={s.wrap}>
      ...
      <div style={s.header}><div style={s.hInner}>
        ...
        <div style={s.tabs}>
          {[["dashboard","Dashboard"],["board","Weekly Board"],["fleet","Fleet"],["maint","Maintenance"],["costs","Costs"],["drivers","Drivers"],["attend","Attendance"],["settings","Settings"]].map(([k,l])=>
      ...
      <div style={s.body}>
        ...
        {tab==="fleet"&&<div>
          ...
          {showAddT&&<div style={s.addForm}>
            ...
            <input style={s.fInp} placeholder="Model (e.g. M2 106, Cascadia)" value={newT.md||""} onChange={e=>setNewT({...newT,md:e.target.value})}/>
            <div style={{display:"flex",gap:6}}>
              ...
              <div style={{flex:"1 1 600px",minWidth:0}}>
                ...
                <div style={s.tWrap}>
                  <table style={{...s.table,fontSize:12}}>
                    <thead><tr>{["Truck #","Make","Model","Year","Trans","Axle","Status",...DAYS,"Repairs",""].map((h,i)=>{
                      ...
                      {truckList.map(t=>{
                        ...
                        return <tr key={t.id} style={{...s.tr,background:isDown?"#fef2f2":"transparent"}}>
                          <td style={{...s.ltd,fontWeight:700,fontFamily:"monospace",color:isDown?C.red:C.brand,cursor:"pointer",whiteSpace:"nowrap"}} onClick={()=>setHistoryTruck(t.id)}>
                            #{t.id}
                            <div style={{fontSize:9,color:"#94a3b8",fontWeight:400,fontFamily:"sans-serif"}}>{t.mk}{t.model?` ${t.model}`:""}</div>
                          </td>
                          {DAYS.map((d,i)=>{
                            const driver=truckDayDriver[`${t.id}-${d}`]||"";
                            const isToday=i===di;
      ...
      {historyTruck&&(()=>{
        ...
        return <div style={s.modal} onClick={()=>{setTruckEdit(null);setHistoryTruck(null);}}><div style={{...s.modalBox,maxWidth:520}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:6,alignItems:"center"}}>
              {t&&!truckEdit&&<button onClick={()=>setTruckEdit({orig:t.id,id:t.id,mk:normalizeMake(t.mk)||"Freightliner",md:t.md||"",tr:t.tr==="M"?"M":"A",ax:t.ax==="Tandem"?"Tandem":"Single"})} style={{padding:"5px 10px",fontSize:11,fontWeight:700,background:"#fff",color:C.brand,border:`1px solid ${C.brand}`,borderRadius:5,cursor:"pointer",whiteSpace:"nowrap"}}>✏️ Edit</button>}
          ...
          {t&&truckEdit&&<div style={{background:"#fffbeb",borderRadius:8,padding:12,marginBottom:12,border:`1px solid ${C.accent}66`}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              ...
              <label style={{flex:"1 1 140px",minWidth:120}}>
                ...
                <input style={{...s.fInp,width:"100%",boxSizing:"border-box"}} placeholder="e.g. M2 106" value={truckEdit.md} onChange={e=>setTruckEdit({...truckEdit,md:e.target.value})} onKeyDown={e=>{if(e.key==="Enter")commitTruckEdit();if(e.key==="Escape")setTruckEdit(null);}}/>
              </label>
