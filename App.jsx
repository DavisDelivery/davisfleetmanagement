...
const TRACTOR_MAKES=["Kenworth","Peterbilt","Freightliner","Volvo","Mack","International","Western Star","Other"];
const BOX_MAKES=["Freightliner","Hino","International","Ford","Isuzu","Other"];
...
// v2.16.17: one make list shared by the Add Truck form and the Truck Report editor,
// so the two can't drift apart.
const MAKES=["Freightliner","Hino","International","Ford","Isuzu","Kenworth","Peterbilt","Volvo","Mack","Western Star","Other"];
// v2.30.0: the models this fleet runs, by make. A make listed here gets a Model
// dropdown everywhere a model is entered; any other make keeps free text. Stored makes
// can still be the old codes (FRTLN, HINO), so they resolve through the same aliases
// normalizeMake uses.
...
// v2.31.2: what a truck IS, for every list that names one — "Box · Hino 338",
// "Tractor · Freightliner Cascadia". Lists used to put a box truck's make and a tractor's
// type in the same slot, so a box truck never said it was one, and once the tractors
// were recorded as Freightliners "Freightliner" could mean either. A make of "Tractor"
// is an old placeholder, not a make, and is left out. Same shape as the shop portal's
// truckLabel().
const truckDesc=(t)=>{
  if(!t)return "";
  const kind=t.type==="tractor"?"Tractor":"Box";
  const mk=String(MAKE_ALIASES[t.mk]||t.mk||"").trim();
  const what=[mk.toLowerCase()==="tractor"?"":mk,t.md].filter(Boolean).join(" ");
...
// Bare "YYYY-MM-DD" parses as UTC midnight, which is the PREVIOUS day in US
// timezones. Anchor bare dates to local midnight so week-bucketing and rendering
// don't shift a day; pass strings that already carry a time component through.
function localDate(d){return (typeof d==="string"&&/^\d{4}-\d{2}-\d{2}$/.test(d))?new Date(d+"T00:00:00"):new Date(d);}
...
// alphabetical: Weekly Board, Drivers, Attendance, dispatch, drill-downs. Sorting the
// full name string gives first-name order, matching how the roster already reads.
function sortDrivers(a){return [...(a||[])].sort((x,y)=>String((x&&x.name)||"").localeCompare(String((y&&y.name)||""),undefined,{sensitivity:"base"}));}
// v2.26.0: mirrors of parseMoney() and toYMD() in netlify/functions/auto-sync.mts.
// Complete Fleet Services prints money as "$5,755.63" and dates as "9/4/2026". Both
// were taken at face value: `q.parsed.total||0` made a $5,755 repair a $0 one the
// moment the parser returned the string as printed, and costShardKey below only reads
// YYYY-MM, so "9/4/2026" parks the invoice in the "unknown" shard where no month view
// ever looks. Read what the parser returned properly rather than asking it again — a
// value already in the right shape passes through untouched.
function parseMoney(v){
  if(typeof v==="number")return isFinite(v)?v:0;
  const s=String(v==null?"":v).trim();
  if(!s)return 0;
  const negative=/^\(.*\)$/.test(s)||/^-/.test(s);   // (1,234.00) and -1,234.00 are both credits
  const n=Number(s.replace(/[^0-9.]/g,""));
  return isFinite(n)?(negative?-n:n):0;
}
// Returns "" — not a guess — for anything unreadable, so the caller keeps whatever the
// parser said rather than silently stamping a date the invoice never carried.
function toYMD(v){
  const s=String(v==null?"":v).trim();
  if(!s)return "";
...
// v2.16.7: cost entries are sharded across per-MONTH Firestore docs (fl-costs-<YYYY-MM>)
// so the log never hits Firestore's ~1 MB per-document limit — a month's invoices are
// far smaller than a year's, so no single shard gets close, and adding an invoice only
...
// both are kept.
//
// v2.24.7: kept, but deliberately NOT re-keyed here. This runs on the read path, and
// re-keying on read made the ids in memory diverge from the ids on disk. saveCosts
// then asks costAdditionsToKeep() which stored rows are missing from memory, the
// disk copy of every re-keyed row answered "me" — its old id appears in neither the
// in-memory set nor the snapshot — and each one was re-added as though the server had
// just imported it. One edit in a month turned two rows into three and $450.29 into
// $547.49. Reading must mirror disk exactly; healCollidedIds() below does the
// re-keying on an explicit, saved action instead.
function dedupById(items){
  ...
  for(const it of items){
    ...
    out.push(it);                                            // real row that collided — keep it, id untouched
  }
  return out;
}
// The re-keying half, for callers that persist what they return. Rows that share an id
// with an earlier row keep their content and get a fresh id, so a later delete or
// reassign touches one invoice rather than every row that happened to collide with it.
function healCollidedIds(entries){
  const seen=new Set();const out=[];let healed=0;
  for(const e of (entries||[])){
    const k=e&&e.id!=null?String(e.id):null;
    if(k===null){out.push(e);continue;}
    if(!seen.has(k)){seen.add(k);out.push(e);continue;}
    const fresh={...e,id:newId()};
    seen.add(String(fresh.id));out.push(fresh);healed++;
  }
  return{entries:out,healed};
}
...
// user delete are indistinguishable from memory alone — both are "on disk, not in
// state" — so the snapshot of what this tab last knew the month to hold breaks the tie:
//   on disk, not in snapshot, not in memory → arrived after we loaded → keep
...
function costAdditionsToKeep(stored,prevArr,memArr){
  ...
  return dedupById(stored||[]).filter(e=>e&&e.id!=null&&!memIds.has(String(e.id))&&!prevIds.has(String(e.id)));
}
...
/* ── v2.19.0 ledger integrity. Mirrors splitMultiTruck / entryFingerprint in
   netlify/functions/auto-sync.mts — the browser and the server both import invoices,
   and they have to produce the same rows and the same keys. Change one, change both.

   #0424 was showing $498,362 of spend, roughly ten times the next truck, on a fleet
   where every unit does similar work. Two causes, both here:

   1. A fuel service log is ONE PDF listing every unit filled that day. The parser
      sometimes returned it as a single row carrying the delivery total, pinned on
      whichever unit was printed first — and 0424 is the lowest unit number in this
      fleet, so it was first on 104 of them. The row's own line items disagreed
      ("Diesel - Truck 0451  73.15"), so splitMultiTruck trusts those instead.
   2. The same document was imported over and over — one delivery landed eight
      times — because both dedup keys were unstable: Gmail regenerates attachment
      IDs, and a service log prints no invoice number so the parser invented a
...
// v2.26.0: the one shaping every approved row gets on its way from the review queue
// into the ledger. Both approve paths used to inline `{...e,truckId:normalizeTruckId(...)}`
// and neither touched the date or the total, so a row that reached the queue with
// "9/4/2026" or "$5,755.63" kept them — landing in the "unknown" shard, or at $0. Fixed
// in one place so the two paths cannot drift.
function normalizeApprovedRow(e,fleetIds){
  return {...e,truckId:normalizeTruckId(e&&e.truckId,fleetIds),date:toYMD(e&&e.date)||(e&&e.date),total:parseMoney(e&&e.total)};
}
// v2.26.0: read the date and the money through the same two functions the entry
// builders use. This key is what makes an invoice imported by the browser and the same
// invoice imported by the server the same invoice — so if one side ever holds "9/4/2026"
// or "$5,755.63" where the other holds the normalized form, the key silently stops
// matching and the document imports twice. A value already normalized is unchanged, so
// no fingerprint in existing data moves.
function entryFingerprint(e){
  return [String((e&&e.vendor)||"").trim().toLowerCase(),(toYMD(e&&e.date)||String((e&&e.date)||"")).slice(0,10),
    String((e&&e.truckId)||""),parseMoney(e&&e.total).toFixed(2)].join("|");
}
// A row whose line items name several trucks is a DOCUMENT total, not one truck's
// cost. Split it back out using the amounts the document itself prints. Whatever the
// lines don't account for — tax, delivery, or trucks the parser dropped — becomes one
// INVENTORY row rather than being spread across the trucks that ARE named: inflating
// a real truck's fuel to make the total balance is the same class of error as the bug
// being fixed. INVENTORY is where this app already parks unallocated fuel cost.
function splitMultiTruckEntry(e){
  const lines=Array.isArray(e&&e.lineItems)?e.lineItems:[];
  const per=new Map();
  for(const l of lines){
    const m=TRUCK_IN_DESC.exec(String((l&&l.desc)||""));
    if(!m)continue;
    per.set(m[1],(per.get(m[1])||0)+(Number(l&&l.amount)||0));
  }
 ...
 *
 * Deduped on both sides of the split, which is what makes the button safe to press
 * twice. The same log was sometimes collapsed onto a different truck each time it was
 * imported, so those copies don't collide until after they've been split back out.
 */
function repairCostLedger(entries){
  const sum=a=>a.reduce((s,c)=>s+(Number(c&&c.total)||0),0);
  // Oldest first, so the copy that is kept is the one already referenced elsewhere.
  const byAdded=(a,b)=>String((a&&a.addedAt)||"").localeCompare(String((b&&b.addedAt)||""));
  const dedup=(rows)=>{
    const seen=new Map();const dropped=[];
    for(const c of rows){const k=entryFingerprint(c);if(seen.has(k)){dropped.push(c);continue;}seen.set(k,c);}
    return{kept:[...seen.values()],dropped};
  };
  const first=dedup([...(entries||[])].sort(byAdded));
  let splitDocs=0;
  const split=[];
  for(const c of first.kept){
    const parts=splitMultiTruckEntry(c);
    if(parts.length>1)splitDocs++;
    split.push(...parts);
  }
  const second=dedup(split);
...
/* ── v2.24.9: the first load must be able to fail ─────────────────────────────
   window.storage.get() throws "not found" for a document that does not exist yet —
   the ordinary first-run case — and it also throws, or hangs, when the database
   cannot be reached. Those two must not be treated alike: a missing document means
   "seed the defaults", while an unreachable database means "we know nothing", and
   entering the app on that seeds the DEFAULT truck roster over a real fleet the
   moment anything is saved. So classify the failure, and put a ceiling on the wait. */
const LOAD_TIMEOUT_MS=20000;
// v2.27.0: a ceiling per DOCUMENT, not one shared ceiling over all of them. The whole
// first load used to be `withTimeout(Promise.all([...]))` — a single 20s race against
// nine reads at once — so one slow document threw away the eight that had already
...
function withTimeout(promise,ms,label){
  let timer;
  const timeout=new Promise((_,rej)=>{timer=setTimeout(()=>rej(new Error(`timed out: ${label}`)),ms);});
  return Promise.race([promise,timeout]).finally(()=>clearTimeout(timer));
}
// Always resolves — with the record, or with why there isn't one. Carries the key and
// the elapsed time so a failure can say WHICH read was slow instead of guessing at a
// cause. Never rejects, so no single read can take the others down with it.
function readDoc(key,ms=READ_TIMEOUT_MS){
  const t0=Date.now();
  return withTimeout(window.storage.get(key),ms,key)
    .then(rec=>({key,ok:true,rec,ms:Date.now()-t0}))
    .catch(e=>{
      const msg=String((e&&e.message)||e);
      return {key,ok:false,notFound:NOT_FOUND_RE.test(msg),timedOut:/timed out/i.test(msg),err:msg,ms:Date.now()-t0};
    });
}
const recOf=r=>(r&&r.ok)?r.rec:null;
// v2.27.0: "this document does not exist" is the only failure that may fall through to
// a default. Anything else means we do not KNOW what is stored.
//
// This replaces looksOffline(), which only tripped when EVERY read failed. A single
// failed fl-trucks read therefore fell straight through to
// `setTrucks(...length?t:[...ST,...TR])` and seeded the built-in demo roster over a
// real fleet — and the next save wrote it to disk. The all-or-nothing test was the
// bug: the check has to be per document, on the one that matters.
// v2.28.0: a stalled Firestore stream does not heal itself, and retrying does not
// touch it. Every read is a one-shot listener on ONE shared WebChannel; when that
// channel wedges, the SDK keeps waiting (its own forward-channel timeout is 600s) and
// the next attempt simply queues another listener behind the same dead stream. That is
...
function describeReads(results){
  return results.filter(readFailed)
...
// ── SETTINGS ──
// A place to use the mechanic portal the way a mechanic does, without touching real
// data. Its own component so the iframe's lifetime is exactly the Settings tab's.
function SettingsScreen(){
  // Saves the portal attempted in the test frame, none of which reached the database.
  // Naming what each one would have changed is the useful part: closing a repair also
  // puts the truck back on the Weekly Board, which is easy to forget from the shop side.
  ...
  const reset=()=>{setHeld([]);setFrameKey(k=>k+1);};
  const touched=[...new Set(held.map(k=>k==="fl-repairs"?"repair tickets":k.startsWith("fl-stat-")?"Driver Board status":k==="fl-trucks"?"truck list":k))];
  const box={background:"#fff",border:"1px solid #e2e8f0",borderRadius:10,padding:"16px 18px",marginBottom:16};
  const btn={padding:"8px 14px",fontSize:13,fontWeight:700,borderRadius:6,cursor:"pointer",border:"1px solid #cbd5e1",background:"#fff",color:"#1e293b",minHeight:36};
  return <div style={{maxWidth:980,margin:"0 auto"}}>
    <div style={box}>
      <div style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
        <div>
          <div style={{fontSize:17,fontWeight:800,color:"#1e293b"}}>🔧 Mechanic portal — test view</div>
          <div style={{fontSize:13,color:"#475569",marginTop:4,lineHeight:1.5,maxWidth:640}}>
            This is the same screen your mechanics use, showing your real trucks and open repairs.
            Tap anything — add notes, tick off work, close a repair.{" "}
            <b style={{color:"#b45309"}}>Nothing you do here is saved</b>, so you can try every button
            without changing a real ticket or putting a truck back in service.
          </div>
        </div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          <button style={btn} onClick={()=>setWide(w=>!w)}>{wide?"📱 Phone size":"🖥️ Full width"}</button>
          <button style={btn} onClick={reset} title="Throw away test changes and reload with live data">↺ Reset</button>
          <a href="/mechanic/" target="_blank" rel="noopener" style={{...btn,textDecoration:"none",display:"inline-flex",alignItems:"center",background:"#1e5b92",color:"#fff",border:"1px solid #1e5b92"}} title="Opens the REAL portal — changes there are saved">Open live portal ↗</a>
        </div>
      </div>
      <div data-testid="held-summary" style={{marginTop:12,fontSize:12,color:held.length?"#b45309":"#64748b",fontWeight:held.length?700:400}}>
        {held.length
          ?`${held.length} save${held.length===1?"":"s"} held back (${touched.join(", ")}) — the test view shows ${held.length===1?"it":"them"}, but nothing reached your real data. Reset to start over.`
          :"Nothing changed yet. Saves made in the test view are held back here and never reach your real data."}
      </div>
    </div>
...
function App(){
  ...
  const[showAdmin,setShowAdmin]=useState(false); // reveals the developer Data Diagnostic tools (tap the version number)
  // In-app toast notifications (replaces browser toast() for non-blocking notices).
  const[toasts,setToasts]=useState([]);
  const toastSeq=useRef(0);
  const dismissToast=useCallback(id=>setToasts(t=>t.filter(x=>x.id!==id)),[]);
  const toast=useCallback((msg,type)=>{
    const m=String(msg==null?"":msg);
    if(!type)type=/fail|error|invalid|✗|❌|out of service/i.test(m)?"error":/✓|✅|copied|imported|deleted|merged|redistribut|caught up|saved|added/i.test(m)?"success":"info";
    const id=++toastSeq.current;
    setToasts(t=>[...t,{id,msg:m,type}]);
    setTimeout(()=>setToasts(t=>t.filter(x=>x.id!==id)),type==="error"?8000:5000);
  },[]);
  // Promise-based confirm/prompt modal (replaces blocking window.confirm/prompt).
  // uiConfirm(msg,opts) resolves true/false; uiPrompt(msg,default,opts) resolves string|null.
  const[dialog,setDialog]=useState(null);
  const uiConfirm=useCallback((message,opts={})=>{
    // Destructive messages get a red confirm button automatically.
    const danger=opts.danger!==undefined?!!opts.danger:/delete|permanently|cannot be undone/i.test(String(message));
    return new Promise(resolve=>setDialog({kind:"confirm",message,okLabel:opts.okLabel||"OK",cancelLabel:opts.cancelLabel||"Cancel",danger,resolve}));
  },[]);
  const uiPrompt=useCallback((message,defaultValue="",opts={})=>new Promise(resolve=>setDialog({kind:"prompt",message,value:defaultValue==null?"":String(defaultValue),placeholder:opts.placeholder||"",okLabel:opts.okLabel||"OK",resolve})),[]);
  const closeDialog=useCallback((result)=>{setDialog(d=>{if(d)d.resolve(result);return null;});},[]);
  ...
  const[statDrill,setStatDrill]=useState(null); // dashboard stat-card drill-down: key of the tile clicked, or null
  ...
  const[newT,setNewT]=useState({id:"",mk:"Freightliner",md:"",tr:"A",ax:"Single",del:false,type:"straight",year:null});
  // Repair form
  ...
  // whole order. Showing every note box at once would bury the Repair Completed button.
  const[openItemNote,setOpenItemNote]=useState(null);
  const[repairTruckSearch,setRepairTruckSearch]=useState(""); // filter for the Log New Repair truck picker
  // v2.16.10: "who's using this device" — there are no logins, so notes had no
  // author (you could see WHEN a note was added but never WHO). This per-device
  // name lives in localStorage (same key as the mechanic portal) and is stamped
  // onto each note added from this device. Honour-system label, not auth.
  const[deviceUser,setDeviceUser]=useState(()=>{try{return(localStorage.getItem("fl-device-user")||"").trim();}catch(e){return "";}});
  const saveDeviceUser=useCallback(n=>{const v=(n||"").trim();setDeviceUser(v);try{localStorage.setItem("fl-device-user",v);}catch(e){}},[]);
  // v2.16.15: ask for the name at the moment it matters — when opening a repair,
  // adding a note, or completing a repair — the way the shop portal already does.
  // Before this, the fleet app only stamped a name if you'd happened to set it via
  // the chip and otherwise recorded nothing, silently losing the attribution.
  const ensureDeviceUser=useCallback(async()=>{
    if(deviceUser)return deviceUser;
    const next=await uiPrompt("Who's using this device?\n\nYour name is recorded on repairs you open, notes you add, and repairs you complete. (Leave blank to skip.)","",{placeholder:"e.g. Ryan"});
    if(next===null)return "";
    const v=(next||"").trim();
    saveDeviceUser(v);
    return v;
  },[deviceUser,saveDeviceUser,uiPrompt]);
  const askDeviceUser=useCallback(async()=>{
    const next=await uiPrompt("Who's using this device?\n\nYour name gets stamped on every note you add here.",deviceUser,{placeholder:"e.g. Ryan"});
    if(next===null)return;
    saveDeviceUser(next);
    toast((next||"").trim()?`Notes will be signed "${next.trim()}"`:"Name cleared — notes won't be signed");
  },[deviceUser,saveDeviceUser,uiPrompt,toast]);
  // Truck history modal
  ...
  // v2.10.13: custom until-date (YYYY-MM-DD). Default to today (meaning: no upper bound).
  const[autoScanCustomUntil,setAutoScanCustomUntil]=useState(()=>todayYMD());
  ...
  const[costSearch,setCostSearch]=useState(""); // v2.10.42: free-text search across costs
  const[pdfPreview,setPdfPreview]=useState(null); // v2.10.44: in-app PDF preview modal {url, label}
  const[attachInvoice,setAttachInvoice]=useState(null); // v2.10.45: attach-PDF modal for records without one {record, recordType:'core'|'cost', label}
  const[attachStatus,setAttachStatus]=useState(""); // v2.10.45: upload progress / error text
  ...
  const[showFuelCompareReport,setShowFuelCompareReport]=useState(false); // v2.10.1
  ...
  const[attendWeeksBack,setAttendWeeksBack]=useState(52); // v2.10.21: number of recent weeks to include in attendance stats; "all" = no filter

  const wk=wK(weekDate);
  // v2.24.9: the first load had no time limit and no way to fail. Every read below is
  // written `.catch(()=>null)`, which handles a read that REJECTS — but a Firestore
  // read on a stalled connection does not reject, it simply never settles. Promise.all
  // then never resolves, setLoaded(true) never runs, and the app sits on its bare
  // spinner forever with nothing to act on. Reported twice from the yard, both times on
  // a weak LTE signal, which is exactly when the SDK hangs rather than erroring.
  const[loadError,setLoadError]=useState(null);   // {kind:"timeout"|"unreachable",detail:[...]} or null
  const[loadPartial,setLoadPartial]=useState(null); // reads that failed without being fatal
  // The ledger loads behind the app now, so there is a window where costEntries is []
  // because it has not arrived — NOT because there are no invoices. Saving in that
  ...
  const loadCostsFromShards=useCallback(async(migrate=false)=>{
    try{
      ...
      // storage.list now returns the documents it already read, so the common path
      // needs no follow-up gets at all. Fall back per-key if a value is missing.
      const cached=(lst&&lst.values)||null;
  ...
  useEffect(()=>{let cancelled=false;(async()=>{
    ...
    // The roster is the one thing we refuse to guess at. Entering the app with the
    // built-in demo fleet in place of a real one is not a display glitch: the first
    // save writes it over what is stored. A document that genuinely does not exist yet
    // is a fresh install and may seed defaults; a read that FAILED may not.
    if(readFailed(tR)||readFailed(dR)){
    ...
    const t=pj(recOf(tR),null);setTrucks(Array.isArray(t)&&t.length?t:[...ST,...TR]);
    const d=pj(recOf(dR),null);setDrivers(sortDrivers(Array.isArray(d)&&d.length?d:[...ID]));
    // The rest are additive: an empty list reads as "none yet", which is what an absent
    // document means anyway, and nothing seeds a default over them.
    ...
    // Anything that failed but is not fatal is still worth saying out loud rather than
    // rendering a half-empty screen that looks like missing data.
    const partial=describeReads(docs);
    if(partial.length){
      setLoadPartial(partial);
      toast(`Some data didn't load: ${partial.join("; ")}`,"warn");
    }

    // v2.27.0: the app opens now. The ledger is far the largest thing this app reads —
    // a documentId() query plus every month's shard, the bulk of the payload — and it
    // used to be awaited HERE, before the first render, with no ceiling of any kind. A
    // slow ledger therefore held the whole app on the spinner indefinitely, which is
    // the other half of what "won't load" has meant. Costs belong to one tab; the
  ...
  // it was wasted. storage.list() is a documentId() range query that ALREADY returns
  // every matching document's value (index.html) — loadCostsFromShards has consumed
  // lst.values since it was fixed, this path never was. It ignored them and re-fetched
  // each week one at a time, with `await` INSIDE a for loop, so a two-year install did
  // ~104 sequential round trips on the same wedged-prone stream: measured at 16s of
  // continuous traffic at 150 ms/op, growing by another round trip every week forever.
  //
  // It also had no cancellation guard while depending on `wk`, so every click of
  // "← Prev" started another full sweep on top of the last. Measured: six clicks in
  // under a second took one cold start from 123 Firestore operations to 514, still
  // running 30s later. That is a self-inflicted denial of service on the one
  // connection every read in the app shares.
  useEffect(()=>{let cancelled=false;(async()=>{if(!loaded)return;
    const weeks={};
    try{
      const keys=await window.storage.list('fl-asgn-');
      if(cancelled)return;
      if(keys&&keys.keys&&keys.keys.length>0){
        const have=(keys&&keys.values)||null;
        for(const key of keys.keys){
          // The value is already in hand from the range query. Only fall back to a
          // per-key read if the query somehow did not carry it.
          try{
            let raw=have&&have[key]!==undefined?have[key]:null;
            if(raw===null){const r=await window.storage.get(key);if(cancelled)return;raw=r&&r.value;}
            if(raw!=null)weeks[key.replace('fl-asgn-','')]=JSON.parse(raw);
          }catch(e){}
        }
      }else{
    ...
    if(!cancelled)setAttendWeeks(weeks);
  })();return()=>{cancelled=true;};},[loaded,wk]);
  ...
  const sv=useCallback(async(k,v)=>{
    try{
      ...
      if(res===null){
        ...
        // The banner can be scrolled off; the toast cannot be missed. toast() already
        // classifies anything matching /fail|error/ as the 8-second error style.
        toast(`Save failed — ${k}`);
        return false;
  ...
  // v2.19.1: the server APPENDS to a month shard (readShardedList → appendSharded in
  // auto-sync.mts); this tab used to OVERWRITE it from a snapshot taken at page load,
  // and nothing listens on fl-costs. So approving one review item on a tab that had
  // been open a while silently deleted every invoice imported since that load — worst
  // for the 365-day backlog sweep, whose rows are outside the 30-day re-crawl and
  // never come back, and for hand-keyed rows, which have no gmailRef at all.
  //
  // Re-read the month before writing it, and let costAdditionsToKeep() decide what
  // survives. This is only the IO half; the rule itself is module-level and tested.
  const readMonthAdditions=useCallback(async(k,memItems)=>{
    try{
      const lst=await window.storage.list(`fl-costs-${k}`).catch(()=>null);
      if(!lst||!Array.isArray(lst.keys)||!lst.keys.length)return[];
      const vals=lst.values||{};
      const stored=[];
      for(const key of lst.keys){
        let raw=vals[key];
        if(raw===undefined){const rec=await window.storage.get(key).catch(()=>null);raw=rec&&rec.value;}
        if(raw==null)continue;
        try{const arr=JSON.parse(raw);if(Array.isArray(arr))stored.push(...arr);}catch(e){}
      }
      let prev=[];
      try{const p=JSON.parse(costShardJsonRef.current[k]||"[]");if(Array.isArray(p))prev=p;}catch(e){}
      return costAdditionsToKeep(stored,prev,memItems);
    }catch(e){return[];}
  },[]);
  const saveCosts=useCallback(c=>{
    // v2.27.0: refuse to write the ledger before it has been read. costEntries starts
    // empty and the shard bookkeeping (costShardJsonRef) starts blank, so a save in
    // that window looks exactly like "every month is now empty" — the same shape as the
    // roster bug above, with the invoice history as the casualty.
    if(!costsReadyRef.current){
      toast("Still loading the cost ledger — try that again in a moment.");
      return false;
    }
    setCostEntries(c);
    ...
    const touched=[];
    for(const k of keys){if(costShardJsonRef.current[k]!==JSON.stringify(groups[k]))touched.push(k);}
    for(const k of Object.keys(costShardJsonRef.current)){if(!keys.has(k)&&costShardJsonRef.current[k]!=="[]")touched.push(k);}
    if(!touched.length)return true;   // nothing to write is a success, not a refusal
    (async()=>{
      const recovered=[];
      for(const k of touched){
        const mem=groups[k]||[];
        const add=await readMonthAdditions(k,mem);
        const merged=add.length?[...mem,...add]:mem;
        if(add.length)recovered.push(...add);
        bump(k,merged);
        costShardJsonRef.current[k]=JSON.stringify(merged);
      }
      if(recovered.length){
        setCostEntries(prev=>dedupById([...prev,...recovered]));
        toast(`Kept ${recovered.length} invoice${recovered.length===1?"":"s"} imported while this tab was open`);
      }
    })();
    return true;
  },[svSharded,readMonthAdditions,toast]);
  ...
  const exportCostsCSV=(entries,filename)=>{
    ...
    const rows=[["Date","Truck","Vendor","Category","Amount","Invoice #","Notes"]];
    [...entries].sort((a,b)=>String(a.date||"").localeCompare(String(b.date||""))).forEach(c=>rows.push([c.date||"",c.truckId||"",c.vendor||"",c.category||"",(c.total!=null?c.total:""),c.invoiceNum||"",c.notes||""]));
    const csv=rows.map(r=>r.map(esc).join(",")).join("\n");
    const url=URL.createObjectURL(new Blob([csv],{type:"text/csv"}));
    const a=document.createElement("a");a.href=url;a.download=filename||`cost-ledger-${todayYMD()}.csv`;a.click();URL.revokeObjectURL(url);
  };
  // v2.16.7: archive invoices dated before `cutoffYear` — export them to CSV, move them
  // into archive shards (fl-arch-costs-<year>, NOT loaded by the active ledger), then drop
  // them from the active log so it stays lean. Nothing is deleted (CSV + archive doc keep it).
  const archiveOldCosts=async(cutoffYear)=>{
    const cy=Number(cutoffYear);
    if(!Number.isFinite(cy)){toast("Pick a valid cutoff year.");return;}
    const old=costEntries.filter(c=>{const ys=String(c.date||"").slice(0,4);const y=/^\d{4}$/.test(ys)?Number(ys):NaN;return Number.isFinite(y)&&y<cy;});
    ...
    toast(`Archived ${old.length} invoice(s) before ${cy}. Active ledger trimmed & CSV downloaded.`);
  };
  ...
  const readReviewQueueOnce=useCallback(async()=>{
    ...
    const docs=await Promise.all(keys.map(k=>(have&&have[k]!==undefined)
      ...
      :window.storage.get(k).catch(()=>null)));
  ...
  // Live drift check: when you change a TODAY assignment we call Motive for just
  // that truck and flag if the board driver differs from Motive's current driver.
  // (Motive's current_driver only reflects today's actual driver, so the live
  ...
  const buildMotivePlan=(day)=>{
    ...
    trucks.forEach(t=>{
      ...
      // Motive exposes the assignable driver as the nested `permanent_driver`
      // object (current_driver is read-only ELD activity, so we diff against the
      // permanent driver we set via the assign endpoint).
      const cur=mv&&mv.permanent_driver&&mv.permanent_driver.id!=null?String(mv.permanent_driver.id):null;
  ...
  // Read-only audit: for the given day compare who the board has on each mapped
  // truck vs the driver ASSIGNED to it in Motive (vehicle.permanent_driver — the
  // assignment, which is what the board plans; current_driver is just whoever is
  // physically driving right now). Matches by mapped Motive id, falling back to name.
  const buildMotiveComparison=(day)=>{
    const norm=x=>String(x||"").toLowerCase().replace(/[^a-z0-9]/g,"");
    const rows=[];
    trucks.forEach(t=>{
      const vid=motiveMap.vehicles[t.id];
      if(!vid)return; // truck not mapped to a Motive vehicle
      const mv=motiveVehicles.find(v=>String(v.id)===String(vid));
      const pd=mv&&mv.permanent_driver?mv.permanent_driver:null;
      const motiveId=pd&&pd.id!=null?String(pd.id):null;
      const motiveName=pd?`${pd.first_name||""} ${pd.last_name||""}`.trim():null;
      const drv=drivers.find(d=>asgn[`${d.name}-${day}`]===t.id);
      const boardName=drv?drv.name:null;
      const boardMotiveId=drv?(motiveMap.drivers[drv.name]||null):null;
      if(!boardName&&!motiveId)return; // nobody on either side — nothing to compare
      let status;
      if(boardName&&motiveId){
        const idMatch=boardMotiveId&&String(boardMotiveId)===String(motiveId);
        const nameMatch=motiveName&&norm(boardName)===norm(motiveName);
        status=(idMatch||nameMatch)?"match":"mismatch";
      }else if(boardName)status="board_only"; // board has a driver, Motive shows none
      else status="motive_only";              // Motive shows a driver the board doesn't
      rows.push({truckId:t.id,boardName,motiveName,status});
    });
    const order={mismatch:0,motive_only:1,board_only:2,match:3};
    rows.sort((a,b)=>(order[a.status]-order[b.status])||String(a.truckId).localeCompare(String(b.truckId)));
    return rows;
  };
  // Check one or more trucks against Motive right now and record flags. Compares
  // the board to each truck's Motive permanent (assigned) driver — the assignment,
  // not whoever is physically driving. One vehicles call covers all trucks.
  const liveCheckTrucks=useCallback(async(truckIds,day,asgnMap)=>{
    if(!truckIds.length)return;
    const norm=x=>String(x||"").toLowerCase().replace(/[^a-z0-9]/g,"");
    const boardOf=tid=>{const d=drivers.find(dr=>asgnMap[`${dr.name}-${day}`]===tid);return d?d.name:null;};
    const ts0=Date.now();
    setMotiveLiveFlags(f=>{const n={...f};truckIds.forEach(tid=>{n[tid]=motiveMap.vehicles[tid]?{...(f[tid]||{}),status:"checking",boardName:boardOf(tid),ts:ts0}:{status:"unmapped",boardName:boardOf(tid),ts:ts0};});return n;});
    const mapped=truckIds.filter(tid=>motiveMap.vehicles[tid]);
    if(!mapped.length)return;
    try{
      const r=await fetch("/api/motive?action=vehicles");
      const j=await r.json().catch(()=>({}));
      if(!r.ok)throw new Error(j.error||`HTTP ${r.status}`);
      const vs=(j.vehicles||[]).map(x=>x.vehicle||x);
      setMotiveVehicles(vs); // keep modal/badge fresh too
      const pdById={};vs.forEach(v=>{pdById[String(v.id)]=v.permanent_driver||null;});
      const ts=Date.now();
      setMotiveLiveFlags(f=>{const n={...f};mapped.forEach(tid=>{
        const vid=motiveMap.vehicles[tid];
        const pd=pdById[String(vid)]||null;
        const motiveName=pd?`${pd.first_name||""} ${pd.last_name||""}`.trim():null;
        const motiveId=pd&&pd.id!=null?String(pd.id):null;
        const drv=drivers.find(d=>asgnMap[`${d.name}-${day}`]===tid);
        const boardName=drv?drv.name:null;
        const boardMotiveId=drv?(motiveMap.drivers[drv.name]||null):null;
        let status;
        if(boardName&&motiveId)status=((boardMotiveId&&String(boardMotiveId)===String(motiveId))||(motiveName&&norm(boardName)===norm(motiveName)))?"match":"mismatch";
        else if(boardName)status="board_only"; // assigned on board, no permanent driver set in Motive
        else if(motiveId)status="motive_only"; // Motive has a permanent driver the board doesn't have on this truck
        else status="clear";
        n[tid]={status,motiveName,boardName,ts};
      });return n;});
    }catch(e){setMotiveLiveFlags(f=>{const n={...f};mapped.forEach(tid=>{n[tid]={...(f[tid]||{}),status:"error",msg:e.message,ts:Date.now()};});return n;});}
  },[motiveMap,drivers]);
  // Fire the live check whenever a TODAY assignment actually changes (user edit),
  // not on week navigation or the initial load.
  const prevAsgnRef=useRef(null);const prevWkRef=useRef(wk);
  useEffect(()=>{
    const prev=prevAsgnRef.current;const sameWeek=prevWkRef.current===wk;
    prevAsgnRef.current=asgn;prevWkRef.current=wk;
    if(!motiveLiveOn||prev===null||!sameWeek)return;          // first load or week switch → skip
    if(wK(weekDate)!==wK(new Date()))return;                  // only the current calendar week
    const todayKey=DAYS[Math.min(todayDI(),4)];
    const trucksToCheck=new Set();
    Object.keys({...prev,...asgn}).forEach(k=>{
      if(!k.endsWith(`-${todayKey}`)||prev[k]===asgn[k])return;
      [asgn[k],prev[k]].forEach(tid=>{if(tid&&!OFF_OPTS.includes(tid))trucksToCheck.add(tid);});
    });
    liveCheckTrucks([...trucksToCheck],todayKey,asgn);
  },[asgn,wk,motiveLiveOn,weekDate,liveCheckTrucks]);
  ...
  // Uses Firestore onSnapshot on the kv/fl-repairs doc. The listener fires:
  //   1. Immediately on mount with current data (harmless duplicate of init load)
  //   2. Every time the mechanic portal (or any other client) writes to fl-repairs
  // Unsubscribes cleanly on unmount. Only active after initial load completes
  // to avoid a race with the init fetch.
  useEffect(()=>{
    if(!loaded)return;
    if(!window.db)return;
    const unsub=window.db.collection("kv").doc("fl-repairs").onSnapshot(snap=>{
      if(!snap.exists)return;
      try{
        const fresh=JSON.parse(snap.data().v);
        // Only update state if the data actually differs from what we have
        // (avoids loops when WE are the writer, since our own saveRepairs
        // will trigger onSnapshot for our own write)
        setRepairs(prev=>{
          const prevStr=JSON.stringify(prev);
          const freshStr=JSON.stringify(fresh);
          return prevStr===freshStr?prev:fresh;
        });
      }catch(e){console.log("fl-repairs onSnapshot parse error:",e);}
    },err=>{console.log("fl-repairs listener error:",err);});
    return()=>unsub();
  },[loaded]);

  // v2.10.0: manually mark a core as returned (for cases where auto-reconcile missed it)
  const markCoreReturned=async(coreId)=>{
    const today=todayYMD();
    const note=await uiPrompt("Credit memo # or note (optional):","");
    saveCores(cores.map(c=>c.id===coreId?{...c,status:"returned",returnedDate:today,returnedCreditInvoice:note||"(manual)"}:c));
  };
  const unreturnCore=async(coreId)=>{
    if(!await uiConfirm("Move this core back to Outstanding?"))return;
  ...
  const deleteCore=async(coreId)=>{
    if(!await uiConfirm("Delete this core record? This does NOT delete the associated cost entry."))return;
    saveCores(cores.filter(c=>c.id!==coreId));
  };

  // v2.10.41: Resolve the best clickable source URL for an invoice record.
  // v2.10.44: PURE in-app only — no Gmail fallbacks. Returns the in-app file URL
  // (/api/invoice-file?key=...) for the stored PDF, or null if no PDF exists.
  // Resolution: 1) record's own fileUrl (PDF in Netlify Blobs), 2) fileUrl from
  // a matching cost entry (covers cores that share an invoice with a cost entry).
  // If neither path yields a stored file → returns null → invoice renders as plain text.
  const findInvoiceSourceUrl=(record)=>{
    if(!record)return null;
    if(record.fileUrl)return record.fileUrl;
    // Fallback for cores: find a cost entry with matching invoiceNum and use its fileUrl
    const invNum=record.returnedCreditInvoice||record.invoiceNum;
    if(!invNum)return null;
    const matchingEntry=costEntries.find(c=>c.invoiceNum===invNum&&c.fileUrl);
    return matchingEntry?matchingEntry.fileUrl:null;
  };

  // v2.10.45: Unified invoice-click handler.
  // If a PDF is stored → open it in the in-app preview modal.
  // If NO PDF is stored → open the attach-PDF modal so the user can upload one
  // and have it permanently attached to this record (no more orphan invoices).
  const openInvoice=(record,recordType,labelPrefix)=>{
    const url=findInvoiceSourceUrl(record);
    const invNum=record.returnedCreditInvoice||record.invoiceNum||"";
    const vendor=record.vendor||"";
    const label=`${labelPrefix||"Inv"} ${invNum}${vendor?` — ${vendor}`:""}`;
    if(url){setPdfPreview({url,label});}
    else{setAttachInvoice({record,recordType,label});setAttachStatus("");}
  };

  // v2.10.45: Upload a file to /api/invoice-file and attach the resulting fileUrl
  // to the target record. Updates the right collection (cores or cost entries),
  // saves to Firebase, then auto-opens the PDF preview with the just-uploaded file.
  const uploadInvoiceAttachment=async(file)=>{
    if(!file||!attachInvoice)return;
    const {record,recordType,label}=attachInvoice;
    setAttachStatus(`Reading "${file.name}"…`);
    try{
      // Read file as base64 — FileReader handles large files without stack overflow
      const base64=await new Promise((resolve,reject)=>{
        const r=new FileReader();
        r.onload=()=>{const s=r.result;resolve(s.split(",")[1]||"");};
        r.onerror=()=>reject(new Error("File read failed"));
        r.readAsDataURL(file);
      });
      const sizeMB=(base64.length*3/4/1024/1024).toFixed(2);
      if(base64.length*3/4>5*1024*1024){setAttachStatus(`✗ File too large (${sizeMB}MB) — max is 5MB`);return;}
      setAttachStatus(`Uploading ${sizeMB}MB…`);
      const up=await fetch("/api/invoice-file",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({filename:file.name,mimeType:file.type||"application/pdf",data:base64})
      });
      if(!up.ok){setAttachStatus(`✗ Upload failed: HTTP ${up.status}`);return;}
      const upData=await up.json();
      if(!upData.success){setAttachStatus(`✗ Upload failed: ${upData.error||"unknown"}`);return;}
      // Patch the record's fileUrl + fileKey on the right collection
      const patch={fileUrl:upData.url,fileKey:upData.key};
      if(recordType==="core"){
        saveCores(cores.map(c=>c.id===record.id?{...c,...patch}:c));
      }else if(recordType==="cost"){
        saveCosts(costEntries.map(c=>c.id===record.id?{...c,...patch}:c));
      }
      setAttachStatus(`✓ Attached "${file.name}". Opening…`);
      setTimeout(()=>{
        setAttachInvoice(null);
        setAttachStatus("");
        setPdfPreview({url:upData.url,label});
      },400);
    }catch(e){setAttachStatus(`✗ ${e.message||"Upload failed"}`);}
  };
  ...
  const redistributeFuelFoxOverhead=()=>{
    ...
    if(overheadRows.length===0){toast("No FuelFox Invoice overhead rows found to redistribute. Make sure you've imported both the Service Log AND Invoice PDFs.");return;}
  ...
  const runAiAnalysis=async(customQ)=>{
    ...
    try{
      ...
      const asgnSummary={};
      drivers.forEach(d=>{const trucks=[];DAYS.forEach(day=>{const v=asgn[`${d.name}-${day}`]||"";if(v&&!OFF_OPTS.includes(v))trucks.push(v);});if(trucks.length)asgnSummary[d.name]={role:d.role,trucks};});
      ...
      Object.entries(attendWeeks).forEach(([wk,wkAsgn])=>{
        drivers.forEach(d=>{
          ...
          DAYS.forEach(day=>{
            ...
            if(v==="CALLED OUT")attSummary[d.name].calledOut++;
            else if(v==="NO SHOW")attSummary[d.name].noShow++;
...
Format your response as clear sections with headers using ** for bold. Use specific numbers, names, and amounts. Be direct and actionable — this is for the owner who makes operational decisions daily. Keep it concise but thorough.`}
  ...
  const clearAllDismissed=useCallback(async()=>{
    if(!await uiConfirm("Clear the dismissed-email list? Previously dismissed emails will reappear in future searches."))return;
    setDismissedMsgs({});
    window.storage.delete("fl-gmail-dismissed").catch(()=>{});
  },[]);

  const disconnectGmail=async()=>{
    if(!await uiConfirm("Disconnect Gmail? You'll need to re-authorize to fetch invoices again."))return;
    try{await window.storage.delete("fl-gmail-token");}catch(e){}
    setGmailConn(null);
    setGmailResults(null);
  };

  // v2.10.3: Fetch raw email results for one vendor (returns array, no state side-effects)
  // v2.10.13: optional beforeDate to support custom date ranges (not just days-back).
  // beforeDate format: "YYYY/MM/DD" (Gmail query syntax)
  const fetchVendorEmails=async(vendor,daysBack=30,beforeDate=null)=>{
    ...
    try{
      const resp=await fetch("/api/gmail-search",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({refresh_token:gmailConn.refresh_token,vendor,afterDate,beforeDate:beforeDate||""})});
      const data=await resp.json();
      if(data.error)return [{error:String(data.error).substring(0,300),_vendor:vendor}];
      if(Array.isArray(data.results))return data.results.map(r=>({...r,_vendor:vendor}));
      return [];
    }catch(err){return [{error:(err&&err.message)||"Failed to fetch",_vendor:vendor}];}
  };

  const fetchGmailInvoices=async(vendor,daysBack=30,beforeDate=null)=>{
    ...
    setGmailResults(null);
    const results=await fetchVendorEmails(vendor||"peach state",daysBack,beforeDate);
    if(results.length===0){
      const rangeDesc=beforeDate?`in the selected date range`:`in the last ${daysBack} days`;
      setGmailResults([{error:`No matching emails found ${rangeDesc}.`}]);
    }else{
  ...
  // v2.10.3: one-click "Scan All New" — fetches all known vendors, queues only
  // unimported attachments, then auto-triggers scanInvoices via useEffect below.
  const[autoScanPhase,setAutoScanPhase]=useState(null); // null | "searching" | "queueing"
  const[autoScanProgress,setAutoScanProgress]=useState({total:0,done:0});
  const[autoScanPending,setAutoScanPending]=useState(false);

  const scanAllNewInvoices=async(daysBack=30,beforeDate=null)=>{
    if(!gmailConn||!gmailConn.refresh_token){
      toast("Connect Gmail first (Costs tab → Connect Gmail button).");
      return;
    }
    setAutoScanPhase("searching");
    setAutoScanProgress({total:0,done:0});

    // Fan out to all known vendors in parallel
    const vendorNames=knownVendors.map(v=>v.name.toLowerCase());
    const perVendor=await Promise.all(vendorNames.map(v=>fetchVendorEmails(v,daysBack,beforeDate)));
    const allResults=perVendor.flat();

    // Show them in the UI immediately so user sees what we found
    const rangeDesc=beforeDate?`in the selected date range`:`in the last ${daysBack} days`;
    setGmailResults(allResults.length>0?allResults:[{error:`No matching emails found ${rangeDesc} across any vendor.`}]);

    // Build dedup set (copy of render-side attIsImported logic)
    ...
    if(queueable.length===0){
      ...
      toast(`✓ All caught up. No new invoices in the last ${daysBack} days.`);
      return;
    }

    // Queue phase — sequentially to respect rate limits and preserve error reporting
    setAutoScanPhase("queueing");
    setAutoScanProgress({total:queueable.length,done:0});
    for(let i=0;i<queueable.length;i++){
      await processGmailAttachment(queueable[i].r,queueable[i].a);
      setAutoScanProgress({total:queueable.length,done:i+1});
    }

    // Flag: once scanQueue state settles, useEffect below will kick off scanInvoices()
    setAutoScanPending(true);
    setAutoScanPhase(null);
  };

  // ── v2.10.48: Server-side auto-sync helpers ──
  // The /api/auto-sync function does Gmail search → PDF extract → Anthropic parse
  // → confidence gate → write to Firestore. We just trigger it and poll status.

  const triggerServerSync=async(daysBack=7)=>{
    setSyncing(true);
    try{
      const resp=await fetch("/api/auto-sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({daysBack})});
      const data=await resp.json();
      if(!resp.ok){
  ...
  // v2.17.0: one-click backlog sweep over the durable server-side work queue. Each
  // pass is one time-budgeted /api/auto-sync run; the server keeps a persistent queue
  // and a resumable discovery cursor in Blobs, so passes spend their budget importing
  // instead of re-scanning Gmail from scratch. We keep passing until the server says
  // done (mailbox fully crawled AND queue drained). Cost-safe: attachments are deduped
  // BEFORE the paid AI call. Stoppable mid-run — the queue survives and resumes.
  const catchUpBacklog=async(daysBack=365)=>{
    ...
    if(!await uiConfirm(`Catch up the invoice backlog?\n\nScans about the last ${months} months of vendor email and imports anything not already in the ledger. Confident invoices import automatically; uncertain ones go to the Review Queue.\n\nIt keeps running on the server until everything is imported — you can close this tab and it carries on. The counter shows how many attachments are left. Already-imported invoices are skipped for free; only genuinely new ones use AI credits. Stop pauses it any time; it resumes where it left off.`))return;
    backlogStopRef.current=false;
    let pass=0,imported=0,queued=0,scanned=0,stalled=0,stuck=0,remaining=null,busyWaits=0;
    const MAX_PASSES=60;
    setBacklog({running:true,pass:0,imported:0,queued:0,scanned:0,remaining:null});
    while(pass<MAX_PASSES){
      if(backlogStopRef.current){
        /* v2.17.1: the server drains on its own (auto-continue) — Stop must tell IT,
           not just this loop, or it keeps importing after the button says stopped. */
        try{await fetch("/api/auto-sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({stopChain:true})});}catch(e){}
        setBacklog({running:false,pass,imported,queued,scanned,remaining,note:"Stopped — the queue is saved; the next scheduled sync or another click resumes it."});
        toast(`Backlog sweep stopped — imported ${imported}, queued ${queued}.`);return;
      }
      setBacklog({running:true,pass:pass+1,imported,queued,scanned,remaining});
      let data;
      try{
        ...
        setBacklog({running:false,pass,imported,queued,scanned,remaining,note:`Failed: ${e.message}`});
        toast(`Backlog sweep failed: ${e.message}`);return;
      }
      /* v2.17.1: busy means the server is running a pass itself (auto-continue) —
         that IS the sweep. Watch its progress from sync-state instead of treating
         the lock as contention; the tab can close at any point and it keeps going. */
      if(data&&data.busy){
        let st=null;
        try{st=await(await fetch("/api/sync-status")).json();if(st&&!st.error)setSyncStatus(st);}catch(e){}
        if(st&&typeof st.remaining==="number")remaining=st.remaining;
        if(st&&typeof st.epochImported==="number")imported=st.epochImported;
        if(st&&typeof st.epochQueued==="number")queued=st.epochQueued;
        setBacklog({running:true,pass,imported,queued,scanned,remaining});
        if(st&&st.done)break;
        if(++busyWaits>240){setBacklog({running:false,pass,imported,queued,scanned,remaining,note:"Still draining on the server — safe to close this tab and check back."});return;}
        await new Promise(r=>setTimeout(r,3000));continue;
      }
      if(data&&(data.paused||data.error)){
        const msg=data.error||"paused";
        setBacklog({running:false,pass,imported,queued,scanned,remaining,note:msg});
        toast(`Backlog sweep stopped: ${msg}`);
        await refreshSyncStatus();return;
      }
      pass++;busyWaits=0;
      /* v2.17.1: prefer the server's epoch running totals — chain links import
         between our passes, and their work must show up in the counter too. */
      if(typeof data.epochImported==="number")imported=data.epochImported;else imported+=data.imported||0;
      if(typeof data.epochQueued==="number")queued=data.epochQueued;else queued+=data.queued||0;
      scanned+=data.processed||0;stuck=data.stuck||0;
      if(typeof data.remaining==="number")remaining=data.remaining;
      setBacklog({running:true,pass,imported,queued,scanned,remaining});
      await refreshSyncStatus();
      if(data.done)break; // mailbox fully crawled AND queue drained — genuinely caught up
      /* v2.17.0: progress is anything processed OR newly discovered (a crawl-only pass
         is still progress). Two consecutive passes with neither means the server can't
         move at all — stop and say so rather than spinning to MAX_PASSES. (Adapted from
         the v2.16.18 stall guard, which keyed off the old timedOut flag.) */
      if(!(data.processed||data.discovered)){
        if(++stalled>=2){
          setBacklog({running:false,pass,imported,queued,scanned,remaining,note:`Stalled — no progress two passes in a row${remaining?` (${remaining} still queued)`:""}. Try again later.`});
          await loadCostsFromShards(false);
          toast(`Backlog sweep stalled after ${pass} passes — imported ${imported}, queued ${queued}.${remaining?` ${remaining} attachment(s) still pending.`:""}`);
          return;
        }
      }else stalled=0;
    }
    ...
    /* v2.16.19: attachments the server gave up on after repeated failures aren't "caught
       up" either — name them here rather than letting a clean ✓ hide the one thing that
       still needs a human. */
    const stuckNote=stuck?` · ${stuck} attachment(s) skipped after repeated failures`:"";
    setBacklog({running:false,pass,imported,queued,scanned,remaining:hitCap?remaining:0,note:(hitCap?`Stopped watching after ${MAX_PASSES} passes — the server keeps draining on its own.`:"Caught up ✓")+stuckNote});
    toast((hitCap
      ?`Stopped watching after ${MAX_PASSES} passes — imported ${imported} so far; the server keeps going on its own.`
      :`Backlog caught up — imported ${imported}, queued ${queued} for review.`)
      +(stuck?` ${stuck} attachment(s) couldn't be read — see the sync status for details.`:""));
  };
  ...
  // Push vendor list + truck IDs to Blobs whenever they change, so the scheduled
  // sync has fresh data even when no browser is open.
  const syncConfigToServer=useCallback(async(vendorsArg,trucksArg)=>{
    try{
      await fetch("/api/save-sync-config",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({
          vendors:vendorsArg.map(v=>({name:v.name,category:v.category})),
          truckIds:trucksArg.map(t=>t.id),
        }),
      });
    }catch(e){/* silent — non-critical */}
  },[]);

  // Initial status fetch + push current config to server
  useEffect(()=>{
    if(!loaded)return;
    refreshSyncStatus();
  },[loaded,refreshSyncStatus]);

  // Push config to server whenever vendors or trucks change (debounced via deps)
  useEffect(()=>{
    if(!loaded||knownVendors.length===0||trucks.length===0)return;
  ...
  // Save token to Blobs IF we have one locally but the server doesn't (handles
  // upgrade case where token was saved client-side before Blobs storage existed).
  useEffect(()=>{
    if(!loaded||!gmailConn?.refresh_token||!syncStatus)return;
    if(syncStatus.tokenStored)return; // already on server
    fetch("/api/save-gmail-token",{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({refresh_token:gmailConn.refresh_token,email:gmailConn.email,connected_at:gmailConn.connected_at}),
    }).then(()=>refreshSyncStatus()).catch(()=>{});
  ...
  // and invoiceNum checks are effectively uniform across one document's split rows,
  // since splitMultiTruckEntry copies them from the shared parse — not independently
  // re-derivable here, and left alone: if a genuinely new problem exists on those, the
  // item stays in the queue for a human instead of being guessed at.)
  // v2.26.0: the date IS re-derived, because it is a regex on the row and skipping it
  // was not neutral. evaluateConfidence returns on the FIRST failing (row, check) pair
  // and checks date before roster, so an item queued as "not in fleet roster" can still
  // hold a LATER row with an unreadable date that check never reached. Clearing the
  // roster problem then imported that row straight into the "unknown" cost shard, which
  // no month view shows — the invoice looked filed and was gone. Normalized first, so
  // only a date nothing can read keeps the item queued.
  ...
  const reviewRowBlocked=(row,fleetIds)=>{
    ...
    // total is per-row (each truck's slice of a split invoice), unlike vendor and
    // invoiceNum which evaluateConfidence copies uniformly onto every split row — so a
    // bad total on some OTHER row is a real, independently-possible reason to stay
    // queued, not something the stored (row,check)-shortcircuited reason would ever
    // have told us. The date is judged AFTER toYMD, so only one nothing can read blocks.
    if(!(parseMoney(row&&row.total)>0))return true;
    if(!/^\d{4}-\d{2}-\d{2}$/.test(toYMD(row&&row.date)))return true;
  ...
  const describeRowBlock=(row,fleetIds)=>{
    ...
    if(!(parseMoney(row&&row.total)>0))return"Missing or zero total";
    if(!/^\d{4}-\d{2}-\d{2}$/.test(toYMD(row&&row.date)))return"Bad date format";
  ...
  const resolveQueueFor=(id,fleetIds,remapRow)=>{
    ...
    const approved=clearable.flatMap(item=>splitMultiTruck(coalesceRepairInvoice(item.parsed.map(e=>normalizeApprovedRow(e,fleetIds)))));
    // v2.27.0: if the ledger has not loaded, saveCosts refuses — and clearing the queue
    // anyway would drop these invoices on the floor: gone from the queue, never in the
    // ledger. Leave them queued and say so instead.
    if(approved.length&&!saveCosts([...costEntries,...approved]))return{imported:0,stuck:touching.length};
  ...
  const quickAddTruckAndClearQueue=async(missingId)=>{
    ...
    if(!await uiConfirm(`Add truck #${missingId} to the fleet?${extra}\n\nYou can set its make, model and axle configuration from the Fleet List tab any time.`))return;
  ...
  const approveReviewItem=async(itemId)=>{
    ...
    const fleetIds=new Set([...trucks.map(t=>t.id),...retiredTrucks.map(t=>t.id)]);
    const approved=coalesceRepairInvoice((item.parsed||[]).map(e=>normalizeApprovedRow(e,fleetIds)));
    const newCosts=[...costEntries,...splitMultiTruck(approved)];
    if(!saveCosts(newCosts))return;   // see resolveQueueFor — never unqueue what was not filed
    saveReviewQueue(reviewQueue.filter(q=>q.id!==itemId));
  };
  // v2.17.0: a human saying no to a server-found attachment — rejecting a review item
  // or deleting an imported invoice — leaves a tombstone in fl-rejected-refs. The
  // server rebuilds its dedup index from the ledger at each discovery epoch; without
  ...
  const processGmailAttachment=async(emailResult,att)=>{
    ...
    if(costEntries.some(c=>c.gmailRef===gmailTag)){
      toast("This attachment has already been saved.");
      return;
    }
    // Check for duplicate — already in current scan queue?
    if(scanQueue.some(q=>q.gmailRef===gmailTag)){
      toast("This attachment is already in your scan queue.");
      return;
    }
    ...
    try{
      ...
      if(isPdf){
        ...
        let pdfText="";let pageCount=1;let extractionInfo="";
        try{
          ...
          pdfText=pages.join("\n\n");
          // v2.10.2: log extraction stats so we can diagnose image-based vs text-based PDFs
          console.log(`[pdf-extract] "${att.filename}" ${pageCount}pg, ${totalTextItems} text items, ${pdfText.length} chars extracted`);
          if(pdfText.trim().length<=50){
            extractionInfo=`image-based (${totalTextItems} text items, ${pdfText.length} chars)`;
            console.log(`[pdf-extract] "${att.filename}" appears to be image-based/scanned — will send as document to Claude`);
          }
        }catch(e){
          extractionInfo=`extraction threw: ${e.message}`;
          console.log(`[pdf-extract] "${att.filename}" text extraction failed:`,e.message);
        }
      ...
      toast("Failed to process attachment: "+(err.message||"Unknown error"));
    }
  ...
  const scanInvoices=async()=>{
    ...
    // v2.10.15: extracted from inline for-loop so we can run items in parallel below.
    // Mutates updated[i] by closure. Safe to run concurrently because each invocation
    // only touches its own index; children pushed to `updated` are parsed already
    // (don't conflict with the concurrent readyIndices batching).
    const scanOneItem=async(i)=>{
      if(updated[i].status!=="ready")return;
      ...
      try{
        const isPdfDoc=updated[i].type==="pdf-doc";
        // v2.10.7 — FINAL STRATEGY: render PDFs to JPEGs client-side via pdf.js,
        // send to Claude as image blocks. This bypasses Anthropic's PDF parser
        // entirely (which was intermittently rejecting valid FuelFox PDFs with
        // "The string did not match the expected pattern" — v2.9.2 through v2.10.6
        // all failed to fix this reliably).
        //
        // pdf.js is the same engine the View modal uses — if you can click View
        // and see the PDF, pdf.js can render it to images. No PDF-parser variance.
        const MAX_B64_CHARS=5_300_000;
        const TOTAL_PAYLOAD_CAP=5_300_000;
        const hasPdfText=isPdfDoc&&updated[i].pdfText&&updated[i].pdfText.trim().length>50;
        let contentBlock=null;
        let base64Data=null;
        let mediaType=null;
        let pdfPageImages=null; // array of image blocks once we render the PDF
        let renderFailureReason=null;

        if(isPdfDoc){
          let pdfB64=(updated[i].dataUrl||"").split(",")[1];
          if(!pdfB64){
            throw new Error(`PDF data missing for "${updated[i].file}" — delete this entry and re-upload.`);
          }
          // v2.10.9: TEXT-FIRST ROUTING. If pdf.js extracted a clean text layer
          // during ingestion (FuelFox, Quick Fuel, Peach State — most vendors have
          // text layers), skip canvas/JPEG rendering entirely and send the text
          // directly to Claude. This:
          //   • avoids iOS Safari canvas quirks (the 04-09-002 failure)
          //   • avoids Anthropic's strict image-base64 validator
          //   • uses ~90% fewer tokens than rendered JPEGs
          //   • is ~3x faster per scan
          // Fallback to JPEG rendering only when no text layer exists (scanned PDFs).
          if(hasPdfText){
            console.log(`[scan] v2.10.9: "${updated[i].file}" has ${updated[i].pdfText.length} chars of extracted text — using TEXT mode (skipping JPEG render)`);
            // pdfPageImages stays null, contentBlock stays null,
            // usingTextFallback becomes true below, promptIntro injects pdfText.
          }else{
          ...
          if(!window.pdfjsLib){
            try{
              ...
              renderFailureReason=`pdf.js failed to load: ${loadErr.message}`;
            }
          }
          // v2.10.8: safe atob helper — diagnoses WHY it failed and auto-refetches
          // from Gmail if the source is Gmail-sourced. Captures full diagnostics
          // in the UI error for visibility on mobile (no devtools needed).
          const safeAtobForPdf=async()=>{
            // Normalize: strip anything not base64, fix padding
            const originalLen=pdfB64.length;
            let clean=pdfB64.replace(/-/g,"+").replace(/_/g,"/").replace(/[^A-Za-z0-9+/]/g,"");
            const strippedCount=pdfB64.length-pdfB64.replace(/[^A-Za-z0-9+/\-_]/g,"").length;
            const padLen=(4-(clean.length%4))%4;
            clean=clean+"=".repeat(padLen);
            try{
              return atob(clean);
            }catch(atobErr){
              // Diagnostic: what exactly is in this string?
              const badCharMatches=pdfB64.match(/[^A-Za-z0-9+/\-_=\s]/g);
              const badCharSummary=badCharMatches
                ? `${badCharMatches.length} illegal char(s): ${[...new Set(badCharMatches)].slice(0,5).map(c=>`"${c}"(charCode ${c.charCodeAt(0)})`).join(",")}`
                : "no illegal chars found (but atob still failed)";
              const diag=`atob failed: "${atobErr.message||atobErr}". Original length ${originalLen}, cleaned length ${clean.length}, mod 4 = ${clean.length%4}, ${strippedCount} chars stripped by cleanup, ${badCharSummary}. First 40 chars: "${pdfB64.substring(0,40)}". Last 40 chars: "${pdfB64.substring(pdfB64.length-40)}".`;
              console.log(`[scan] v2.10.8 atob diagnostic for "${updated[i].file}":`,diag);

              // Auto-refetch from Gmail if the item came from Gmail
              if(updated[i].gmailRef&&typeof updated[i].gmailRef==="string"&&updated[i].gmailRef.startsWith("gmail:")){
                const parts=updated[i].gmailRef.split(":");
                const messageId=parts[1];
                const attachmentId=parts[2];
                if(messageId&&attachmentId&&gmailConn?.refresh_token){
                  console.log(`[scan] v2.10.8: atob failed, refetching from Gmail: msg=${messageId} att=${attachmentId}`);
                  try{
                    const resp=await fetch("/api/gmail-attachment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({refresh_token:gmailConn.refresh_token,messageId,attachmentId})});
                    if(resp.ok){
                      const rd=await resp.json();
                      if(rd.data){
                        let freshClean=rd.data.replace(/-/g,"+").replace(/_/g,"/").replace(/[^A-Za-z0-9+/]/g,"");
                        freshClean=freshClean+"=".repeat((4-(freshClean.length%4))%4);
                        try{
                          const binary=atob(freshClean);
                          console.log(`[scan] v2.10.8: Gmail refetch succeeded (${freshClean.length} chars), proceeding with fresh bytes`);
                          // Update the queue item's dataUrl so subsequent passes work too
                          pdfB64=freshClean;
                          updated[i].dataUrl=`data:application/pdf;base64,${freshClean}`;
                          updated[i].origDataUrl=updated[i].dataUrl;
                          return binary;
                        }catch(fresh2){
                          throw new Error(`${diag} Gmail refetch also failed atob: ${fresh2.message}. The attachment may be corrupt on Gmail's side — download manually from Gmail and use Choose Files.`);
                        }
                      }
                    }
                    throw new Error(`${diag} Gmail refetch returned HTTP ${resp.status}.`);
                  }catch(refetchErr){
                    throw new Error(`${diag} Refetch from Gmail failed: ${refetchErr.message}`);
                  }
                }
              }
              // Not Gmail-sourced or no gmailConn — surface the diagnostic as the error
              throw new Error(diag);
            }
          };
          // Render each page to a JPEG image block
          if(!renderFailureReason){
            try{
              const binary=await safeAtobForPdf();
              const bytes=new Uint8Array(binary.length);
              for(let j=0;j<binary.length;j++)bytes[j]=binary.charCodeAt(j);
              const pdf=await window.pdfjsLib.getDocument({data:bytes}).promise;
              pdfPageImages=[];
              let totalSize=0;
              // First pass: render at 1.5x (readable for small numbers/prices)
              for(let p=1;p<=pdf.numPages;p++){
                const page=await pdf.getPage(p);
                ...
                let jpegB64=canvas.toDataURL("image/jpeg",0.85).split(",")[1];
                if(jpegB64.length>MAX_B64_CHARS){
                  // Single page too big at 0.85 — retry at lower quality
                  jpegB64=canvas.toDataURL("image/jpeg",0.6).split(",")[1];
                }
                totalSize+=jpegB64.length;
                console.log(`[scan] v2.10.7: rendered page ${p}/${pdf.numPages} of "${updated[i].file}" → ${Math.round(jpegB64.length/1024)}KB JPEG`);
                pdfPageImages.push({type:"image",source:{type:"base64",media_type:"image/jpeg",data:jpegB64}});
              }
              // Second pass: if total payload exceeds cap, re-render everything at 1.0x + 0.6 quality
              if(totalSize>TOTAL_PAYLOAD_CAP){
                console.log(`[scan] v2.10.7: total ${Math.round(totalSize/1024)}KB exceeds cap, downscaling all pages`);
                pdfPageImages=[];
                totalSize=0;
                for(let p=1;p<=pdf.numPages;p++){
                  const page=await pdf.getPage(p);
                ...
                console.log(`[scan] v2.10.7: after downscale → total ${Math.round(totalSize/1024)}KB`);
              }
              console.log(`[scan] v2.10.7: "${updated[i].file}" ready as ${pdfPageImages.length} JPEG image(s)`);
            }catch(renderErr){
              renderFailureReason=`pdf.js render threw: ${renderErr.message}`;
              console.log(`[scan] v2.10.7: PDF render failed for "${updated[i].file}":`,renderErr.message);
              pdfPageImages=null;
            }
          }
          // Fallback: if render failed but we have extracted text, use text mode
          if(renderFailureReason&&hasPdfText){
            console.log(`[scan] v2.10.7: render failed, falling back to TEXT mode (${updated[i].pdfText.length} chars extracted)`);
            // contentBlock stays null — promptIntro will inject the text
          }else if(renderFailureReason){
            throw new Error(`Could not render "${updated[i].file}" to images, and no text layer available. ${renderFailureReason}. Try deleting and re-uploading.`);
          }
          } // end v2.10.9 else (hasPdfText false → JPEG render path)
        }else{
          // Image path (photos, direct image uploads) — unchanged
        ...
        // Build prompt. When we fell back to text mode (PDF render failed but text available), inject text.
        // Otherwise (PDF rendered to images OR direct image upload), use generic prompt.
        const usingTextFallback=isPdfDoc&&!pdfPageImages&&hasPdfText;
        const isPdfText=usingTextFallback; // legacy alias
        const promptIntro=usingTextFallback
...
This document may contain MULTIPLE invoices (e.g., Quick Fuel lists many trucks, each treated as a separate invoice). Extract EVERY invoice/truck-row found and return a JSON ARRAY of objects (no markdown, no backticks, no explanation). Even if there is only one invoice, return it inside an array.`
        ...
        const msgBody={messages:[{role:"user",content:[
              ...(pdfPageImages?pdfPageImages:(contentBlock?[contentBlock]:[])),
...
Always match to the closest fleet number. Use the TOTAL line (including tax) for the total amount. For the date, use DATE INVOICE (not DATE SHIPPED). Return ONLY the JSON array.`}
        ...
        // v2.10.11: fetch with 90s timeout. Long-running Claude scans (29+ trucks)
        // can exceed Netlify's 26s sync function timeout. Instead of hanging forever,
        // fail fast at 90s with a clear message. Also update UI with elapsed time
        // so the user can see something is happening.
        //
        // v2.10.14: handle non-JSON 5xx responses (e.g. Netlify edge's 504 with an
        // HTML "Inactivity Timeout" body). Without this, r.json() throws a cryptic
        // "Unexpected token '<'" error and the real cause (gateway timeout) is lost.
        const fetchWithTimeout=async(body)=>{
          const ctrl=new AbortController();
          const startTime=Date.now();
          const timer=setTimeout(()=>ctrl.abort(),90000); // 90 second hard cap
          // Live elapsed-time ticker so "scanning" doesn't look frozen
          const tickInterval=setInterval(()=>{
            const elapsed=Math.round((Date.now()-startTime)/1000);
            updated[i].parsed={_waitMsg:`Still scanning... ${elapsed}s elapsed`};
            setScanQueue([...updated]);
          },3000);
          try{
            const r=await fetch("/api/scan-invoice",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body),signal:ctrl.signal});
            const contentType=r.headers.get("content-type")||"";
            // v2.10.14: if the response isn't JSON, read as text and surface clearly
            if(!contentType.toLowerCase().includes("json")){
              const rawText=await r.text();
              // Detect Netlify/Cloudflare/Akamai upstream timeout HTML pages
              if(/inactivity.?timeout|gateway.?timeout|504|too much time has passed/i.test(rawText)){
                throw new Error(`Gateway timed out (HTTP ${r.status}). The Netlify function exceeded its sync timeout — Claude took too long to respond. Payload size: ${Math.round(JSON.stringify(body).length/1024)}KB. This usually means the prompt is too big OR the model is overloaded. Current fix: we're on Haiku 4.5 now so this should be rare; if it keeps happening, split the file.`);
              }
              throw new Error(`Server returned non-JSON response (HTTP ${r.status}, content-type: ${contentType||"none"}). First 200 chars: ${rawText.substring(0,200)}`);
            }
            // Parse JSON and also check HTTP status (some Netlify functions return JSON with error field on non-200)
            ...
            if(e.name==="AbortError"){
              throw new Error(`Scan timed out after 90s. The Claude API request took too long (likely ${msgBody.messages[0].content.find(c=>c.type==="text")?.text.length||0} chars of prompt generating too many JSON rows). Try: (1) split into smaller PDFs, (2) use a different vendor category.`);
            }
            // v2.10.14: JSON parse errors get a friendlier message
            if(e.name==="SyntaxError"&&/json|token/i.test(e.message||"")){
              throw new Error(`Server returned malformed response (could not parse as JSON). The Netlify function may have crashed or returned an HTML error page. Check Netlify function logs.`);
            }
            throw e;
          }finally{
            clearTimeout(timer);
        ...
        let resp=null;
        let data=await fetchWithTimeout(msgBody);
        ...
        while(data.error&&/rate.?limit|429/i.test(String(data.error)+String(data.details||""))&&rateRetryAttempt<3){
          ...
          updated[i].parsed=null;
          setScanQueue([...updated]);
          data=await fetchWithTimeout(msgBody);
        }
        ...
        if(data.error&&/string did not match/i.test(data.error+(data.details||""))&&mediaType&&mediaType.startsWith("image/")){
          try{
            ...
            if(pngB64&&pngB64.length<=MAX_B64_CHARS){
              ...
              data=await fetchWithTimeout(retryBody);
            }
          }catch(retryErr){/* fall through to error handler below */}
        }

        // v2.10.7: No document-mode fallback needed — we now render all PDFs to
        // JPEG images client-side BEFORE the first Anthropic call. The old v2.10.6
        // fallback (which ran after document mode failed) is no longer reachable.

        if(data.error){
          // v2.10.9: surface full context when Anthropic rejects a payload
          const fullErr=data.error+(data.details?": "+data.details.substring(0,400):"");
          const mode=usingTextFallback?"TEXT":pdfPageImages?`JPEG×${pdfPageImages.length}`:contentBlock?"IMAGE":"EMPTY";
          const imgKB=pdfPageImages
            ? Math.round(pdfPageImages.reduce((s,b)=>s+(b.source?.data?.length||0),0)*0.75/1024)
            : Math.round((base64Data?.length||0)*0.75/1024);
          if(/string did not match|could not (process|decode)|invalid.*base64|image too large/i.test(fullErr)){
            throw new Error(`Claude API rejected payload. Mode=${mode}, size=${imgKB}KB. Detail: ${fullErr.substring(0,250)}`);
          }
          throw new Error(`Claude API error (mode=${mode}, size=${imgKB}KB): ${fullErr.substring(0,400)}`);
        }
        const text=data.content?.find(c=>c.type==="text")?.text||"";
        ...
        updated[i].status="error";
        // v2.10.10: tag every error with the version + mode so we can tell from
        // the UI alone whether the new code ran, and which path failed.
        const modeTag=hasPdfText?"TEXT":pdfPageImages?`JPEG×${pdfPageImages.length}`:contentBlock?"IMG":"?";
        const rawMsg=err.message||"Failed to parse";
        updated[i].parsed={error:`[v2.10.10 ${modeTag}] ${rawMsg}`};
        console.log(`[scan] v2.10.10 error in mode ${modeTag} for "${updated[i].file}":`,rawMsg,err);
      }
      ...
      setScanQueue([...updated]);
      // v2.10.15: no per-item throttle — rate-limit pacing is handled by the batch
      // concurrency cap below, and by the existing inner rate-limit-retry loop.
    };

    // v2.10.15: batched-parallel processing for bulk imports. CONCURRENCY=5 was the
    // sweet spot — below Anthropic's 429 cliff for most tiers, well above sequential.
    // For a 300-email backfill this drops wall-clock from ~55min to ~10-15min (bounded
    // by Anthropic's tokens/min rate limit, not by our code).
    // v2.10.17: lowered from 5 to 3 — Chad observed rate-limit retries firing on
    // every batch at 5 concurrent, meaning we were spending more time in exponential
    // backoff than parallel work was saving. 3 concurrent stays under Anthropic's
    // tokens/min cap cleanly for his tier. Raise this if Anthropic tier increases.
    const CONCURRENCY=3;
    const readyIndices=[];
    for(let k=0;k<updated.length;k++){
      if(updated[k].status==="ready")readyIndices.push(k);
    }
    for(let b=0;b<readyIndices.length;b+=CONCURRENCY){
      const batch=readyIndices.slice(b,b+CONCURRENCY);
      await Promise.all(batch.map(i=>scanOneItem(i).catch(e=>{
        // Individual item errors are already surfaced via updated[i].parsed in scanOneItem.
        // This catch is a safety net so one item's rejection doesn't abort the batch.
        console.log(`[scan] v2.10.15: scanOneItem(${i}) rejected:`,e);
      })));
    }
    setScanning(false);
  };

  // v2.10.3: when scanAllNewInvoices finishes queueing, auto-trigger scanInvoices
  // once React has flushed the new queue items. Watching scanQueue.length ensures
  // the closure that fires has the latest scanQueue with queued items visible.
  useEffect(()=>{
    if(!autoScanPending)return;
    if(scanning)return;
    const readyCount=scanQueue.filter(q=>q.status==="ready").length;
    if(readyCount===0)return;
    setAutoScanPending(false);
    scanInvoices();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  },[autoScanPending,scanQueue.length,scanning]);
  ...
  const confirmScannedInvoices=async()=>{
    ...
    // v2.26.0: parseMoney, not `c.total||0`. A parser that returns the amount as the
    // invoice prints it ("$5,755.63") made this `Math.round(NaN)` on the candidate side
    // and a real number on the stored side, so the two never matched and the same
    // invoice imported again on every scan — which is half of how one truck reached
    // six figures it never spent.
    const fp=(c)=>`${(c.vendor||"").toLowerCase()}|${c.invoiceNum||""}|${Math.round(parseMoney(c.total)*100)}`;
    ...
    if(items.length===0){toast("No parsed invoices to save.");return;}
    ...
    for(let idx=0;idx<items.length;idx++){
      ...
      const entryTotal=parseMoney(q.parsed.total);
      const entryDate=toYMD(q.parsed.date)||q.parsed.date||todayYMD();
      ...
      newEntries.push({
        ...
        pricePerGallon:q.parsed.pricePerGallon!=null?Number(q.parsed.pricePerGallon):(q.parsed.gallons&&entryTotal?Number((entryTotal/Number(q.parsed.gallons)).toFixed(4)):null),
      ...
      if(entryCategory==="Core"){
        ...
        if(entryTotal>0){
          ...
          newCoreRecords.push({
            ...
            notes:q.parsed.notes||"",
            // v2.10.41: preserve source link refs so invoice numbers can be made clickable
            gmailRef:q.gmailRef||null,
          ...
          if(match){
            ...
            newCoreRecords.push({
              ...
              // v2.10.41: preserve source link refs so credit invoice numbers can be made clickable
              gmailRef:q.gmailRef||null,
              fileUrl:fileUrl||null,
              addedAt:new Date().toISOString(),
            });
    ...
    if(newCoreRecords.length>0||coreReconciliations.length>0){
      ...
      if(coreReconciliations.length>0){
        ...
        nextCores=nextCores.map(c=>{
          ...
          return {...c,status:"returned",returnedDate:negEntry?.date||todayYMD(),returnedCreditInvoice:negEntry?.invoiceNum||null};
        });
      }
    ...
    if(msgs.length>0)toast(msgs.join("\n"));
    setScanQueue([]);
  };
  ...
  const deleteCost=async(id)=>{
    if(!await uiConfirm("Delete this cost entry?"))return;
    tombstoneGmailRef(costEntries.find(c=>c.id===id));
  ...
  const allocateInventory=(entryId,truckId)=>{
    const next=costEntries.map(c=>c.id===entryId?{...c,allocatedToTruck:truckId,allocatedDate:todayYMD()}:c);
    saveCosts(next);
    setAllocModal(null);
  };
  ...
  const quickFuelAnalytics=useMemo(()=>{
    ...
    qfEntries.forEach(c=>{
      ...
      const wkKey=wkOf(c.date)||"unknown";
  ...
  // v2.10.1: fuel vendor comparison (Quick Fuel vs FuelFox)
  // Both vendors normalized to the same per-truck, per-gallon shape.
  // Quick Fuel: price-per-gallon is already TRUE cost (tax baked in)
  // FuelFox: price-per-gallon is TRUE cost ONLY AFTER Redistribute Overhead has been run.
  //          Entries with fuelfoxOverheadApplied = a DD### invoice number have true cost.
  //          Entries WITHOUT that flag are still at base price ($2.72) and should be excluded
  //          from the comparison so we don't understate FuelFox's true cost.
  const fuelComparison=useMemo(()=>{
    const parseGallons=(c)=>{
      if(c.gallons)return Number(c.gallons)||0;
      const li=c.lineItems||[];
      for(const item of li){const m=(item.desc||"").match(/(\d+\.?\d*)\s*gal/i);if(m)return Number(m[1])||0;}
      return 0;
    };

    // Bucket entries for each vendor
    const qfEntries=costEntries.filter(c=>{
      const v=(c.vendor||"").toLowerCase();
      return v==="quick fuel"||v==="quickfuel";
    }).map(c=>({
      ...c,
      _gallons:parseGallons(c),
      _cost:Number(c.total)||0,
      _ppg:c.pricePerGallon!=null?Number(c.pricePerGallon):(parseGallons(c)>0?(Number(c.total)||0)/parseGallons(c):0),
      _vendor:"Quick Fuel",
    })).filter(c=>c._gallons>0&&c._cost>0);

    const ffAllEntries=costEntries.filter(c=>{
      const v=(c.vendor||"").toLowerCase();
      if(v!=="fuelfox atlanta"&&v!=="fuelfox")return false;
      // Exclude the INVENTORY overhead rows (they're tax+fee buckets, not per-truck fuel)
      if(c.truckId==="INVENTORY")return false;
      return true;
    });
    // Only include FuelFox entries that have had overhead applied (true cost)
    const ffEntries=ffAllEntries.filter(c=>c.fuelfoxOverheadApplied).map(c=>({
      ...c,
      _gallons:parseGallons(c),
      _cost:Number(c.total)||0,
      _ppg:c.pricePerGallon!=null?Number(c.pricePerGallon):(parseGallons(c)>0?(Number(c.total)||0)/parseGallons(c):0),
      _vendor:"FuelFox Atlanta",
    })).filter(c=>c._gallons>0&&c._cost>0);
    const ffMissingOverhead=ffAllEntries.filter(c=>!c.fuelfoxOverheadApplied).length;

    if(qfEntries.length===0&&ffEntries.length===0)return null;

    // Helpers
    const monthKey=(d)=>d?d.substring(0,7):"unknown"; // YYYY-MM
    const weekKey=(d)=>{try{return wK(new Date(d));}catch{return "unknown";}};

    // Aggregate by period for a given entry list
    const aggregateByKey=(entries,keyFn)=>{
      const buckets={};
      entries.forEach(e=>{
        const k=keyFn(e.date);
        if(!buckets[k])buckets[k]={key:k,entries:[],gallons:0,cost:0};
        buckets[k].entries.push(e);
        buckets[k].gallons+=e._gallons;
        buckets[k].cost+=e._cost;
      });
      return Object.values(buckets).map(b=>({
        ...b,
        gwAvg:b.gallons>0?b.cost/b.gallons:0, // gallons-weighted avg
        simpleAvg:b.entries.length>0?b.entries.reduce((a,e)=>a+e._ppg,0)/b.entries.length:0,
      })).sort((a,b)=>a.key.localeCompare(b.key));
    };

    const qfByMonth=aggregateByKey(qfEntries,monthKey);
    const ffByMonth=aggregateByKey(ffEntries,monthKey);
    const qfByWeek=aggregateByKey(qfEntries,weekKey);
    const ffByWeek=aggregateByKey(ffEntries,weekKey);

    // Sort entries newest-first for the invoice tables
    const qfSorted=[...qfEntries].sort((a,b)=>(b.date||"").localeCompare(a.date||""));
    const ffSorted=[...ffEntries].sort((a,b)=>(b.date||"").localeCompare(a.date||""));

    // Overall rollups
    const roll=(entries)=>{
      const gal=entries.reduce((a,e)=>a+e._gallons,0);
      const cost=entries.reduce((a,e)=>a+e._cost,0);
      const gwAvg=gal>0?cost/gal:0;
      const simpleAvg=entries.length>0?entries.reduce((a,e)=>a+e._ppg,0)/entries.length:0;
      return{gallons:gal,cost,gwAvg,simpleAvg,count:entries.length};
    };

    const qfRoll=roll(qfEntries);
    const ffRoll=roll(ffEntries);

    // Monthly comparison — union of months present in either vendor
    const allMonths=new Set([...qfByMonth.map(m=>m.key),...ffByMonth.map(m=>m.key)]);
    const monthCompare=[...allMonths].sort().map(m=>{
      const qf=qfByMonth.find(x=>x.key===m);
      const ff=ffByMonth.find(x=>x.key===m);
      const delta=(qf&&ff)?ff.gwAvg-qf.gwAvg:null;
      return{month:m,qf,ff,delta};
    });

    const allWeeks=new Set([...qfByWeek.map(w=>w.key),...ffByWeek.map(w=>w.key)]);
    const weekCompare=[...allWeeks].sort().reverse().map(w=>{
      const qf=qfByWeek.find(x=>x.key===w);
      const ff=ffByWeek.find(x=>x.key===w);
      const delta=(qf&&ff)?ff.gwAvg-qf.gwAvg:null;
      return{week:w,qf,ff,delta};
    });

    return{
      qfEntries:qfSorted,
      ffEntries:ffSorted,
      qfByMonth,ffByMonth,qfByWeek,ffByWeek,
      qfRoll,ffRoll,
      monthCompare,weekCompare,
      ffMissingOverhead, // count of FuelFox entries that need overhead redistributed
    };
  },[costEntries]);
  ...
  const sameAsYesterday=async(targetDayIdx)=>{
    if(targetDayIdx<0||targetDayIdx>4)return;
    const targetDay=DAYS[targetDayIdx];
    // Normal case: copy the prior weekday in this same week. Monday has no prior
    // weekday, so it rolls back to the PREVIOUS week's Friday (fetched from storage).
    const crossWeek=targetDayIdx===0;
    const sourceDay=crossWeek?"Fri":DAYS[targetDayIdx-1];
    const sourceLabel=crossWeek?"last Friday":sourceDay;
    let sourceAsgn=asgn;
    if(crossWeek){
      const pd=new Date(weekDate);pd.setDate(pd.getDate()-7);const prevWeekKey=wK(pd);
      sourceAsgn=attendWeeks[prevWeekKey];
      if(!sourceAsgn){try{const r=await window.storage.get(`fl-asgn-${prevWeekKey}`);sourceAsgn=r?JSON.parse(r.value):{};}catch(e){sourceAsgn={};}}
    }
    const prevDay=sourceDay; // status-history detection below keys off this (same-week only)
    const next={...asgn};
    let copied=0,kept=0;
    drivers.forEach(d=>{
      const targetKey=`${d.name}-${targetDay}`;
      // v2.16.16: NEVER overwrite a cell already set for the target day. Dispatchers
      // routinely make manual changes in the morning and then hit "Same as Yesterday"
      // to fill in everyone else — this used to clobber those edits. Fill blanks only.
      if((asgn[targetKey]||"").trim()){kept++;return;}
      const prevVal=sourceAsgn[`${d.name}-${sourceDay}`]||"";
      // Don't repeat one-off absences — a call-out/no-show shouldn't auto-carry forward.
      if(prevVal&&prevVal!=="CALLED OUT"&&prevVal!=="NO SHOW"){next[targetKey]=prevVal;copied++;}
    });

    // v2.10.51: Detect trucks that were OOS YESTERDAY (the day being copied from)
    // but whose repair ticket is now closed — i.e. the mechanic completed the repair.
    // We look at prevDay (not targetDay) because closing a repair now clears only the
    // current day's status, leaving yesterday's OOS intact as history. So when the
    // dispatcher copies forward, yesterday still shows OOS, and that's our signal:
    //   - the truck was down yesterday,
    //   - but has no open ticket now → it's been repaired.
    // We do NOT carry that OOS into today (the copy only moves driver assignments,
    // never status), and we clear any stale OOS sitting on today's cell so it shows
    // available. Then we flag these trucks to the dispatcher so they get reassigned.
    const nextStat={...tStat};
    const repairedTrucks=[];
    let clearedToday=false;
    // Status history is per-week, so the repaired-since-yesterday detection only
    // applies to same-week copies (not Monday←last-Friday).
    if(!crossWeek)trucks.forEach(t=>{
      const prevKey=`${t.id}-${prevDay}`;
      const prevCur=(tStat[prevKey]||"").toLowerCase().trim();
      const wasDownYesterday=prevCur==="oos"||prevCur==="bd"||prevCur.includes("repair")||prevCur.includes("interstate")||prevCur.includes("shop")||prevCur.includes("mech");
      if(!wasDownYesterday)return;
      const hasOpenRepair=repairs.some(r=>r.truckId===t.id&&r.status==="open");
      if(hasOpenRepair)return; // still legitimately down — leave it alone
      // Repaired since yesterday. Flag it, and make sure today's cell isn't stale-OOS.
      repairedTrucks.push(t.id);
      const todayKey=`${t.id}-${targetDay}`;
      const todayCur=(nextStat[todayKey]||"").toLowerCase().trim();
      const todayDown=todayCur==="oos"||todayCur==="bd"||todayCur.includes("repair")||todayCur.includes("interstate")||todayCur.includes("shop")||todayCur.includes("mech");
      if(todayDown){nextStat[todayKey]="HERE";clearedToday=true;}
    });
    ...
    if(copied>0)saveAsgn(next);
    if(clearedToday)saveTStat(nextStat);

    // Build the dispatcher message. v2.16.16: always report how many cells were left
    // alone, so it's obvious the button filled blanks and did not touch today's edits.
    const keptNote=kept>0?` Left ${kept} ${kept===1?"driver":"drivers"} unchanged — already set for ${targetDay}.`:"";
    let msg="";
    if(copied>0)msg+=`Copied ${copied} assignment${copied===1?"":"s"} from ${sourceLabel} → ${targetDay}.${keptNote}`;
    else if(kept>0)msg+=`Nothing new to copy — ${kept} ${kept===1?"driver is":"drivers are"} already set for ${targetDay}, left as-is.`;
    else msg+=`No assignments found for ${sourceLabel} to copy.`;
    if(oosAssignments.length>0){
      const n=oosAssignments.length;
      msg+=`\n\n🔧 ${n} assignment${n===1?"":"s"} landed on a truck that's OUT OF SERVICE (in the shop):\n\n  ${oosAssignments.map(a=>`#${a.truck} — ${a.driver}`).join("\n  ")}\n\nReassign ${n===1?"this driver":"these drivers"} to an available truck.`;
    }
    if(repairedTrucks.length>0){
      const n=repairedTrucks.length;
      msg+=`\n\n⚠ ${n} truck${n===1?"":"s"} ${n===1?"was":"were"} OOS ${prevDay} but ${n===1?"its":"their"} repair ${n===1?"ticket has":"tickets have"} since been completed — ${n===1?"it's":"they're"} back in service:\n\n  #${repairedTrucks.join(", #")}\n\n${prevDay}'s OOS history is kept, but ${n===1?"this truck was":"these trucks were"} NOT carried over as OOS today. Assign driver${n===1?"":"s"} as needed.`;
    }
    toast(msg);
  };
  ...
  // Trucks assigned to more than one driver on a given day → double-booked (always an error).
  const dupByDay=useMemo(()=>{const m={};DAYS.forEach(day=>{const c={};drivers.forEach(d=>{const v=asgn[`${d.name}-${day}`];if(v&&!OFF_OPTS.includes(v))c[v]=(c[v]||0)+1;});m[day]=new Set(Object.keys(c).filter(k=>c[k]>1));});return m;},[asgn,drivers]);
  // Trucks with an open repair ticket → out of service (shouldn't have a driver).
  ...
  const gTS=(id,day)=>{
    ...
    // 4. Driver assigned to this truck today (the authoritative source)
    const hasDriver=drivers.some(d=>{const v=asgn[`${d.name}-${day}`];return v===id;});
    if(hasDriver)return"assigned";
    // 5. v2.10.49: status board has leftover text (e.g. driver initials) but NO driver
    //    is actually assigned to this truck today in the asgn grid. Previously this
    //    returned "assigned", which made trucks show an Assigned badge with a blank
    //    driver column (the column reads asgn, the badge read tStat). Treat it as
    //    "needs assignment" instead — the truck shows initials from an earlier point
    //    in the week but today's driver row is empty, so dispatch should confirm
    //    who's actually in it. Verified against live data: only affects trucks with
    //    genuine mismatches (e.g. #8257 "MC", #6957 "DS"), not trucks with real
    //    same-day assignments (those are caught by step 4 above).
    if(st&&st!==""){
      return"unassigned";
    }
  ...
  // v2.16.3: Is a truck out of service on a specific calendar day because of an
  // OPEN repair? Used by the Fleet tab's per-day grids. Two rules matter here:
  //  • Only OPEN repairs count — a closed repair leaves the truck back in service,
  //    so stale OOS status-board marks from a finished repair no longer paint OOS.
  //  • OOS starts the day the repair went in (dateIn) and carries forward — it does
  //    NOT reach back over days the truck was in service before the breakdown.
  // (Compares date-only, local tz, so a repair opened Wed leaves Mon/Tue alone.)
  const repairOOSOn=(id,colDate)=>{
    const ymd=x=>`${x.getFullYear()}-${String(x.getMonth()+1).padStart(2,"0")}-${String(x.getDate()).padStart(2,"0")}`;
    const col=ymd(colDate);
    return repairs.some(r=>r.truckId===id&&r.status==="open"&&r.dateIn&&ymd(localDate(r.dateIn))<=col);
  };

  // Dashboard stats
  const stats=useMemo(()=>{
    const dk=DAYS[Math.min(todayDI(),4)];
    // Trucks with a driver on the board today.
    ...
    return{total:trucks.length,straight:trucks.filter(t=>t.type==="straight").length,tractor:trucks.filter(t=>t.type==="tractor").length,auto:trucks.filter(t=>t.tr==="A").length,manual:trucks.filter(t=>t.tr==="M").length,autoBox:trucks.filter(t=>t.tr==="A"&&t.type==="straight").length,autoTr:trucks.filter(t=>t.tr==="A"&&t.type==="tractor").length,manBox:trucks.filter(t=>t.tr==="M"&&t.type==="straight").length,manTr:trucks.filter(t=>t.tr==="M"&&t.type==="tractor").length,assigned,oos,repair,avail,drivers:drivers.length,davis:drivers.filter(d=>d.category==="Davis").length,owner:drivers.filter(d=>d.category==="Owner").length,openRepairs:repairs.filter(r=>r.status==="open").length};
  },[trucks,drivers,asgn,tStat,repairs]);

  // v2.16.5: the actual items behind each dashboard tile, so clicking a tile can
  // drill into "which trucks/drivers is this?". Buckets mirror the `stats` memo
  // exactly (down > in-repair > assigned > available) so the lists reconcile to
  // the numbers on the cards.
  const statLists=useMemo(()=>{
    const dk=DAYS[Math.min(todayDI(),4)];
    const driverOf={}; // truckId -> [driver names on the board today]
    drivers.forEach(d=>{const v=asgn[`${d.name}-${dk}`];if(v&&!OFF_OPTS.includes(v)&&v!=="")(driverOf[v]=driverOf[v]||[]).push(d.name);});
    const assignedTrucks=new Set(Object.keys(driverOf));
    ...
    return{
      total:trucks,straight:trucks.filter(t=>t.type==="straight"),tractor:trucks.filter(t=>t.type==="tractor"),
      auto:trucks.filter(t=>t.tr==="A"),manual:trucks.filter(t=>t.tr==="M"),
      assigned:assignedT,oos:oosT,repair:repairT,avail:availT,
      allDrivers:drivers,openRepairs:repairs.filter(r=>r.status==="open"),driverOf,
    };
  },[trucks,drivers,asgn,tStat,repairs]);

  const filteredTrucks=useMemo(()=>trucks.filter(t=>{
    if(search&&!String(t.id||"").includes(search)&&!String(t.mk||"").toLowerCase().includes(search.toLowerCase()))return false;
  ...
  const attendance=useMemo(()=>{
    ...
    // v2.10.21: filter to the most recent N weeks based on attendWeeksBack.
    // Week keys are ISO YYYY-Www (e.g. "2025-W47"), which sort lexicographically.
    let weekEntries=Object.entries(attendWeeks);
    if(attendWeeksBack!=="all"&&Number.isFinite(Number(attendWeeksBack))){
      weekEntries=weekEntries
        .sort((a,b)=>b[0].localeCompare(a[0]))    // most recent first
        .slice(0,Number(attendWeeksBack));
    }
    weekEntries.forEach(([weekKey,weekAsgn])=>{
      DAYS.forEach(day=>{
        drivers.forEach(d=>{
          const v=weekAsgn[`${d.name}-${day}`]||"";
    ...
    return result;
  },[drivers,attendWeeks,attendWeeksBack]);

  const startEdit=(key,cur)=>{setEditCell(key);setEditVal(cur||"");};
  const commitEdit=async(key,type,value)=>{
    const val=value!==undefined?value:editVal;
    if(type==="assign"){
      const next={...asgn,[key]:val};
      // Block double-booking: if this truck is already on another driver today, offer to move it.
      if(val&&!OFF_OPTS.includes(val)){
        const day=key.slice(key.lastIndexOf("-")+1);
        const thisName=key.slice(0,key.lastIndexOf("-"));
        // Warn when assigning a truck that's out of service (open repair ticket).
        if(oosTrucks.has(val)){
          const openR=repairs.find(r=>r.truckId===val&&r.status==="open");
          if(!await uiConfirm(`Truck ${val} is OUT OF SERVICE (open repair${openR&&openR.reason?`: ${openR.reason}`:""}).\n\nAssign it to ${thisName} anyway?`)){setEditCell(null);setEditVal("");return;}
        }
        const others=truckHolders(val,day,thisName);
        if(others.length){
          const ok=await uiConfirm(`Truck ${val} is already assigned to ${others.join(", ")} on ${day}.\n\nOK = move it to ${thisName} (unassign ${others.length>1?"them":others[0]}).\nCancel = don't assign.`);
          if(!ok){setEditCell(null);setEditVal("");return;}
          others.forEach(n=>{next[`${n}-${day}`]="";});
        }
      }
      saveAsgn(next);
    }else saveTStat({...tStat,[key]:val});
  ...
  const addRepair=async(truckId)=>{
    ...
    const openedBy=await ensureDeviceUser();
    const rec={id:Date.now(),truckId,reason:repairForm.reason,notes:repairForm.notes,shop:repairForm.shop,dateIn:new Date().toISOString(),estReturn:repairForm.estReturn||null,cost:repairForm.cost?Number(repairForm.cost):0,dateClosed:null,status:"open",...(openedBy?{openedBy}:{})};
  ...
  // the daily OOS status change is for the SAME reason as yesterday — don't spawn a
  // duplicate ticket every day. Chad flagged this after seeing truck #0920 appear
  ...
  const openRepairFormIfNeeded=async(truckId)=>{
    ...
    // repair — confirm before spawning a new ticket so we don't undo the mechanic's work.
    const recentClose=repairs
      .filter(r=>r.truckId===truckId&&r.status==="closed"&&r.dateClosed)
      .sort((a,b)=>(b.dateClosed||"").localeCompare(a.dateClosed||""))[0];
    if(recentClose){
      const hoursSince=(Date.now()-new Date(recentClose.dateClosed).getTime())/3600000;
      if(hoursSince<36){
  ...
  const closeRepair=async(id,cost)=>{
    ...
    // v2.16.11: record WHO marked it complete (same per-device name that signs notes)
    const next=repairs.map(r=>r.id===id?{...r,status:"closed",dateClosed:new Date().toISOString(),cost:cost||r.cost,...(closedBy?{closedBy}:{})}:r);
    saveRepairs(next);
    // v2.10.51: clear OOS status for TODAY ONLY — not the rest of the week.
    // Per Chad: completing a repair should NOT wipe the OOS history for the
    // earlier days the truck was actually down (that's the historical record we
    // keep, v2.10.39). Clearing only today means yesterday stays OOS, which is
    // exactly what lets "Copy all" detect "this truck was OOS yesterday but the
    // ticket is now closed → don't carry it forward, flag it to dispatch."
    if(repair){
      const ti=Math.min(todayDI(),4);
      const key=`${repair.truckId}-${DAYS[ti]}`;
      const cur=(tStat[key]||"").toLowerCase();
      if(cur==="oos"||cur==="bd"||cur.includes("repair")||cur.includes("interstate")||cur.includes("shop")||cur.includes("mech")){
        saveTStat({...tStat,[key]:"HERE"});
      }
    }
  };
  // v2.10.19: one-time cleanup for duplicate open repairs created by pre-v2.10.18
  // daily-OOS behavior. Groups open repairs by (truckId, reason); for each group
  // with 2+ entries, keeps the one with the EARLIEST dateIn and closes the rest
  // as auto-merged (status="closed", notes tagged with "Auto-merged duplicate").
  // Returns {groups, duplicatesToClose} without mutating state when preview=true.
  const findDuplicateOpenRepairs=()=>{
    const open=repairs.filter(r=>r.status==="open");
    const byKey={};
    open.forEach(r=>{
      const key=`${r.truckId}||${(r.reason||"").toLowerCase().trim()}`;
      if(!byKey[key])byKey[key]=[];
      byKey[key].push(r);
    });
    const dupGroups=Object.entries(byKey).filter(([,arr])=>arr.length>1);
    // For each group: earliest stays open, rest get closed
    const toClose=[];
    dupGroups.forEach(([,arr])=>{
      const sorted=[...arr].sort((a,b)=>(a.dateIn||"").localeCompare(b.dateIn||""));
      // keep sorted[0] open, close sorted[1..]
      for(let i=1;i<sorted.length;i++)toClose.push(sorted[i]);
    });
    return {dupGroups,toClose};
  };
  const mergeDuplicateOpenRepairs=async()=>{
    const {dupGroups,toClose}=findDuplicateOpenRepairs();
    if(toClose.length===0){
      toast("No duplicate open repairs found.");
      return;
    }
    const trucksAffected=[...new Set(dupGroups.map(([k])=>k.split("||")[0]))];
    const msg=`Found ${toClose.length} duplicate open repair ticket${toClose.length>1?"s":""} across ${trucksAffected.length} truck${trucksAffected.length>1?"s":""}:\n\n${trucksAffected.slice(0,10).map(tid=>`  • Truck #${tid}`).join("\n")}${trucksAffected.length>10?`\n  ...and ${trucksAffected.length-10} more`:""}\n\nFor each truck+reason combo, the EARLIEST ticket will stay open and newer duplicates will be closed. This cannot be undone.\n\nProceed?`;
    if(!await uiConfirm(msg))return;
    const closedAt=new Date().toISOString();
    const toCloseIds=new Set(toClose.map(r=>r.id));
    const next=repairs.map(r=>{
      if(!toCloseIds.has(r.id))return r;
      return {...r,status:"closed",dateClosed:closedAt,notes:((r.notes||"")+" [Auto-merged duplicate ticket · v2.10.19]").trim()};
    });
    saveRepairs(next);
    toast(`Closed ${toClose.length} duplicate ticket${toClose.length>1?"s":""}. ${trucksAffected.length} truck${trucksAffected.length>1?"s":""} now have 1 open ticket each.`);
  };
  // v2.10.20: inline-edit a repair ticket (reason, notes, shop, estReturn, cost).
  // Called from the Maintenance tab's per-card editable fields.
  const updateRepair=(id,updates)=>{
    saveRepairs(repairs.map(r=>r.id===id?{...r,...updates}:r));
  };

  // v2.16.4: Print (or save-as-PDF) the current open repairs. Rather than fight
  // the interactive on-screen cards with print CSS, build a clean self-contained
  // document — one block per open ticket with its notes, shop, dates and a
  // sign-off line — open it in a new window and fire the browser print dialog.
  const printOpenRepairs=()=>{
    const open=repairs.filter(r=>r.status==="open");
    const esc=s=>String(s==null?"":s).replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
    const typeOf=r=>{const t=trucks.find(x=>x.id===r.truckId);return t?t.type:"straight";};
    const makeLabel=r=>{const t=trucks.find(x=>x.id===r.truckId);return t?`${truckDesc(t)} · ${t.tr==="A"?"Auto":"Man"}`:"";};
    const cardHTML=r=>{
      const notesLog=r.notesLog||(r.notes?[{ts:r.dateIn||r.id,text:r.notes}]:[]);
      // v2.18.0: the printed sheet is what goes out to the bay, so it has to show
      // which jobs are already finished and what was noted against each — a flat
      // list would send a mechanic to redo a job someone closed this morning.
      const notes=notesLog.map(entry=>{
        const d=new Date(entry.ts);
        const label=isNaN(d.getTime())?"":d.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"2-digit"});
        const box=entry.done?"&#9745;":"&#9744;";
        const doneTail=entry.done?` <span class="dn">done ${esc(entry.doneAt?dateStr(entry.doneAt):"")}${entry.doneBy?` by ${esc(entry.doneBy)}`:""}</span>`:"";
        const subs=(Array.isArray(entry.notes)?entry.notes:[]).map(n=>{
          const nd=new Date(n.ts);
          const nl=isNaN(nd.getTime())?"":nd.toLocaleDateString("en-US",{month:"short",day:"numeric"});
          return `<div class="subnote"><span class="nt">${esc(nl)}</span> ${esc(n.text)}${n.by?` <span class="nt">— ${esc(n.by)}</span>`:""}</div>`;
        }).join("");
        return `<div class="note${entry.done?" done":""}">${box} <span class="nt">${esc(label)}</span> ${esc(entry.text)}${doneTail}</div>${subs}`;
      }).join("");
      const meta=[`In: ${esc(dateStr(r.dateIn))}`];
      if(r.estReturn)meta.push(`Est: ${esc(dateStr(r.estReturn))}`);
      if(r.cost>0)meta.push(`$${Number(r.cost).toLocaleString()}`);
      if(r.shop)meta.push(esc(r.shop));
      return `<div class="card"><div class="chd"><span class="tno">#${esc(r.truckId)}</span> <span class="mk">${esc(makeLabel(r))}</span><span class="rsn">${esc(r.reason||"")}</span></div>${notes?`<div class="notes">${notes}</div>`:""}<div class="meta">${meta.join(" &middot; ")}</div><div class="signoff">Completed &#9744;&nbsp;&nbsp;&nbsp; Mechanic __________________&nbsp;&nbsp;&nbsp; Date __________</div></div>`;
    };
    const section=(label,list)=>`<h2>${label} (${list.length})</h2>${list.length?list.map(cardHTML).join(""):'<div class="empty">None down.</div>'}`;
    const box=open.filter(r=>typeOf(r)==="straight");
    const tractor=open.filter(r=>typeOf(r)==="tractor");
    const now=new Date();
    const html=`<!doctype html><html><head><meta charset="utf-8"><title>Open Repairs — Davis Fleet</title><style>*{box-sizing:border-box}body{font-family:'Segoe UI',Helvetica,Arial,sans-serif;color:#111;margin:24px}.hdr{display:flex;justify-content:space-between;align-items:flex-end;border-bottom:2px solid #1e5b92;padding-bottom:8px;margin-bottom:14px}.hdr h1{font-size:20px;margin:0;color:#1e5b92}.hdr .sub{font-size:12px;color:#555}.hdr .gen{font-size:11px;color:#777;text-align:right}h2{font-size:14px;margin:16px 0 8px;color:#153f66;border-bottom:1px solid #ccc;padding-bottom:4px}.card{border:1px solid #bbb;border-left:4px solid #c0392b;border-radius:4px;padding:8px 10px;margin-bottom:8px;break-inside:avoid;page-break-inside:avoid}.chd{display:flex;align-items:baseline;gap:8px;flex-wrap:wrap}.tno{font-family:monospace;font-weight:700;font-size:15px;color:#1e5b92}.mk{font-size:11px;color:#555}.rsn{margin-left:auto;font-size:11px;font-weight:700;color:#c0392b}.notes{margin:6px 0;padding:6px 8px;background:#f6f6f6;border-radius:4px}.note{font-size:12px;color:#222;margin:2px 0}.note .nt{color:#888;font-size:10px;margin-right:4px}.note.done{color:#777;text-decoration:line-through}.note.done .dn{color:#2e7d32;text-decoration:none;font-size:10px;font-weight:700}.subnote{font-size:11px;color:#444;margin:1px 0 3px 18px;padding-left:6px;border-left:2px solid #ddd}.meta{font-size:11px;color:#555;margin-top:4px}.signoff{font-size:11px;color:#333;margin-top:6px;border-top:1px dashed #ccc;padding-top:5px}.empty{font-size:12px;color:#999;font-style:italic;margin-bottom:8px}@media print{body{margin:0.5in}}</style></head><body><div class="hdr"><div><h1>Open Repairs</h1><div class="sub">Davis Fleet Management</div></div><div class="gen">${esc(now.toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"}))}<br>${open.length} open repair${open.length===1?"":"s"}</div></div>${open.length?section("Box Trucks",box)+section("Tractors",tractor):'<div class="empty">No open repairs. All trucks operational.</div>'}<script>window.onload=function(){window.focus();window.print();};<\/script></body></html>`;
    const w=window.open("","_blank");
    if(!w){toast("Couldn't open the print window — allow pop-ups for this site, then try again.");return;}
  ...
  const addTruck=()=>{if(!newT.id.trim()||trucks.find(t=>t.id===newT.id))return;saveTrucks([...trucks,{...newT,id:newT.id.trim(),md:(newT.md||"").trim()}]);setNewT({id:"",mk:"Freightliner",md:"",tr:"A",ax:"Single",del:false,type:"straight",year:null});setShowAddT(false);};
  // v2.10.28: inline-edit any truck field from the Fleet list view
  const updateTruck=(id,updates)=>saveTrucks(trucks.map(t=>t.id===id?{...t,...updates}:t));

  // ── v2.16.17: edit a truck's number / make / model from its Truck Report ──
  // The truck # is the foreign key for everything: weekly assignments store it as
  // the CELL VALUE, daily status stores it inside the CELL KEY, and repairs, cost
  // entries, cores, the review queue and the Motive mapping all carry it as
  // truckId. Renaming only the fl-trucks row would orphan the truck's entire
  // history the moment the number changed, so a renumber has to rewrite all of it.
  const[truckEdit,setTruckEdit]=useState(null); // {orig,id,mk,md} while the report's edit panel is open
  const[truckEditBusy,setTruckEditBusy]=useState(false);

  // Rewrites every stored week (not just the one on screen) — assignments key off
  // the truck # by value, statuses by key prefix. Returns how many docs changed.
  const renumberTruckInWeeks=async(oldId,newId)=>{
    let weeksTouched=0;
    const rewrite=async(prefix,fn)=>{
      let keys=[];
      try{const r=await window.storage.list(prefix);keys=(r&&r.keys)||[];}catch(e){keys=[];}
      for(const key of keys){
        let m=null;
        try{const d=await window.storage.get(key);m=d?JSON.parse(d.value):null;}catch(e){continue;}
        if(!m||typeof m!=="object")continue;
        const out=fn(m);
        if(!out)continue;
        await window.storage.set(key,JSON.stringify(out));
        weeksTouched++;
        const weekKey=key.slice(prefix.length);
        if(prefix==="fl-asgn-"){
          setAttendWeeks(w=>({...w,[weekKey]:out}));
          if(weekKey===wk)setAsgn(out);
        }else if(weekKey===wk)setTStat(out);
      }
    };
    // Assignments: "<driver>-<day>" → truck #. The truck # is the value.
    await rewrite("fl-asgn-",m=>{
      let hit=false;const out={};
      for(const k of Object.keys(m)){if(m[k]===oldId){out[k]=newId;hit=true;}else out[k]=m[k];}
      return hit?out:null;
    });
    // Daily status: "<truck #>-<day>" → status. The truck # is part of the key.
    await rewrite("fl-stat-",m=>{
      let hit=false;const out={};
      for(const k of Object.keys(m)){
        const day=DAYS.find(d=>k===`${oldId}-${d}`);
        if(day){out[`${newId}-${day}`]=m[k];hit=true;}else out[k]=m[k];
      }
      return hit?out:null;
    });
    return weeksTouched;
  ...
  const commitTruckEdit=async()=>{
    if(!truckEdit||truckEditBusy)return;
    const oldId=truckEdit.orig;
    const newId=String(truckEdit.id||"").trim();
    const mk=truckEdit.mk||"";
    const md=String(truckEdit.md||"").trim();
    const tr=truckEdit.tr==="M"?"M":"A";
    const ax=truckEdit.ax==="Tandem"?"Tandem":"Single";
    const del=!!truckEdit.del;
    if(!newId){toast("Truck # can't be blank.","error");return;}
    const renumber=newId!==oldId;
    if(renumber){
      if(trucks.some(t=>t.id===newId)){toast(`Truck #${newId} already exists in the fleet — pick a different number.`,"error");return;}
      if(retiredTrucks.some(t=>t.id===newId)){toast(`Truck #${newId} is in the retired list. Restore or renumber that one first.`,"error");return;}
      const nRep=repairs.filter(r=>r.truckId===oldId).length;
      const nCost=costEntries.filter(c=>c.truckId===oldId).length;
      const detail=[nRep?`${nRep} repair ticket${nRep===1?"":"s"}`:null,nCost?`${nCost} invoice${nCost===1?"":"s"}`:null].filter(Boolean).join(" and ");
      if(!await uiConfirm(`Renumber truck #${oldId} → #${newId}?\n\nAll history moves with it${detail?`, including ${detail}`:""}, plus every weekly assignment and daily status.\n\nThis cannot be undone automatically.`))return;
    }
    setTruckEditBusy(true);
    try{
      saveTrucks(trucks.map(t=>t.id===oldId?{...t,id:newId,mk,md,tr,ax,del}:t));
      if(renumber){
        if(repairs.some(r=>r.truckId===oldId))saveRepairs(repairs.map(r=>r.truckId===oldId?{...r,truckId:newId}:r));
        if(costEntries.some(c=>c.truckId===oldId))saveCosts(costEntries.map(c=>c.truckId===oldId?{...c,truckId:newId}:c));
        if(cores.some(c=>c.truckId===oldId))saveCores(cores.map(c=>c.truckId===oldId?{...c,truckId:newId}:c));
        // Review-queue rows carry the truck # inside item.parsed[] (each pending line item).
        if(reviewQueue.some(q=>Array.isArray(q&&q.parsed)&&q.parsed.some(e=>e&&e.truckId===oldId)))
          saveReviewQueue(reviewQueue.map(q=>Array.isArray(q&&q.parsed)?{...q,parsed:q.parsed.map(e=>e&&e.truckId===oldId?{...e,truckId:newId}:e)}:q));
        if(motiveMap.vehicles&&motiveMap.vehicles[oldId]!==undefined){
          const v={...motiveMap.vehicles};v[newId]=v[oldId];delete v[oldId];
          saveMotiveMap({...motiveMap,vehicles:v});
        }
        const weeks=await renumberTruckInWeeks(oldId,newId);
        setHistoryTruck(newId);
        toast(`Truck #${oldId} renumbered to #${newId}. Moved its history across ${weeks} week${weeks===1?"":"s"}.`,"success");
      }else{
        toast(`Saved #${newId}${mk?` · ${normalizeMake(mk)}`:""}${md?` ${md}`:""} · ${tr==="A"?"Auto":"Manual"}${del?" · Deleted":""} · ${ax}.`,"success");
      }
      setTruckEdit(null);
    }catch(e){
      toast(`Couldn't save the truck: ${e.message||e}`,"error");
    }finally{setTruckEditBusy(false);}
  };
  // v2.10.43: display-side normalization for legacy make abbreviations.
  // Belt-and-suspenders on top of the 🔤 Migrate button — even if some trucks
  // still have FRTLN/INTL/HINO stored, they render with the full names.
  const normalizeMake=(mk)=>{
    if(!mk)return "";
    return MAKE_ALIASES[mk]||mk;
  };
  const removeTruck=async id=>{
    const hasCosts=costEntries.some(c=>c.truckId===id);
    if(hasCosts){
      const choice=await uiConfirm(`Truck ${id} has historical cost data.\n\nOK = Move to Retired (keeps history)\nCancel = Keep in fleet`);
      if(!choice)return;
      const t=trucks.find(x=>x.id===id);
      saveRetiredTrucks([...retiredTrucks,{...(t||{}),id,retiredDate:todayYMD(),reason:"Manually retired"}]);
      saveTrucks(trucks.filter(t=>t.id!==id));
    }else{
      if(!await uiConfirm(`Remove truck ${id}?`))return;
      saveTrucks(trucks.filter(t=>t.id!==id));
    }
  };
  const removeDriver=async name=>{
    const d=drivers.find(x=>x.name===name);if(!d)return;
    if(!await uiConfirm(`Move ${name} to the Retired Drivers list?\n\nThey'll be removed from the active roster but kept for history, and you can restore them anytime.`))return;
    saveDrivers(drivers.filter(x=>x.name!==name));
    if(!retiredDrivers.some(x=>x.name===name))saveRetiredDrivers([...retiredDrivers,{...d,retiredDate:todayYMD()}]);
  };
  ...
  const purgeDriver=async name=>{if(!await uiConfirm(`Permanently delete ${name} from the retired list?\n\nThis cannot be undone.`))return;saveRetiredDrivers(retiredDrivers.filter(x=>x.name!==name));};

  // v2.24.9: a load that cannot finish now says so and offers a way out. It must NOT
  // fall through to the app: the roster fallbacks would seed the built-in truck list,
  // and the next save would write that over a real fleet.
  if(loadError)return <div style={{display:"flex",justifyContent:"center",alignItems:"center",height:"100vh",background:"#f5f7fa",padding:20}}>
    <div style={{maxWidth:380,textAlign:"center",color:"#243447"}}>
      <div style={{fontSize:34,marginBottom:10}}>📡</div>
      <div style={{fontSize:18,fontWeight:800,marginBottom:8}}>Can&rsquo;t reach the database</div>
      {/* v2.27.0: this used to say the stall was "usually a weak signal". That was a
          guess, it was shown to someone on full bars, and it sent them looking at the
          wrong thing. Say what happened and which read it was; the cause is what the
          detail line is for. */}
      <div style={{fontSize:14,lineHeight:1.5,color:"#5b6b7a",marginBottom:14}}>
        {loadError.kind==="timeout"
          ? "Your fleet roster took too long to load."
          : "The database did not answer when asked for your fleet roster."}
        <br/><br/>Nothing has been changed or lost — the app stopped here on purpose rather than starting up with a blank roster and saving that over your fleet.
      </div>
      {Array.isArray(loadError.detail)&&loadError.detail.length>0&&
        <div style={{fontSize:11,lineHeight:1.6,color:"#8593a3",marginBottom:16,textAlign:"left",
          background:"#eef2f6",border:"1px solid #dde3ea",borderRadius:6,padding:"8px 10px",fontFamily:"ui-monospace,Menlo,Consolas,monospace"}}>
          {loadError.detail.map((d,i)=><div key={i}>{d}</div>)}
        </div>}
      <button onClick={async()=>{setLoadError(null);await resetFirestoreConnection();setLoadAttempt(a=>a+1);}}
        style={{background:C.brand,color:"#fff",border:0,borderRadius:6,padding:"12px 22px",fontSize:16,fontWeight:700,minHeight:44,cursor:"pointer"}}>Try again</button>
      <div style={{fontSize:11,color:"#94a3b8",marginTop:14}}>v{APP_VERSION}</div>
    </div>
  </div>;
  if(!loaded)return <div style={{display:"flex",justifyContent:"center",alignItems:"center",height:"100vh",background:"#f5f7fa",flexDirection:"column",gap:12}}>
    <div style={{width:36,height:36,border:"3px solid #dde3ea",borderTop:`3px solid ${C.brand}`,borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>
    {/* Say what it is waiting on. A bare spinner is indistinguishable from a hang. */}
    <div style={{fontSize:13,color:"#6b7785"}}>Loading your fleet&hellip;</div>
  </div>;
    ...
    <div style={s.wrap}>
      ...
      <div style={s.header}><div style={s.hInner}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{borderLeft:`2px solid ${C.brand}33`,paddingLeft:10}}><div style={{fontSize:13,fontWeight:700,color:C.dark,lineHeight:1.2}}>Fleet</div><div style={{fontSize:13,fontWeight:700,color:C.dark,lineHeight:1.2}}>Management</div><div onClick={()=>setShowAdmin(a=>!a)} title="Tap to toggle admin/diagnostic tools" style={{fontSize:8,color:showAdmin?C.accent:"#94a3b8",marginTop:1,cursor:"pointer",userSelect:"none",fontWeight:showAdmin?700:400}}>v{APP_VERSION}{showAdmin?" · admin":""}</div></div>
        </div>
        <div style={s.tabs}>
          {[["dashboard","Dashboard"],["board","Driver Board"],["fleet","Fleet List"],["maint","Maintenance"],["costs","Costs"],["drivers","Drivers"],["attend","Attendance"],["settings","Settings"]].map(([k,l])=>
            <button key={k} onClick={()=>setTab(k)} style={tab===k?{...s.tab,...s.tabOn}:s.tab}>{l}{k==="maint"&&stats.openRepairs>0?<span style={s.badge}>{stats.openRepairs}</span>:null}{k==="costs"&&cores.filter(c=>c.status==="outstanding").length>0?<span style={s.badge}>{cores.filter(c=>c.status==="outstanding").length}</span>:null}</button>
          )}
        </div>
      </div></div>
      ...
      <div style={s.body}>
        ...
        {/* v2.19.1: this banner used to render inside the Costs tab only. A dispatcher
            marking a truck out of service on the Weekly Board, or a mechanic closing a
            repair, saw the optimistic state update and no warning at all when the write
            was actually rejected — the one notice that existed was on a tab they were
            not looking at, and the reason lived only in a devtools global. Out-of-service
            and repair status decide whether a truck goes on the road, so a rejected write
            has to be visible wherever it happens. */}
        {saveError&&<div style={{background:"#fef2f2",border:`1px solid ${C.red}`,borderRadius:8,padding:"10px 12px",marginBottom:16,display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
          <div style={{fontSize:12,color:C.red,fontWeight:600,lineHeight:1.5}}>
            ⚠ Not saved: <code style={{fontSize:11}}>{saveError.key}</code> {saveError.msg}.
            <div style={{fontSize:11,fontWeight:400,color:"#7f1d1d",marginTop:2}}>
              {/^fl-costs/.test(saveError.key||"")
                ?"Recent changes to this data may not be saved. Archive or export old cost entries to free space."
                :"Your last change was not written to the database. Check your connection and try it again — reloading now would lose it."}
            </div>
          </div>
          <button onClick={()=>setSaveError(null)} style={s.xBtn} title="Dismiss">×</button>
        </div>}

        {/* Live Motive conflict banner — only fires on a REAL conflict (Motive's
            assigned/permanent driver differs from the board). Trucks the board
            assigns but that have no Motive assignment are a quiet footnote.
            v2.16.9: scoped to Dispatch + Weekly Board. It used to render outside
            every tab check, so a driver-assignment warning sat on top of
            Maintenance, Costs, Drivers, etc. — pages where it isn't actionable.
            These are the two screens where assignments are actually made/viewed. */}
        {(tab==="dashboard"||tab==="board")&&(()=>{
          const SM={mismatch:{c:C.red,label:"Different driver in Motive"},motive_only:{c:C.accent,label:"Unassigned on board"}};
          const fresh=Object.entries(motiveLiveFlags).filter(([,f])=>f.ts>motiveDismissAt);
          const conflicts=fresh.filter(([,f])=>f.status==="mismatch"||f.status==="motive_only");
          const pending=fresh.filter(([,f])=>f.status==="board_only").length;
          if(!conflicts.length)return null; // nobody logged into a conflicting truck → stay quiet
          return <div style={{margin:"0 0 12px",background:"#fef2f2",border:`1px solid ${C.red}55`,borderRadius:8,padding:"10px 12px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
              <div style={{fontSize:12,fontWeight:800,color:C.red}}>⚠️ {conflicts.length} truck{conflicts.length===1?"":"s"} conflict{conflicts.length===1?"s":""} with Motive (today)</div>
              <button onClick={()=>setMotiveDismissAt(Date.now())} style={{border:"none",background:"none",fontSize:11,color:"#94a3b8",cursor:"pointer",fontWeight:700}}>Dismiss</button>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:3,marginTop:6}}>
              {conflicts.map(([tid,f])=>{const sm=SM[f.status];return(
                <div key={tid} style={{fontSize:11,display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontWeight:800,minWidth:42}}>#{tid}</span>
                  <span style={{flex:1,minWidth:0}}><span style={{color:"#64748b"}}>board:</span> <b>{f.boardName||"—"}</b> <span style={{color:"#cbd5e1"}}>vs</span> <span style={{color:"#64748b"}}>Motive:</span> <b>{f.motiveName||"—"}</b></span>
                  <span style={{fontSize:9,fontWeight:700,color:sm.c,textTransform:"uppercase",whiteSpace:"nowrap"}}>{sm.label}</span>
                </div>);})}
            </div>
            {pending>0&&<div style={{fontSize:10,color:"#94a3b8",marginTop:6}}>+ {pending} more on the board with no driver assigned in Motive</div>}
          </div>;
        })()}

        {/* ══ DAILY DISPATCH ══ */}
        {/* ══ DASHBOARD ══ */}
        {/* v2.21.0: Dispatch and Dashboard were one screen's worth of information split
            across two tabs. Four of the six live tiles here — Assigned Today, Available,
            OOS/Down, Total Drivers — restated counts the dispatch lists already showed,
            and showed them with the unit and driver names attached, which is strictly
            more useful than the number alone. Merged, summary first then the detail it
            summarises: tiles, today's assignment lists, then the standing fleet
            breakdown and the advisor. */}
        {tab==="dashboard"&&<div>
          <div style={s.statGrid}>
            <Stat l="Total Fleet" v={stats.total} sub={`${stats.straight} Straight · ${stats.tractor} Tractor`} c={C.brand} onClick={()=>setStatDrill("total")}/>
            <Stat l="Assigned Today" v={stats.assigned} c={C.green} onClick={()=>setStatDrill("assigned")}/>
            <Stat l="Available" v={stats.avail} c={C.cyan} onClick={()=>setStatDrill("avail")}/>
            <Stat l="OOS / Down" v={stats.oos} c={C.red} onClick={()=>setStatDrill("oos")}/>
            <Stat l="Open Repairs" v={stats.openRepairs} c={C.yellow} onClick={()=>setStatDrill("openRepairs")}/>
            <Stat l="Total Drivers" v={stats.drivers} sub={`${stats.davis} Davis · ${stats.owner} Owner Op`} c={C.purple} onClick={()=>setStatDrill("drivers")}/>
            <Stat l="Automatic" v={stats.auto} sub={`${stats.autoBox} Box · ${stats.autoTr} Tractor`} c={C.green} onClick={()=>setStatDrill("auto")}/>
            <Stat l="Manual" v={stats.manual} sub={`${stats.manBox} Box · ${stats.manTr} Tractor`} c={C.accent} onClick={()=>setStatDrill("manual")}/>
          </div>
        ...
        {(()=>{
          ...
          // v2.10.24: YESTERDAY SNAPSHOT MODE
          // If today has fewer than 3 driver assignments, assume dispatch hasn't
          // been updated yet and fall back to yesterday's data as a read-only
          // preview. This gives dispatch the 4AM view they need without anyone
          // having to manually hit "Same as Yesterday". Once today's assignments
          // are filled in (>=3 drivers assigned), the real data takes over automatically.
          // v2.10.27: Monday now reads last week's Friday from attendWeeks instead
          // of silently doing nothing (di>0 guard removed).
          const todayAssigned=drivers.filter(d=>{
            const v=asgn[`${d.name}-${today}`]||"";
            return v&&!OFF_OPTS.includes(v)&&v!=="";
          }).length;
          const useYesterdaySnapshot=todayAssigned<3;
          // Determine fallback day and data source
          let viewDay=null;
          let viewAsgnSource={};
          let snapshotLabel="yesterday";
          if(useYesterdaySnapshot){
            if(di>0){
              // Tue–Fri: read same week's prior day from current asgn
              viewDay=DAYS[di-1];
              viewAsgnSource=asgn;
              snapshotLabel=["Mon","Tue","Wed","Thu","Fri"][di-1];
            } else {
              // Monday: read last week's Friday from attendWeeks
              viewDay="Fri";
              snapshotLabel="Fri";
              const lastMon=new Date(gM(weekDate));
              lastMon.setDate(lastMon.getDate()-7);
              const lastWkKey=wK(lastMon);
              viewAsgnSource=attendWeeks[lastWkKey]||{};
            }
          }
          const getViewVal=(driverName)=>{
            if(!useYesterdaySnapshot||!viewDay)return asgn[`${driverName}-${today}`]||"";
            return viewAsgnSource[`${driverName}-${viewDay}`]||asgn[`${driverName}-${today}`]||"";
          };
          const viewTStat=(truckId)=>{
            if(!useYesterdaySnapshot||!viewDay)return tStat[`${truckId}-${today}`]||"";
            // tStat is current-week only; for Mon cross-week fallback use today's tStat
            const key=di>0?`${truckId}-${viewDay}`:`${truckId}-${today}`;
            return tStat[key]||"";
          };

          // Categorize drivers using viewDay data
          const working=[];const off=[];const calledOut=[];const noTruck=[];
          drivers.filter(d=>d.category==="Davis").forEach(d=>{
            const v=getViewVal(d.name);
          ...
          return <div>
            {/* Date header */}
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:useYesterdaySnapshot?8:16}}>
              <div>
            ...
            {/* Yesterday snapshot banner */}
            {useYesterdaySnapshot&&<div style={{background:"#fffbeb",border:`1px solid ${C.accent}55`,borderRadius:8,padding:"10px 14px",marginBottom:16,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
              <div>
                <div style={{fontSize:12,fontWeight:700,color:C.accent}}>📋 Showing {snapshotLabel}'s assignments as today's snapshot</div>
                <div style={{fontSize:11,color:"#92400e",marginTop:2}}>Today's assignments haven't been entered yet ({todayAssigned} assigned). This view will update automatically once dispatch fills in {dayFull}.</div>
              </div>
              <button onClick={()=>sameAsYesterday(di)} style={{padding:"8px 14px",fontSize:12,fontWeight:700,background:C.accent,color:"#fff",border:"none",borderRadius:6,cursor:"pointer",whiteSpace:"nowrap"}}>
                📋 Copy to {dayFull}
              </button>
            </div>}

            {/* Who is on today. Trucks are counted by the tiles at the top of this screen,
                so this row is people only — no tile reports driver status.
                The truck pills that used to sit here ("Trucks Free", "Down") were removed
                when Dispatch and Dashboard became one screen: they read as duplicates of
                the Available and OOS/Down tiles a few inches above, but were not the same
                number. trucksDown uses viewTStat, so it follows the yesterday-snapshot and
                counts interstate/repair/mech statuses; stats.oos uses today's tStat and
                counts open repairs instead. Two almost-identically-labelled numbers that
                legitimately disagree read as a bug. Neither count is lost — both still
                head their own list below, with the unit numbers attached. */}
            ...
            {/* Needs attention — suppress noTruck warning in snapshot mode since it's yesterday's data */}
            {(!useYesterdaySnapshot&&noTruck.length>0||calledOut.length>0||openRep.length>0)&&<div style={{background:"#fef2f2",border:`1px solid ${C.red}22`,borderRadius:8,padding:12,marginBottom:16}}>
              <div style={{fontSize:12,fontWeight:700,color:C.red,marginBottom:6}}>⚠ Needs Attention</div>
              {!useYesterdaySnapshot&&noTruck.length>0&&<div style={{fontSize:12,color:"#334155",marginBottom:4}}><span style={{fontWeight:700}}>{noTruck.length} driver{noTruck.length>1?"s":""}</span> not assigned a truck: {noTruck.slice(0,5).map(d=>d.name.split(" ")[0]).join(", ")}{noTruck.length>5?"...":""}</div>}
            ...
            {/* Working drivers */}
            <div style={{fontSize:13,fontWeight:700,color:C.green,marginBottom:6}}>ON THE ROAD — {working.length} Davis + {ownerWorking.length} Owner{useYesterdaySnapshot?<span style={{fontSize:10,fontWeight:400,color:C.accent,marginLeft:8}}>(from {snapshotLabel})</span>:null}</div>
            <div style={s.tWrap}><table style={{...s.table,fontSize:12}}><thead><tr>
              ...
              {working.map(d=> <tr key={d.name} style={s.tr}>
                ...
                <td style={s.ltd}>{d.truckInfo?truckDesc(d.truckInfo):"—"}</td>
            ...
            {trucksAvail.length>0&&<div style={{marginTop:16}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(120px, 1fr))",gap:6}}>
                {trucksAvail.map(t=><div key={t.id} style={{padding:"8px 10px",background:"#fff",borderRadius:6,border:"1px solid #e2e8f0",cursor:"pointer"}} onClick={()=>setHistoryTruck(t.id)}>
                  ...
                  <div style={{fontSize:10,color:"#6b7785"}}>{truckDesc(t)} · {t.tr==="A"?"A":"M"} · {t.ax==="Tandem"?"T":"S"}</div>
                </div>)}
              </div>
            </div>}
            ...
            {trucksDown.length>0&&<div style={{marginTop:16}}>
              ...
              {trucksDown.map(t=>{const r=repairs.find(x=>x.truckId===t.id&&x.status==="open");
                return <div key={t.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px",background:"#fef2f2",borderRadius:6,border:`1px solid ${C.red}22`,marginBottom:4,cursor:"pointer"}} onClick={()=>setHistoryTruck(t.id)}>
                  <div><span style={{fontFamily:"monospace",fontWeight:700,color:C.red}}>#{t.id}</span><span style={{fontSize:11,color:"#6b7785",marginLeft:8}}>{truckDesc(t)}</span></div>
        ...
        {/* Dashboard stat-card drill-down — click a tile to see the trucks/drivers/repairs behind it */}
        {statDrill&&(()=>{
          const close=()=>setStatDrill(null);
          const cfg={
            total:{title:"Total Fleet",color:C.brand,kind:"trucks",list:statLists.total},
            assigned:{title:"Assigned Today",color:C.green,kind:"assigned",list:statLists.assigned},
            avail:{title:"Available Trucks",color:C.cyan,kind:"trucks",list:statLists.avail},
            oos:{title:"OOS / Down",color:C.red,kind:"oos",list:statLists.oos},
            openRepairs:{title:"Open Repairs",color:C.yellow,kind:"repairs",list:statLists.openRepairs},
            drivers:{title:"Total Drivers",color:C.purple,kind:"drivers",list:statLists.allDrivers},
            auto:{title:"Automatic Trucks",color:C.green,kind:"trucks",list:statLists.auto},
            manual:{title:"Manual Trucks",color:C.accent,kind:"trucks",list:statLists.manual},
          }[statDrill];
          if(!cfg)return null;
          const truckChip=(t,extra)=><div key={t.id} onClick={()=>{close();setHistoryTruck(t.id);}} style={{padding:"8px 10px",background:"#fff",borderRadius:6,border:"1px solid #e2e8f0",cursor:"pointer"}}>
            <div style={{fontFamily:"monospace",fontWeight:700,fontSize:13,color:C.brand}}>#{t.id}</div>
            <div style={{fontSize:10,color:"#6b7785"}}>{truckDesc(t)} · {t.tr==="A"?"Auto":"Man"} · {t.ax==="Tandem"?"Tandem":"Single"}</div>
            {extra}
          </div>;
          return <div style={s.modal} onClick={close}>
            <div style={{...s.modalBox,maxWidth:560}} onClick={e=>e.stopPropagation()}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div style={{fontSize:15,fontWeight:800,color:cfg.color}}>{cfg.title} <span style={{color:"#94a3b8",fontWeight:600}}>({cfg.list.length})</span></div>
                <button onClick={close} style={{border:"none",background:"none",fontSize:22,lineHeight:1,cursor:"pointer",color:"#94a3b8"}}>×</button>
              </div>
              {cfg.list.length===0
                ? <div style={{fontSize:12,color:"#94a3b8",fontStyle:"italic",padding:"12px 0"}}>Nothing here right now.</div>
                : cfg.kind==="trucks"
                  ? <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(130px, 1fr))",gap:6}}>{cfg.list.map(t=>truckChip(t))}</div>
                  : cfg.kind==="assigned"
                    ? <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(150px, 1fr))",gap:6}}>{cfg.list.map(t=>truckChip(t,<div style={{fontSize:11,color:C.green,fontWeight:600,marginTop:2}}>{(statLists.driverOf[t.id]||[]).map(n=>n.split(" ")[0]).join(", ")}</div>))}</div>
                    : cfg.kind==="oos"
                      ? <div style={{display:"flex",flexDirection:"column",gap:4}}>{cfg.list.map(t=>{const r=repairs.find(x=>x.truckId===t.id&&x.status==="open");return <div key={t.id} onClick={()=>{close();setHistoryTruck(t.id);}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px",background:"#fef2f2",borderRadius:6,border:`1px solid ${C.red}22`,cursor:"pointer"}}>
                          <div><span style={{fontFamily:"monospace",fontWeight:700,color:C.red}}>#{t.id}</span><span style={{fontSize:11,color:"#6b7785",marginLeft:8}}>{truckDesc(t)}</span></div>
                          <span style={{fontSize:10,color:C.red,fontWeight:600}}>{r?`${r.reason}${r.shop?` · ${r.shop}`:""}`:"Marked OOS"}</span>
                        </div>;})}</div>
                      : cfg.kind==="repairs"
                        ? <div style={{display:"flex",flexDirection:"column",gap:4}}>{cfg.list.map(r=>{const t=trucks.find(x=>x.id===r.truckId);return <div key={r.id} onClick={()=>{close();setHistoryTruck(r.truckId);}} style={{padding:"8px 10px",background:"#fff",borderRadius:6,border:"1px solid #e2e8f0",cursor:"pointer"}}>
                            <div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontFamily:"monospace",fontWeight:700,color:C.brand}}>#{r.truckId}</span><span style={{fontSize:11,fontWeight:700,color:C.red}}>{r.reason}</span></div>
                            <div style={{fontSize:10,color:"#94a3b8",marginTop:2}}>In: {dateStr(r.dateIn)}{r.shop?` · ${r.shop}`:""}{t?` · ${truckDesc(t)}`:""}</div>
                          </div>;})}</div>
                        : cfg.kind==="drivers"
                          ? <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{cfg.list.map(d=><span key={d.name} onClick={()=>{close();setDriverReport(d.name);}} style={{fontSize:11,padding:"5px 10px",background:"#fff",color:"#334155",borderRadius:6,fontWeight:600,border:"1px solid #e2e8f0",cursor:"pointer"}}>{drvIcon(d.role)} {d.name} <span style={{color:"#94a3b8"}}>({(d.role||"").replace("Davis ","").replace("Owner Operator","Owner Op")})</span></span>)}</div>
                          : null}
            </div>
          </div>;
        })()}
        ...
        {tab==="board"&&<div>
          <div style={s.weekNav}>
            ...
            {(()=>{
              const td=motiveVehicles.length?buildMotiveComparison(DAYS[Math.min(todayDI(),4)]).filter(r=>r.status==="mismatch"||r.status==="motive_only").length:0;
              return <button style={{...s.wBtnA,marginLeft:"auto",background:C.purple,borderColor:C.purple}} onClick={openMotiveSync} title="Compare board driver assignments against Motive">🔗 Motive Check{td>0?<span style={{...s.badge,background:"#fff",color:C.red}}>{td}</span>:null}</button>;
            })()}
          </div>
          ...
          {showMotiveSync&&(()=>{
            ...
            <div style={s.modal} onClick={()=>setShowMotiveSync(false)}>
              <div style={{...s.modalBox,maxWidth:580}} onClick={e=>e.stopPropagation()}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{fontSize:15,fontWeight:800,color:C.dark}}>Board vs Motive — driver check</div>
                ...
                <div style={{fontSize:11,color:"#6b7785"}}>Compares the selected day's board assignments against the driver Motive currently reports on each truck, and flags any differences.</div>
                ...
                <div style={{display:"flex",alignItems:"center",gap:8,marginTop:4,flexWrap:"wrap"}}>
                  ...
                  <label style={{fontSize:11,color:"#475569",display:"flex",alignItems:"center",gap:4,marginLeft:"auto",cursor:"pointer"}}>
                    <input type="checkbox" checked={motiveLiveOn} onChange={e=>setMotiveLiveOn(e.target.checked)}/> Live-check today's edits
                  </label>
                </div>
                ...
                {(()=>{
                  const rows=buildMotiveComparison(motiveSyncDay);
                  const conflicts=rows.filter(r=>r.status==="mismatch"||r.status==="motive_only");
                  const pending=rows.filter(r=>r.status==="board_only");   // on board, no Motive assignment
                  const matches=rows.filter(r=>r.status==="match").length;
                  const SM={mismatch:{c:C.red,bg:"#fef2f2",label:"Different driver"},motive_only:{c:C.accent,bg:"#fff7ed",label:"Unassigned on board"}};
                  return <div style={{marginTop:4}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                      <div style={{fontWeight:700,fontSize:12,color:C.dark}}>{motiveSyncDay}: {conflicts.length} conflict{conflicts.length===1?"":"s"}</div>
                      {rows.length>0&&<span style={{fontSize:10,color:"#94a3b8"}}>{matches} match{matches===1?"":"es"}{pending.length?` · ${pending.length} not set in Motive`:""}</span>}
                    </div>
                    <div style={{fontSize:10,color:"#94a3b8",marginTop:2}}>Compares the board against each truck's <b>assigned</b> driver in Motive (permanent driver), not whoever is currently driving.</div>
                    <div style={{maxHeight:230,overflowY:"auto",marginTop:6,display:"flex",flexDirection:"column",gap:4}}>
                      {motiveVehicles.length===0&&!motiveLoading&&<div style={{fontSize:11,color:"#94a3b8"}}>Load from Motive to compare.</div>}
                      {motiveVehicles.length>0&&rows.length===0&&<div style={{fontSize:11,color:"#94a3b8"}}>No mapped trucks have a driver on the board or in Motive for {motiveSyncDay}.</div>}
                      {motiveVehicles.length>0&&conflicts.length===0&&rows.length>0&&<div style={{fontSize:11,color:C.green,fontWeight:600}}>✓ No conflicts — the board matches Motive's assigned drivers.</div>}
                      {conflicts.map(r=>{const sm=SM[r.status];return(
                        <div key={r.truckId} style={{display:"flex",alignItems:"center",gap:8,fontSize:11,background:sm.bg,border:`1px solid ${sm.c}33`,borderRadius:6,padding:"6px 8px"}}>
                          <span style={{fontWeight:800,minWidth:42}}>#{r.truckId}</span>
                          <span style={{flex:1,minWidth:0}}>
                            <span style={{color:"#64748b"}}>Board:</span> <b>{r.boardName||"—"}</b>
                            <span style={{margin:"0 6px",color:"#cbd5e1"}}>vs</span>
                            <span style={{color:"#64748b"}}>Motive:</span> <b>{r.motiveName||"—"}</b>
                          </span>
                          <span style={{fontSize:9,fontWeight:700,color:sm.c,textTransform:"uppercase",whiteSpace:"nowrap"}}>{sm.label}</span>
                        </div>);})}
                      {pending.length>0&&<details style={{marginTop:2}}>
                        <summary style={{fontSize:10,color:"#94a3b8",cursor:"pointer"}}>{pending.length} on board with no driver assigned in Motive</summary>
                        <div style={{display:"flex",flexDirection:"column",gap:2,marginTop:4}}>
                          {pending.map(r=><div key={r.truckId} style={{fontSize:11,color:"#64748b"}}><b style={{minWidth:42,display:"inline-block"}}>#{r.truckId}</b> {r.boardName}</div>)}
                        </div>
                      </details>}
                    </div>
                  </div>;
                ...
                <div style={{marginTop:8,fontSize:10,color:"#94a3b8",borderTop:"1px solid #f1f5f9",paddingTop:8}}>
                  Read-only. Pushing assignments to Motive needs a TMS integration enabled on your Motive account (Motive sets a vehicle's driver from ELD login), so this checks for drift instead.
                </div>
          ...
          {/* v2.10.31: emergency diagnostic — dev-only, hidden unless admin mode is on (tap the version number) */}
          {showAdmin&&<details style={{marginBottom:12,padding:"8px 12px",background:"#fffbeb",border:`1px solid ${C.accent}55`,borderRadius:6}}>
            <summary style={{fontSize:11,fontWeight:700,color:C.accent,cursor:"pointer"}}>🔍 Data Diagnostic & Recovery <span style={{fontWeight:400,color:"#b45309"}}>— developer tools, use with care</span></summary>
            <div style={{marginTop:10,fontSize:11,fontFamily:"monospace",color:"#1e293b"}}>
              <div><b>Current weekDate:</b> {weekDate.toString()}</div>
              <div><b>Current wk (storage key):</b> fl-asgn-{wk}</div>
              <div><b>asgn entries in memory:</b> {Object.keys(asgn).length}</div>
              <div><b>attendWeeks (history) loaded:</b> {Object.keys(attendWeeks).length} weeks</div>
              <div style={{marginTop:10,display:"flex",gap:6,flexWrap:"wrap"}}>
                <button onClick={async()=>{
                  try{
                    const keys=await window.storage.list("fl-asgn-");
                    const list=keys?.keys||[];
                    if(list.length===0){alert("NO fl-asgn-* keys found in Firebase. Either data was wiped or no assignments have ever been saved.");return;}
                    const sample=[];
                    for(const k of list.slice(0,5)){
                      try{const r=await window.storage.get(k);const data=r?JSON.parse(r.value):null;const count=data?Object.keys(data).length:0;sample.push(`  ${k}: ${count} entries`);}catch(e){sample.push(`  ${k}: ERROR ${e.message}`);}
                    }
                    alert(`Found ${list.length} fl-asgn-* keys in Firebase.\n\nFirst 5:\n${sample.join("\n")}\n\nAll keys:\n${list.join("\n")}`);
                  }catch(e){alert("List failed: "+e.message);}
                }} style={{padding:"6px 12px",fontSize:11,fontWeight:700,background:C.brand,color:"#fff",border:"none",borderRadius:4,cursor:"pointer"}}>📊 List asgn keys</button>
                <button onClick={async()=>{
                  try{
                    window.__lastStorageError=null;
                    const keys=await window.storage.list("");
                    const list=keys?.keys||[];
                    const err=keys?.error||window.__lastStorageError;
                    if(list.length===0&&err){
                      alert(`LIST OPERATION FAILED.\n\nError: ${err}\n\nThis means Firestore security rules are blocking the collection-list query. Data may still exist — try the '🎯 Probe known keys directly' button to test specific docs.`);
                      return;
                    }
                    const grouped={};
                    list.forEach(k=>{const prefix=k.match(/^(fl-[a-z]+)/)?.[1]||"other";grouped[prefix]=(grouped[prefix]||0)+1;});
                    const summary=Object.entries(grouped).map(([p,n])=>`  ${p}-*: ${n}`).join("\n");
                    alert(`Total docs in kv collection: ${list.length}\n\nBy prefix:\n${summary||"  (none)"}\n\nFirst keys:\n${list.slice(0,40).join("\n")||"(empty)"}${list.length>40?`\n...(+${list.length-40} more)`:""}`);
                  }catch(e){alert("List failed: "+e.message);}
                ...
                <button onClick={async()=>{
                  const out=document.getElementById("migrate-makes-out");
                  out.style.display="block";
                  const map={"FRTLN":"Freightliner","INTL":"International","HINO":"Hino"};
                  const changes=trucks.filter(t=>map[t.mk]).length;
                  if(changes===0){out.innerHTML='<div style="color:#16a34a">✓ No abbreviated makes found — nothing to migrate.</div>';return;}
                  if(!await uiConfirm(`Migrate ${changes} trucks?\n\nFRTLN → Freightliner\nINTL → International\nHINO → Hino\n\nProceed?`))return;
                  out.innerHTML=`<div style="color:#64748b">Migrating ${changes} trucks...</div>`;
                  const updated=trucks.map(t=>map[t.mk]?{...t,mk:map[t.mk]}:t);
                  window.__lastStorageError=null;
                  const result=await window.storage.set("fl-trucks",JSON.stringify(updated));
                  if(result){
                    saveTrucks(updated);
                    out.innerHTML=`<div style="color:#16a34a">✓ Migrated ${changes} trucks. FRTLN → Freightliner, INTL → International, HINO → Hino. Saved to Firebase.</div>`;
                  }else{
                    const err=window.__lastStorageError||"(no error captured)";
                    out.innerHTML=`<div style="color:#dc2626">✗ Migration FAILED: ${err}</div>`;
                  }
                }} style={{padding:"6px 12px",fontSize:11,fontWeight:700,background:"#8b5cf6",color:"#fff",border:"none",borderRadius:4,cursor:"pointer"}}>🔤 Migrate legacy makes (FRTLN/INTL/HINO)</button>
                <div id="migrate-makes-out" style={{display:"none",marginTop:6,padding:"6px 10px",background:"#f1f5f9",borderRadius:4,fontSize:11,fontFamily:"monospace",lineHeight:1.5}}></div>
                ...
                <button onClick={async()=>{
                  ...
                  const knownKeys=[
                    "fl-trucks","fl-drivers","fl-repairs","fl-costs","fl-cores","fl-retired","fl-vendors",
                    "fl-asgn-2026-05-04","fl-asgn-2026-04-27","fl-asgn-2026-04-20","fl-asgn-2026-04-13",
                    "fl-asgn-2026-04-06","fl-asgn-2026-03-30","fl-asgn-2025-12-29","fl-asgn-2025-06-30",
                    "fl-stat-2026-05-04","fl-stat-2026-04-13","fl-dvir-imported"
                  ];
                  const lines=[];
                  for(const k of knownKeys){
                    try{
                      const r=await window.storage.get(k);
                      if(!r){lines.push(`<div style="color:#dc2626">✗ ${k}: returned null</div>`);continue;}
                      let preview="(parse failed)";
                      try{
                        const parsed=JSON.parse(r.value);
                        if(Array.isArray(parsed))preview=`array with ${parsed.length} items`;
                        else if(typeof parsed==="object"&&parsed)preview=`object with ${Object.keys(parsed).length} keys`;
                        else preview=`${typeof parsed}: ${String(parsed).substring(0,40)}`;
                      }catch(e){preview="(unparseable: "+r.value.substring(0,60)+")";}
                      lines.push(`<div style="color:#16a34a">✓ ${k}: <b>EXISTS</b> — ${preview}</div>`);
                    }catch(e){
                      const msg=e.message||String(e);
                      if(msg==="not found"||/not found/i.test(msg)){
                        lines.push(`<div style="color:#94a3b8">— ${k}: not found (doc doesn't exist)</div>`);
                      }else{
                        lines.push(`<div style="color:#dc2626">✗ ${k}: <b>ERROR</b> — ${msg}</div>`);
                      }
                    }
                  }
                  out.innerHTML=lines.join("");
                }} style={{padding:"6px 12px",fontSize:11,fontWeight:700,background:"#ef4444",color:"#fff",border:"none",borderRadius:4,cursor:"pointer"}}>🎯 Probe known keys directly</button>
                <div id="probe-out" style={{display:"none",marginTop:6,padding:"8px 10px",background:"#f1f5f9",borderRadius:4,fontSize:11,fontFamily:"monospace",lineHeight:1.5,maxHeight:300,overflowY:"auto"}}></div>
              </div>
              <div style={{marginTop:14,paddingTop:10,borderTop:`1px solid ${C.accent}55`}}>
                <div style={{fontSize:11,fontWeight:700,color:C.red,marginBottom:6}}>⚠ RECOVERY — restore from DVIR backup file</div>
                <div style={{fontSize:10,color:"#475569",marginBottom:8}}>This is a VISIBLE recovery — runs in this panel with live progress so you can see exactly what's happening at each step. No page reload needed. Safe to re-run.</div>
                <button id="recovery-btn" onClick={async()=>{
                  const log=document.getElementById("recovery-log");
                  const btn=document.getElementById("recovery-btn");
                  const append=(line,color)=>{
                    const d=document.createElement("div");
                    d.style.color=color||"#1e293b";
                  ...
                  log.innerHTML="";
                  try{
                    append("=== DVIR Recovery Starting ===","#3b82f6");
                    append("Step 1/4: Verifying /dvir_history.json is accessible at this URL...");
                    const t0=Date.now();
                    const resp=await fetch("/dvir_history.json");
                    if(!resp.ok){
                      append(`✗ FETCH FAILED: HTTP ${resp.status} ${resp.statusText}`,"#dc2626");
                      append(`The file is not accessible at ${window.location.origin}/dvir_history.json`,"#dc2626");
                      append(`Either the file isn't deployed, or you're on a different URL than expected.`,"#dc2626");
                      btn.disabled=false;btn.textContent="🔄 Run Visible Recovery";return;
                    }
                    append(`✓ File found. HTTP ${resp.status}. Loading JSON...`,"#16a34a");
                    const data=await resp.json();
                    if(!Array.isArray(data)){
                      append(`✗ File loaded but is not an array. Type: ${typeof data}`,"#dc2626");
                      btn.disabled=false;btn.textContent="🔄 Run Visible Recovery";return;
                    }
                    append(`✓ Loaded ${data.length} weeks (${Math.round((Date.now()-t0)/1000)}s)`,"#16a34a");

                    append(`Step 2/4: Checking which weeks have assignment data...`);
                    const weeksWithData=data.filter(w=>w.week&&w.a&&Object.keys(w.a).length>0);
                    append(`✓ ${weeksWithData.length} weeks have assignment data`,"#16a34a");
                    if(weeksWithData.length===0){
                      append("✗ Backup file has no assignment data to restore","#dc2626");
                      btn.disabled=false;btn.textContent="🔄 Run Visible Recovery";return;
                    }

                    append(`Step 3/4: Writing ${weeksWithData.length} weeks to Firebase...`);
                    let ok=0,fail=0;let lastErr="";
                    for(let i=0;i<weeksWithData.length;i++){
                      const w=weeksWithData[i];
                      try{
                        let ex={};try{const r=await window.storage.get(`fl-asgn-${w.week}`);if(r)ex=JSON.parse(r.value);}catch(e){}
                        const merged={...ex,...w.a};
                        const result=await window.storage.set(`fl-asgn-${w.week}`,JSON.stringify(merged));
                        if(result){
                          ok++;
                          if(i%5===0||i===weeksWithData.length-1)append(`  [${i+1}/${weeksWithData.length}] fl-asgn-${w.week} (${Object.keys(merged).length} entries) ✓`,"#475569");
                        }else{
                          fail++;
                          lastErr=window.__lastStorageError||"(no error captured)";
                          if(fail<=3||fail%10===0)append(`  [${i+1}/${weeksWithData.length}] fl-asgn-${w.week} ✗ ${lastErr}`,"#dc2626");
                        }
                        // Also write status if present
                        if(w.s&&Object.keys(w.s).length>0){
                          let exS={};try{const r=await window.storage.get(`fl-stat-${w.week}`);if(r)exS=JSON.parse(r.value);}catch(e){}
                          await window.storage.set(`fl-stat-${w.week}`,JSON.stringify({...exS,...w.s}));
                        }
                      }catch(e){
                        fail++;
                        append(`  [${i+1}/${weeksWithData.length}] fl-asgn-${w.week} ✗ ERROR: ${e.message}`,"#dc2626");
                      }
                    }
                    append(`✓ Done writing. ${ok} succeeded, ${fail} failed`,fail>0?"#f59e0b":"#16a34a");
                    if(fail>0&&lastErr){
                      append("","#000");
                      append(`*** ROOT CAUSE: ${lastErr}`,"#dc2626");
                      if(/permission|denied|insufficient/i.test(lastErr)){
                        append(`This is a FIRESTORE SECURITY RULES issue.`,"#dc2626");
                        append(`Go to Firebase Console → Firestore Database → Rules`,"#f59e0b");
                        append(`Check if there's a rule blocking writes to fl-asgn-* docs.`,"#f59e0b");
                      }
                    }

                    append(`Step 4/4: Verifying writes...`);
                    const verifyKeys=await window.storage.list("fl-asgn-");
                    const count=verifyKeys?.keys?.length||0;
                    append(`✓ Firebase now has ${count} fl-asgn-* keys`,count>0?"#16a34a":"#dc2626");

                    if(count>0){
                      append("","#000");
                      append("=== RECOVERY COMPLETE ===","#16a34a");
                      append(`Navigate to a past week with ← Prev (e.g. April 2026) to see your data.`,"#16a34a");
                      append(`If the Driver Board still looks empty after navigating, hard-refresh the page.`,"#475569");
                      await window.storage.set("fl-dvir-imported",JSON.stringify({date:new Date().toISOString(),weeks:weeksWithData.length}));
                    }else{
                      append("","#000");
                      append("=== RECOVERY FAILED ===","#dc2626");
                      append(`All writes failed. This means Firestore is rejecting writes.`,"#dc2626");
                      append(`Possible causes: security rules, network, Firebase quota.`,"#dc2626");
                    }
                  }catch(e){
                    append(`✗ FATAL: ${e.message||e}`,"#dc2626");
                    append(`Stack: ${e.stack||"(no stack)"}`,"#dc2626");
                  }
                  btn.disabled=false;btn.textContent="🔄 Run Visible Recovery";
                }} style={{padding:"8px 16px",fontSize:12,fontWeight:700,background:C.red,color:"#fff",border:"none",borderRadius:6,cursor:"pointer"}}>🔄 Run Visible Recovery</button>
                <div id="recovery-log" style={{display:"none",marginTop:10,padding:"10px 12px",background:"#0f172a",color:"#e2e8f0",borderRadius:6,maxHeight:300,overflowY:"auto",fontSize:11,fontFamily:"monospace",border:"1px solid #1e293b"}}></div>
              </div>
            </div>
          </details>}
          ...
          <div style={s.tWrap}><table style={s.table}><thead><tr><th style={{...s.th,...s.thS}}>Driver</th>{DAYS.map((d,i)=><th key={d} style={{...s.th,width:96,minWidth:88}}>{d}<button onClick={()=>sameAsYesterday(i)} title={i>0?`Copy ${DAYS[i-1]} → ${d}`:`Copy last Friday → ${d}`} style={s.sayMini}>←</button></th>)}</tr></thead><tbody>
            {filteredRG.map(g=>{
              ...
              return[
                ...
                ...gd.map(dr=><tr key={dr.name} style={s.tr}><td style={{...s.td,...s.tdN,cursor:"pointer",color:C.brand,textDecoration:"underline",textDecorationColor:C.brand+"44"}} onClick={()=>setDriverReport(dr.name)}>{drvIcon(dr.role)} {dr.name}</td>
                  {DAYS.map(day=>{const ck=`${dr.name}-${day}`;const val=asgn[ck]||"";const isE=editCell===ck;const isOff=OFF_OPTS.includes(val);const ti=val&&!isOff?trucks.find(t=>t.id===val):null;const dup=val&&!isOff&&dupByDay[day]&&dupByDay[day].has(val);const oos=val&&!isOff&&!dup&&oosTrucks.has(val);
                    ...
                    if(isE){const av=getAvail(dr.role,day,val);return <td key={day} style={{...s.td,...s.eCell}}><div style={s.ePanel}>
                      ...
                      <select style={s.tSel} value="" onChange={e=>{if(e.target.value)commitEdit(ck,"assign",e.target.value);}}>
                        ...
                        {av.map(t=><option key={t.id} value={t.id}>{t.id} · {truckDesc(t)} · {t.tr==="A"?"Auto":"Man"} · {t.ax==="Tandem"?"Tndm":"Sngl"}</option>)}
                      </select>
                      <button onClick={()=>setEditCell(null)} style={s.doneBtn}>Done</button>
                    </div></td>;}
                    return <td key={day} style={dup?{...s.td,...dayCol,background:"#fef2f2",boxShadow:`inset 0 0 0 1.5px ${C.red}`}:oos?{...s.td,...dayCol,background:"#fff7ed",boxShadow:`inset 0 0 0 1.5px ${C.accent}`}:{...s.td,...dayCol}} title={dup?`Truck ${val} is double-booked on ${day}`:oos?`Truck ${val} is out of service (open repair) — reassign`:undefined} onClick={()=>startEdit(ck,val)}>{val?<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:1,maxWidth:"100%"}}>
                      <span style={{fontWeight:700,fontFamily:isOff?"inherit":"monospace",fontSize:isOff?9:(String(val).length>7?10:12),whiteSpace:"normal",wordBreak:"break-word",overflowWrap:"anywhere",lineHeight:1.15,maxWidth:"100%",textAlign:"center",color:dup?C.red:oos?C.accent:(isOff?C.accent:"#1e293b")}}>{dup?"⚠ ":oos?"🔧 ":""}{val}</span>
                      {ti&&<span style={{fontSize:8,color:dup?C.red:oos?C.accent:"#94a3b8"}}>{dup?"double-booked":oos?"IN SHOP":`${ti.type==="tractor"?"Tractor":"Box"} · ${ti.tr}`}</span>}
        ...
        {tab==="fleet"&&<div>
          ...
          <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
            ...
            <div style={s.vTog}><button onClick={()=>setFleetView("tile")} style={fleetView==="tile"?{...s.vBtn,...s.vBtnOn}:s.vBtn}>▦</button><button onClick={()=>setFleetView("list")} style={fleetView==="list"?{...s.vBtn,...s.vBtnOn}:s.vBtn}>☰</button><button onClick={()=>setFleetView("board")} style={fleetView==="board"?{...s.vBtn,...s.vBtnOn}:s.vBtn} title="Truck Weekly Board">📅</button></div>
          ...
          {showAddT&&<div style={s.addForm}>
            ...
            <div style={{display:"flex",gap:6}}>
              ...
              <label style={{display:"flex",alignItems:"center",gap:5,fontSize:12,color:C.dark,whiteSpace:"nowrap",cursor:"pointer"}}><input type="checkbox" checked={!!newT.del} onChange={e=>setNewT({...newT,del:e.target.checked})}/>Deleted</label>
              ...
              <input style={s.fInp} type="number" min="1980" max="2099" placeholder="Year" value={newT.year||""} onChange={e=>setNewT({...newT,year:e.target.value?Number(e.target.value):null})}/>
            </div>
          ...
          {fleetView==="tile"&&<div style={s.cardGrid}>{filteredTrucks.map(t=>{
            ...
            return <div key={t.id} style={s.tCard}><div style={s.cHdr}><span style={{...s.tNum,cursor:"pointer",textDecoration:"underline",textDecorationColor:C.brand+"44"}} onClick={()=>setHistoryTruck(t.id)}>#{t.id}</span><span style={{...s.sBdg,background:SC[st2]+"18",color:SC[st2],borderColor:SC[st2]}}>{SL[st2]}</span></div>
              <div style={s.cBody}><Row l="Type" v={t.type==="straight"?"Box Truck":"Tractor"}/><Row l="Make" v={normalizeMake(t.mk)}/>{t.md?<Row l="Model" v={t.md}/>:null}
                ...
                <Row l="Year" v={
                  <input type="number" min="1980" max="2099" placeholder="—"
                    defaultValue={t.year||""}
                    onBlur={e=>{const v=e.target.value.trim();if(v!==String(t.year||""))updateTruck(t.id,{year:v?Number(v):null});}}
                    onKeyDown={e=>{if(e.key==="Enter")e.target.blur();}}
                    title="Click to edit year"
                    style={{width:60,fontSize:11,padding:"2px 4px",border:"1px solid #e5eaf0",borderRadius:4,background:"#f8fafc",outline:"none",color:t.year?C.dark:"#94a3b8",fontWeight:600,textAlign:"right",cursor:"text"}}
                    onFocus={e=>{e.target.style.border=`1px solid ${C.brand}66`;e.target.style.background="#fff";}}
                    onBlurCapture={e=>{e.target.style.border="1px solid #e5eaf0";e.target.style.background="#f8fafc";}}/>
                }/>
                ...
                {t.del?<Row l="Deleted" v="Yes"/>:null}
                <Row l="Axle" v={t.ax}/>{dr&&<Row l="Driver" v={<span style={{color:drDown?"#94a3b8":C.brand,cursor:"pointer",textDecoration:"underline",fontStyle:drDown?"italic":"normal"}} onClick={()=>setDriverReport(dr.name)} title={drDown?"Truck is OOS — driver needs reassignment":""}>{dr.name}{drDown?" (reassign)":""}</span>}/>}
          ...
          {fleetView==="list"&&(()=>{
            // v2.10.40: Split into two side-by-side tables — Box Trucks on left,
            // Tractors on right. Mirrors the layout of the Truck Weekly Board view.
            // Type column is dropped (implicit in which table). On narrow screens
            ...
            const renderRow=(t)=>{
              ...
              const makeOpts=t.type==="tractor"?TRACTOR_MAKES:BOX_MAKES;
              const modelOpts=modelOptionsFor(t.mk);
              return <tr key={t.id} style={s.tr}>
                <td style={{...s.ltd,fontWeight:700,fontFamily:"monospace",color:C.brand,cursor:"pointer",textDecoration:"underline"}} onClick={()=>setHistoryTruck(t.id)}>{t.id}</td>
                <td style={s.ltd}>
                  <select value={t.mk} onChange={e=>updateTruck(t.id,{mk:e.target.value})}
                    style={{width:112,boxSizing:"border-box",fontSize:11,padding:"2px 4px",border:"1px solid #e2e8f0",borderRadius:4,background:"#fff",outline:"none",cursor:"pointer"}}>
                    {makeOpts.map(m=><option key={m} value={m}>{m}</option>)}
                    {!makeOpts.includes(t.mk)&&<option value={t.mk}>{normalizeMake(t.mk)}</option>}
                  </select>
                </td>
                {/* v2.16.17: free-text model, inline-editable like Year above.
                ...
                <td style={s.ltd}>
                  {modelOpts
                    ...
                    :<input placeholder="—" defaultValue={t.md||""}
                      ...
                      onKeyDown={e=>{if(e.key==="Enter")e.target.blur();}}
                      style={{width:96,boxSizing:"border-box",fontSize:11,padding:"2px 6px",border:"1px solid #e2e8f0",borderRadius:4,background:"#fff",outline:"none"}}/>}
                </td>
                <td style={s.ltd}>
                  <input type="number" min="1980" max="2099" placeholder="—"
                    defaultValue={t.year||""}
                    onBlur={e=>{const v=e.target.value.trim();if(v!==String(t.year||""))updateTruck(t.id,{year:v?Number(v):null});}}
                    onKeyDown={e=>{if(e.key==="Enter")e.target.blur();}}
                    style={{width:60,fontSize:11,padding:"2px 6px",border:"1px solid #e2e8f0",borderRadius:4,background:"#fff",outline:"none"}}/>
                </td>
                ...
                {/* v2.30.0: the owner's Deleted column, beside Trans. A marker on the truck
                    like the specs around it — it does not hide, retire or remove the truck
                    (retiring is the × at the end of the row). Saved as t.del. */}
                <td style={{...s.ltd,textAlign:"center"}}>
                  <input type="checkbox" checked={!!t.del} onChange={e=>updateTruck(t.id,{del:e.target.checked})}
                    aria-label={`Deleted: #${t.id}`} title={t.del?`#${t.id} is marked Deleted`:`Mark #${t.id} Deleted`}
            ...
            // v2.30.2: Box Trucks and Tractors are two tables stacked one above the other,
            // and each sized its columns to its own contents (different make lists, driver
            // names, status badges), so no column lined up with the one above it. Both now
            // share one set of fixed widths, and the five days split what is left evenly.
            // Below LIST_MIN_W the table scrolls sideways rather than squeezing the days.
            const LIST_COLS=["Truck #","Make","Model","Year","Trans","Deleted","Axle","Status",...DAYS,"Repairs",""];
            const LIST_COL_W={"Truck #":64,"Make":128,"Model":112,"Year":76,"Trans":52,"Deleted":68,"Axle":68,"Status":118,"Repairs":64,"":40};
            const LIST_MIN_W=1140;
            const renderTable=(list,label,borderColor)=>(
              <div style={{flex:"1 1 600px",minWidth:0}}>
                <div style={{fontSize:13,fontWeight:700,marginBottom:8,paddingBottom:6,borderBottom:`2px solid ${borderColor}`,color:C.dark}}>{label} ({list.length})</div>
                <div style={s.tWrap}>
                  <table style={{...s.table,fontSize:12,tableLayout:"fixed",minWidth:LIST_MIN_W}}>
                    <colgroup>{LIST_COLS.map((h,i)=><col key={h+i} style={LIST_COL_W[h]?{width:LIST_COL_W[h]}:undefined}/>)}</colgroup>
                    <thead><tr>{LIST_COLS.map((h,i)=>{
                      const dayIdx=["Mon","Tue","Wed","Thu","Fri"].indexOf(h);
                      const isTodayCol=dayIdx===di;
                      return <th key={h+i} style={{...s.th,whiteSpace:"nowrap",background:isTodayCol?"#f0f9ff":"transparent",color:isTodayCol?C.brand:"inherit"}}>{h}</th>;
                    })}</tr></thead>
          ...
          {fleetView==="board"&&(()=>{
            ...
            const renderTruckRows=(truckList,label,borderColor)=>(
              <div style={{flex:"1 1 400px",minWidth:0}}>
                <div style={{fontSize:13,fontWeight:700,marginBottom:8,paddingBottom:6,borderBottom:`2px solid ${borderColor}`}}>{label} ({truckList.length})</div>
                <div style={{overflowX:"auto"}}>
                  <table style={{...s.table,fontSize:11,width:"100%"}}>
                    <thead><tr>
                      <th style={{...s.th,minWidth:60,textAlign:"left"}}>Truck</th>
                      {DAYS.map((d,i)=><th key={d} style={{...s.th,textAlign:"center",background:i===di?"#f0f9ff":"transparent",color:i===di?C.brand:"inherit"}}>{d}</th>)}
                    </tr></thead>
                    <tbody>
                      {truckList.map(t=>{
                        const st2=gTS(t.id,dk);
            ...
            return <div>
              ...
              <div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
                ...
                {renderTruckRows(tractors,"🚛 Tractors",C.accent)}
              </div>
            </div>;
          })()}
          ...
          {retiredTrucks.length>0&&<div style={{marginTop:20}}>
            ...
            {retiredTrucks.map(rt=>{
              ...
              return <div key={rt.id} style={{padding:"10px 12px",background:"#fafbfc",border:"1px solid #e2e8f0",borderRadius:6,marginBottom:6,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <div style={{display:"flex",gap:6}}>
                  <button onClick={async()=>{
                    if(!await uiConfirm(`Restore truck ${rt.id} to active fleet?`))return;
                    // Preserve the original record (type/tr/ax/year) — don't rebuild it as a default box truck.
                    const{retiredDate,reason,...orig}=rt;
                    saveTrucks([...trucks,{id:rt.id,mk:rt.mk||"FRTLN",tr:rt.tr||"A",ax:rt.ax||"Single",type:rt.type||"straight",...orig}]);
                  ...
                  <button onClick={async()=>{
                    if(!await uiConfirm(`Permanently delete truck ${rt.id} from retired list? Cost history will be preserved but this truck will no longer appear.`))return;
                    saveRetiredTrucks(retiredTrucks.filter(x=>x.id!==rt.id));
        ...
        {tab==="maint"&&viewMode==="classic"&&(()=>{
          ...
          return <div>
          {/* v2.10.21: Log New Repair moved to top, collapsed by default so it doesn't
              push Open Repairs below the fold. Expand it when you actually need to
              log a new one manually. */}
          <details style={{marginBottom:16,padding:"10px 14px",background:"#f8fafc",borderRadius:8,border:"1px solid #e2e8f0"}}>
            <summary style={{fontSize:13,fontWeight:700,color:C.brand,cursor:"pointer",userSelect:"none",listStyle:"none"}}>
              ➕ Log New Repair
            </summary>
            <div style={{...s.addForm,marginTop:10}}>
              {(()=>{
                ...
                const label=t=>`#${t.id} — ${truckDesc(t)}`;
                return(<>
                  {pickedTruck
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8,marginBottom:4}}>
            <div style={s.secT}>Open Repairs ({openCount})</div>
            <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
              {/* v2.16.10: who's using this device — stamped on notes added here */}
              <button onClick={askDeviceUser}
                title={deviceUser?`Notes you add are signed "${deviceUser}" — click to change`:"Set who's using this device so your notes are signed"}
                style={{padding:"6px 12px",fontSize:11,fontWeight:700,background:"#fff",color:deviceUser?C.dark:C.accent,border:`1px solid ${deviceUser?"#d1d9e0":C.accent}`,borderRadius:14,cursor:"pointer",whiteSpace:"nowrap"}}>
                👤 {deviceUser||"Set your name"}
              </button>
              {dupToClose.length>0&&<button onClick={mergeDuplicateOpenRepairs} style={{padding:"8px 14px",fontSize:12,fontWeight:700,background:C.accent,color:"#fff",border:"none",borderRadius:6,cursor:"pointer",whiteSpace:"nowrap"}} title="Merge duplicate open tickets created by pre-v2.10.18 daily-OOS behavior">
                🧹 Clean Up {dupToClose.length} Duplicate{dupToClose.length>1?"s":""}
              </button>}
              <button onClick={printOpenRepairs} disabled={openCount===0} style={{padding:"8px 14px",fontSize:12,fontWeight:700,background:openCount===0?"#e2e8f0":C.brand,color:openCount===0?"#94a3b8":"#fff",border:"none",borderRadius:6,cursor:openCount===0?"default":"pointer",whiteSpace:"nowrap"}} title="Print the open repairs (or save as PDF)">
                🖨️ Print
              </button>
            </div>
          </div>
          {repairs.filter(r=>r.status==="open").length===0&&<div style={s.emptyMsg}>No open repairs. All trucks operational.</div>}
          {/* v2.10.20: split open repairs into Box Trucks (left) and Tractors (right).
              Each card has inline-editable reason dropdown + notes input.
              On mobile (<900px), columns stack vertically via flex-wrap. */}
          {(()=>{
            const openRepairs=repairs.filter(r=>r.status==="open");
            const byType=(typ)=>openRepairs.filter(r=>{
              const t=trucks.find(x=>x.id===r.truckId);
              return t?t.type===typ:typ==="straight"; // fallback: unknown truck → box
            });
            // v2.10.25: per-card note input refs (avoids per-card useState inside IIFE)
            const noteRefs={};
            const itemNoteRefs={};
            // v2.18.0: one repair order can cover several jobs. Each notesLog entry is a
            // work item — it carries its own `notes` sub-log and its own done flag, both
            // optional, so every ticket written before this reads back as a single open
            // item with no sub-notes and nothing needs migrating.
            const logOf=r=>r.notesLog||(r.notes?[{ts:r.dateIn||new Date().toISOString(),text:r.notes}]:[]);
            const writeLog=(r,next)=>updateRepair(r.id,{notesLog:next,notes:""});
            const patchItem=(r,idx,patch)=>writeLog(r,logOf(r).map((e,i)=>i===idx?{...e,...patch}:e));
            const addNoteToRepair=async(id)=>{
              const inp=noteRefs[id];
              if(!inp)return;
              const text=(inp.value||"").trim();
              if(!text)return;
              const by=await ensureDeviceUser();
              const entry={ts:new Date().toISOString(),text,...(by?{by}:{})};
              const existing=repairs.find(r=>r.id===id);
              if(!existing)return;
              writeLog(existing,[...logOf(existing),entry]);
              inp.value="";
            };
            const toggleItemDone=async(r,idx)=>{
              const cur=logOf(r)[idx];
              if(!cur)return;
              if(cur.done){patchItem(r,idx,{done:false,doneAt:null,doneBy:null});return;}
              const by=await ensureDeviceUser();
              patchItem(r,idx,{done:true,doneAt:new Date().toISOString(),...(by?{doneBy:by}:{})});
            };
            const addItemNote=async(r,idx)=>{
              const key=`${r.id}:${idx}`;
              const inp=itemNoteRefs[key];
              const text=(inp&&inp.value||"").trim();
              if(!text)return;
              const by=await ensureDeviceUser();
              const cur=logOf(r)[idx];
              if(!cur)return;
              patchItem(r,idx,{notes:[...(cur.notes||[]),{ts:new Date().toISOString(),text,...(by?{by}:{})}]});
              inp.value="";
            };
            const renderCard=(r)=>{
              const t=trucks.find(x=>x.id===r.truckId);
              // Build log: migrate legacy r.notes string if no notesLog yet
              const notesLog=r.notesLog||(r.notes?[{ts:r.dateIn||r.id,text:r.notes}]:[]);
              const doneItems=notesLog.filter(e=>e&&e.done).length;
              const allItemsDone=notesLog.length>0&&doneItems===notesLog.length;
              const openItems=notesLog.length-doneItems;
              return <div key={r.id} style={{...s.repairCard,marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,flexWrap:"wrap"}}>
                  <div style={{flex:"1 1 auto"}}><span style={{fontFamily:"monospace",fontWeight:700,fontSize:16,color:C.brand,cursor:"pointer",textDecoration:"underline"}} onClick={()=>setHistoryTruck(r.truckId)}>#{r.truckId}</span>{t&&<span style={{fontSize:11,color:"#6b7785",marginLeft:8}}>{truckDesc(t)} · {t.tr==="A"?"Auto":"Man"}</span>}</div>
                  {notesLog.length>0&&<span title={`${doneItems} of ${notesLog.length} item(s) done`} style={{fontSize:10,fontWeight:700,padding:"3px 7px",borderRadius:10,whiteSpace:"nowrap",background:allItemsDone?C.green+"1a":"#f1f5f9",color:allItemsDone?C.green:"#64748b",border:`1px solid ${allItemsDone?C.green+"55":"#e2e8f0"}`}}>{doneItems}/{notesLog.length} done</span>}
                  <select value={r.reason} onChange={e=>updateRepair(r.id,{reason:e.target.value})} style={{fontSize:11,fontWeight:700,padding:"3px 6px",borderRadius:4,background:C.red+"14",color:C.red,border:`1px solid ${C.red}44`,cursor:"pointer",outline:"none"}} title="Change reason">
                    {OOS_REASONS.map(opt=><option key={opt} value={opt}>{opt}</option>)}
                  </select>
                </div>

                {/* v2.18.0: work items — each its own notes and its own done state */}
                {notesLog.length>0&&<div style={{marginTop:8,background:"#f8fafc",borderRadius:6,padding:"8px 10px",display:"flex",flexDirection:"column",gap:8}}>
                  {notesLog.map((entry,idx)=>{
                    const d=new Date(entry.ts);
                    const label=isNaN(d.getTime())?"":d.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"2-digit"})+" "+d.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"});
                    const subNotes=Array.isArray(entry.notes)?entry.notes:[];
                    const noteKey=`${r.id}:${idx}`;
                    return <div key={idx} style={{display:"flex",flexDirection:"column",gap:4}}>
                      <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
                        <button
                          onClick={()=>toggleItemDone(r,idx)}
                          title={entry.done?"Reopen this item":"Mark this item done"}
                          style={{flexShrink:0,width:18,height:18,marginTop:1,padding:0,lineHeight:1,fontSize:11,fontWeight:800,borderRadius:4,cursor:"pointer",background:entry.done?C.green:"#fff",color:entry.done?"#fff":"transparent",border:`1px solid ${entry.done?C.green:"#cbd5e1"}`}}
                        >✓</button>
                        <span style={{fontSize:10,color:"#94a3b8",whiteSpace:"nowrap",marginTop:2,minWidth:90}}>{label}</span>
                        <span style={{fontSize:12,color:entry.done?"#94a3b8":"#334155",flex:1,minWidth:0,lineHeight:1.5,whiteSpace:"pre-wrap",overflowWrap:"anywhere",textDecoration:entry.done?"line-through":"none"}}>{entry.text}</span>
                        {entry.by&&<span title={`Added by ${entry.by}`} style={{fontSize:10,color:C.brand,fontWeight:600,whiteSpace:"nowrap",marginTop:2}}>— {entry.by}</span>}
                        <button onClick={()=>{
                          const newLog=notesLog.filter((_,i)=>i!==idx);
                          updateRepair(r.id,{notesLog:newLog});
                        }} title="Delete item" style={{fontSize:10,color:"#94a3b8",background:"none",border:"none",cursor:"pointer",padding:"0 2px",lineHeight:1,flexShrink:0,marginTop:1}}>✕</button>
                      </div>
                      {entry.done&&<div style={{fontSize:10,color:C.green,fontWeight:600,marginLeft:26}}>✓ Done {entry.doneAt?dateStr(entry.doneAt):""}{entry.doneBy?` by ${entry.doneBy}`:""}</div>}
                      {subNotes.length>0&&<div style={{marginLeft:26,display:"flex",flexDirection:"column",gap:3,borderLeft:"2px solid #e2e8f0",paddingLeft:8}}>
                        {subNotes.map((n,ni)=>{
                          const nd=new Date(n.ts);
                          const nlabel=isNaN(nd.getTime())?"":nd.toLocaleDateString("en-US",{month:"short",day:"numeric"})+" "+nd.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"});
                          /* Stacked, not a row: time | text | author | ✕ left the body a
                             sliver wide once indented under an item, so a real write-up
                             came out one word per line. Meta on top, body full width. */
                          return <div key={ni}>
                            <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
                              <span style={{fontSize:9,color:"#b6c0cc",whiteSpace:"nowrap"}}>{nlabel}</span>
                              {n.by&&<span style={{fontSize:9,color:C.brand,fontWeight:600,whiteSpace:"nowrap"}}>— {n.by}</span>}
                              {/* v2.19.1: this ✕ sits a couple of pixels from ✎ on a phone and
                                  used to delete a mechanic's write-up on the first tap, with no
                                  undo. Confirm it. */}
                              <button onClick={async()=>{if(await uiConfirm("Delete this note?"))patchItem(r,idx,{notes:subNotes.filter((_,i)=>i!==ni)});}} title="Delete note" style={{marginLeft:"auto",fontSize:10,color:"#cbd5e1",background:"none",border:"none",cursor:"pointer",padding:"2px 4px",lineHeight:1,flexShrink:0}}>✕</button>
                            </div>
                            <div style={{fontSize:11,color:"#475569",lineHeight:1.5,whiteSpace:"pre-wrap",overflowWrap:"anywhere"}}>{n.text}</div>
                          </div>;
                        })}
                      </div>}
                      {openItemNote===noteKey
                        ?<div style={{display:"flex",gap:6,marginLeft:26,alignItems:"center"}}>
                          <input
                            autoFocus
                            ref={el=>{itemNoteRefs[noteKey]=el;}}
                            placeholder={`Note on "${String(entry.text).slice(0,28)}${String(entry.text).length>28?"…":""}"`}
                            onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();addItemNote(r,idx);}else if(e.key==="Escape"){setOpenItemNote(null);}}}
                            style={{flex:1,fontSize:11,color:"#334155",padding:"5px 8px",border:"1px solid #e2e8f0",borderRadius:5,outline:"none",background:"#fff",boxSizing:"border-box"}}
                          />
                          <button onClick={()=>addItemNote(r,idx)} style={{padding:"5px 10px",fontSize:11,fontWeight:700,background:C.brand,color:"#fff",border:"none",borderRadius:5,cursor:"pointer",flexShrink:0}}>Add</button>
                          <button onClick={()=>setOpenItemNote(null)} style={{padding:"5px 8px",fontSize:11,color:"#94a3b8",background:"none",border:"none",cursor:"pointer",flexShrink:0}}>Cancel</button>
                        </div>
                        :<button onClick={()=>setOpenItemNote(noteKey)} style={{alignSelf:"flex-start",marginLeft:26,fontSize:10,fontWeight:600,color:C.brand,background:"none",border:"none",cursor:"pointer",padding:"1px 0"}}>＋ Note</button>}
                    </div>;
                  })}
                </div>}

                {/* Add another work item to this repair order */}
                <div style={{display:"flex",gap:6,marginTop:8,alignItems:"center"}}>
                  <input
                    ref={el=>{noteRefs[r.id]=el;}}
                    placeholder={notesLog.length>0?"Add another repair item...":"Add a repair item..."}
                    onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();addNoteToRepair(r.id);}}}
                    style={{flex:1,fontSize:12,color:"#334155",padding:"7px 10px",border:"1px solid #e2e8f0",borderRadius:6,outline:"none",background:"#fff",boxSizing:"border-box"}}
                  />
                  <button
                    onClick={()=>addNoteToRepair(r.id)}
                    style={{padding:"7px 14px",fontSize:12,fontWeight:700,background:C.brand,color:"#fff",border:"none",borderRadius:6,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}
                  >Add</button>
                </div>
                ...
                <div style={{marginTop:8,display:"flex",gap:8}}>
                  {/* v2.18.0: closing the order closes every item under it, so say what's
                      still open rather than letting a half-finished ticket disappear. */}
                  <button onClick={async()=>{
                    if(openItems>0&&!await uiConfirm(`${openItems} of ${notesLog.length} item(s) on this repair ${openItems===1?"is":"are"} still open:\n\n${notesLog.filter(e=>e&&!e.done).map(e=>`• ${e.text}`).join("\n")}\n\nClose the whole repair order anyway?`))return;
                    const cost=await uiPrompt("Final cost (or leave blank):",r.cost||"");if(cost===null)return;const clean=cost.replace(/[$,\s]/g,"");const n=clean===""?r.cost:Number(clean);if(clean!==""&&!Number.isFinite(n)){toast(`"${cost}" isn't a valid number — enter digits only (e.g. 1200.50).`);return;}await closeRepair(r.id,n);
                  }} style={s.closeRepBtn}>Repair Completed</button>
                </div>
              </div>;
            };
            const boxRepairs=byType("straight");
            const tractorRepairs=byType("tractor");
            return <div style={{display:"flex",gap:16,flexWrap:"wrap",marginTop:8}}>
              <div style={{flex:"1 1 380px",minWidth:0}}>
                <div style={{fontSize:13,fontWeight:700,color:"#1e293b",marginBottom:8,paddingBottom:6,borderBottom:`2px solid ${C.brand}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span>📦 Box Trucks ({boxRepairs.length})</span>
                </div>
                {boxRepairs.length===0?<div style={{fontSize:12,color:"#94a3b8",fontStyle:"italic",padding:"8px 0"}}>No box trucks down.</div>:boxRepairs.map(renderCard)}
              </div>
              <div style={{flex:"1 1 380px",minWidth:0}}>
                <div style={{fontSize:13,fontWeight:700,color:"#1e293b",marginBottom:8,paddingBottom:6,borderBottom:`2px solid ${C.accent}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span>🚛 Tractors ({tractorRepairs.length})</span>
                </div>
                {tractorRepairs.length===0?<div style={{fontSize:12,color:"#94a3b8",fontStyle:"italic",padding:"8px 0"}}>No tractors down.</div>:tractorRepairs.map(renderCard)}
              </div>
            </div>;
          })()}
          ...
          {repairs.filter(r=>!search||String(r.truckId||"").includes(search)).sort((a,b)=>b.id-a.id).map(r=>{
            const t=trucks.find(x=>x.id===r.truckId);
            return <div key={r.id} style={{...s.repairCard,opacity:r.status==="closed"?0.7:1,borderLeftColor:r.status==="open"?C.red:C.green}}>
              ...
              {r.notes&&!r.notesLog&&<div style={{fontSize:11,color:"#6b7785",marginTop:4}}>{r.notes}</div>}
              {r.notesLog&&r.notesLog.length>0&&<div style={{fontSize:11,color:"#6b7785",marginTop:4,display:"flex",flexDirection:"column",gap:2}}>
                {r.notesLog.map((entry,idx)=>{
                  const d=new Date(entry.ts);
                  const label=isNaN(d.getTime())?"":d.toLocaleDateString("en-US",{month:"short",day:"numeric"})+" ";
                  return <span key={idx}><span style={{color:"#94a3b8",fontSize:10}}>{label}</span>{entry.text}{entry.by?<span style={{color:C.brand,fontSize:10,fontWeight:600}}> — {entry.by}</span>:null}</span>;
                })}
              </div>}
        ...
        {/* ══ CORES relocated under Costs → Parts Inventory Tracking in v2.10.16 ══ */}
              ...
              {fl.map(d=>{const tv=asgn[`${d.name}-${dk}`]||"";
                return <div key={d.name} style={s.dRow}><div><div style={{...s.dNm,cursor:"pointer",color:C.brand,textDecoration:"underline",textDecorationColor:C.brand+"44"}} onClick={()=>setDriverReport(d.name)}>{drvIcon(d.role)} {d.name}</div><div style={s.dRl}>{d.role}{d.phone?<> · <a href={`tel:${d.phone}`} style={{color:C.brand,textDecoration:"none"}}>📞 {d.phone}</a></>:""}</div></div>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <button style={s.xBtn} title={`Retire ${d.name}`} onClick={()=>removeDriver(d.name)}>×</button>
                  </div></div>;
              })}</div>;
          })}
          {driverView==="cards"&&(()=>{
              ...
              return <div key={g.key}><div style={{...s.dCat,color:g.color}}>{g.label} ({fl.length})</div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(230px, 1fr))",gap:10,marginTop:8}}>
                  {fl.map(d=>{
                    ...
                    const scheduled=(a.worked+a.calledOut+a.noShow);
        ...
        {/* ══ SETTINGS ══ */}
        {/* v2.29.0: Settings — starting with a working copy of the mechanic portal.
            Nobody in the office ever used the shop portal, so nobody saw what was wrong
            with it and it never got better. This embeds the real portal in test mode:
            it reads the live trucks and repairs, every button works, and NOTHING is
            written (see TEST MODE in mechanic/index.html — reads pass through, writes stay
            in the frame). The frame mounts only while this tab is open, so it adds no
            second Firestore connection to anyone's cold start. */}
        ...
        {tab==="attend"&&(()=>{
          ...
          const sortedDrivers=(()=>{
            ...
            const withStats=drivers.map(d=>{
              const a=attendance[d.name]||{worked:0,off:0,vac:0,calledOut:0,noShow:0,unassigned:0,totalDays:0};
              const scheduledDays=(a.worked+a.calledOut+a.noShow);
          ...
          return <div>
          {/* v2.10.21: date-window filter for attendance stats */}
          {(()=>{
            const totalWeeks=Object.keys(attendWeeks).length;
            const effectiveWeeks=attendWeeksBack==="all"?totalWeeks:Math.min(Number(attendWeeksBack),totalWeeks);
            return <>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8,marginBottom:4}}>
                <div style={s.secT}>Driver Attendance — Last {effectiveWeeks} Week{effectiveWeeks===1?"":"s"}</div>
                <div style={{display:"flex",gap:6,alignItems:"center"}}>
                  <label style={{fontSize:11,fontWeight:600,color:"#6b7785"}}>Window:</label>
                  <select
                    value={attendWeeksBack}
                    onChange={e=>{
                      const v=e.target.value;
                      setAttendWeeksBack(v==="all"?"all":Number(v));
                    }}
                    style={{padding:"6px 8px",fontSize:12,fontWeight:600,border:"1px solid #cbd5e1",borderRadius:6,background:"#fff",color:"#1e293b",outline:"none",cursor:"pointer"}}
                  >
                    <option value={4}>4 weeks</option>
                    <option value={13}>13 weeks (1q)</option>
                    <option value={26}>26 weeks (6mo)</option>
                    <option value={52}>52 weeks (1y)</option>
                    <option value={104}>104 weeks (2y)</option>
                    <option value="all">All ({totalWeeks}w)</option>
                  </select>
                </div>
              </div>
          ...
          <div style={s.tWrap}><table style={{...s.table,fontSize:11}}><thead><tr>
            ...
            {sortedDrivers?sortedDrivers.map(d=>{
              const a=d.a;const rate=d.rate;const scheduledDays=a.worked+a.calledOut+a.noShow;
              // v2.10.23: row highlight = rate < 70 ONLY. Previously also flagged
              // calledOut>=3 or noShow>=1 which caused false positives (Frye at 87%
              // showing red just because he had 4 call-outs over 67 weeks). The
              // individual metric cells still turn red so the numbers are still
              // visible as warnings — the row background is reserved for drivers
              // whose overall rate is genuinely below the threshold.
              const isBad=scheduledDays>0&&rate<70;
              return <tr key={d.name} style={{...s.tr,background:isBad?"#fef2f2":"transparent"}}>
                ...
                <td style={{...s.td,fontWeight:700,color:scheduledDays>0?(rate>=90?C.green:rate>=70?C.accent:C.red):"#94a3b8",whiteSpace:"nowrap"}} title={`${a.worked} worked of ${scheduledDays} scheduled days`}>{scheduledDays>0?`${rate}%`:"—"}{scheduledDays>0&&<span style={{fontSize:9,fontWeight:400,color:"#94a3b8",marginLeft:3}}>({a.worked}/{scheduledDays})</span>}</td>
              </tr>;
            }):RG.map(g=>{
              const gd=drivers.filter(g.f);if(!gd.length)return null;
              return[
                ...
                ...gd.map(d=>{
                  ...
                  const scheduledDays=(a.worked+a.calledOut+a.noShow);
                  const rate=scheduledDays>0?Math.round((a.worked/scheduledDays)*100):0;
                  const isBad=scheduledDays>0&&rate<70; // v2.10.23: same fix as sorted path above
                  return <tr key={d.name} style={{...s.tr,background:isBad?"#fef2f2":"transparent"}}>
                    <td style={{...s.td,fontWeight:600,color:C.brand,cursor:"pointer",textDecoration:"underline",textDecorationColor:C.brand+"44"}} onClick={()=>setDriverReport(d.name)}>{d.name}</td>
                    <td style={{...s.td,color:C.green,fontWeight:700}}>{a.worked}</td>
                    <td style={s.td}>{a.off}</td><td style={s.td}>{a.vac}</td>
        ...
        {/* v2.27.0: the ledger arrives after the app opens, so this tab has a state the
            others do not — "not here yet", which must never be mistaken for "no
            invoices". Cost writes are refused until it lands (see saveCosts). */}
        {tab==="costs"&&costsState!=="ready"&&
          <div style={{marginBottom:14,padding:"10px 12px",borderRadius:8,fontSize:13,display:"flex",alignItems:"center",gap:10,
            background:costsState==="failed"?"#fef2f2":"#eff6ff",border:`1px solid ${costsState==="failed"?"#fecaca":"#bfdbfe"}`,
            color:costsState==="failed"?"#991b1b":"#1e40af"}}>
            {costsState==="failed"
              ? <><span>⚠️</span><span>The cost ledger didn&rsquo;t finish loading, so what&rsquo;s below is incomplete. Nothing here can be edited until it does.</span>
                  <button onClick={()=>setLoadAttempt(a=>a+1)} style={{marginLeft:"auto",background:"#991b1b",color:"#fff",border:0,borderRadius:5,padding:"6px 12px",fontSize:12,fontWeight:700,cursor:"pointer"}}>Retry</button></>
              : <><div style={{width:14,height:14,border:"2px solid #bfdbfe",borderTop:"2px solid #1e40af",borderRadius:"50%",animation:"spin 0.8s linear infinite",flexShrink:0}}/>
                  <span>Loading the cost ledger&hellip; invoices will fill in here shortly.</span></>}
          </div>}
        {tab==="costs"&&viewMode==="classic"&&<div>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:16,background:"#fff",border:"1px solid #e2e8f0",borderRadius:8,padding:"10px 12px"}}>
            <span style={{fontSize:12,fontWeight:700,color:C.dark}}>🗄️ Ledger backup</span>
            <button onClick={()=>exportCostsCSV(costEntries)} disabled={!costEntries.length} style={{fontSize:11,fontWeight:700,padding:"6px 12px",background:costEntries.length?C.green:"#e2e8f0",color:costEntries.length?"#fff":"#94a3b8",border:"none",borderRadius:6,cursor:costEntries.length?"pointer":"default",whiteSpace:"nowrap"}}>📥 Export CSV ({costEntries.length})</button>
            <span style={{width:1,height:20,background:"#e2e8f0"}}/>
            <span style={{fontSize:12,color:"#6b7785"}}>Archive invoices before</span>
            <select value={archiveYear} onChange={e=>setArchiveYear(e.target.value)} style={{fontSize:12,padding:"5px 8px",borderRadius:6,border:"1px solid #d1d9e0",cursor:"pointer"}}>
              {(()=>{const cy=new Date().getFullYear();return[cy,cy-1,cy-2,cy-3].map(y=><option key={y} value={y}>{y}</option>);})()}
            </select>
            <button onClick={()=>archiveOldCosts(archiveYear)} style={{fontSize:11,fontWeight:700,padding:"6px 12px",background:C.accent,color:"#fff",border:"none",borderRadius:6,cursor:"pointer",whiteSpace:"nowrap"}} title="Export older invoices to CSV and move them out of the active ledger (kept in the database, not deleted)">📦 Archive &amp; export</button>
            <span style={{width:1,height:20,background:"#e2e8f0"}}/>
            {/* v2.19.0: repairs what the two import bugs left behind — see
            ...
            <button onClick={async()=>{
              ...
              const a=fuel.audit;
              // v2.24.7: and give any row still sharing an id with another a fresh one.
              // Only safe here, on an action that saves — see dedupById.
              const heal=healCollidedIds(fuel.entries);
              if(r.duplicatesRemoved===0&&r.splitDocs===0&&a.count===0&&heal.healed===0){toast("Ledger looks clean — nothing to fix.");return;}
              const worst=(()=>{
                const b={},after={};
              ...
              if(a.count)lines.push(`• ${a.count} fuel row${a.count===1?"":"s"} ($${Math.round(a.value).toLocaleString()}) too big for one truck to have taken on — the document was never itemized by truck, so this moves to Inventory instead of staying on a unit that did not burn it`);
              if(heal.healed)lines.push(`• ${heal.healed} row${heal.healed===1?"":"s"} sharing an invoice id with another — given their own id, so deleting or reassigning one stops touching the other`);
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              ...
              <div style={{display:"flex",gap:6}}>
                <button onClick={async()=>{
                  ...
                  if(dupes.length===0){toast("No duplicate vendors found.");return;}
                  ...
                  saveCosts(merged);
                  toast(`Merged ${dupes.length} vendor group${dupes.length>1?"s":""}.`);
          ...
          <div style={{...s.addForm,marginBottom:16}}>
            {!gmailConn?<div>
              ...
              {/* v2.10.3: primary one-click action; v2.10.5 added custom date */}
              {(()=>{
                // Effective daysBack resolver — converts "custom" + since-date into days
                const resolveDaysBack=()=>{
                  if(autoScanDaysBack!=="custom")return autoScanDaysBack;
                  const since=new Date(autoScanCustomSince);
                  if(isNaN(since.getTime()))return 30;
                  const diffMs=Date.now()-since.getTime();
                  return Math.max(1,Math.ceil(diffMs/86400000));
                };
                const effectiveDaysBack=resolveDaysBack();
                // v2.10.13: compute beforeDate for Gmail query when custom until is set
                // and is not today. Default ("today") means no upper bound → pass null.
                const computeBeforeDate=()=>{
                  if(autoScanDaysBack!=="custom")return null;
                  const todayStr=todayYMD();
                  if(!autoScanCustomUntil||autoScanCustomUntil>=todayStr)return null;
                  // Gmail query syntax: before:YYYY/MM/DD (exclusive). Add one day so the
                  // selected date is inclusive.
                  const u=new Date(autoScanCustomUntil);
                  u.setDate(u.getDate()+1);
                  return `${u.getFullYear()}/${u.getMonth()+1}/${u.getDate()}`;
                };
                const effectiveBeforeDate=computeBeforeDate();
                // Button label — compact for mobile
                const dateLabel=(()=>{
                  if(autoScanDaysBack==="custom"){
                    const s=new Date(autoScanCustomSince);
                    const u=new Date(autoScanCustomUntil);
                    const fmt=(d)=>isNaN(d.getTime())?"?":d.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"2-digit"});
                    if(effectiveBeforeDate){
                      return `${fmt(s)} → ${fmt(u)}`;
                    }
                    return `since ${fmt(s)}`;
                  }
                  if(autoScanDaysBack>=365&&autoScanDaysBack%365===0){
                    const y=autoScanDaysBack/365;
                    return `last ${y}y`;
                  }
                  return `last ${autoScanDaysBack}d`;
                })();
                return <>
                  {/* v2.10.48: Server-side auto-sync. Runs entirely on Netlify
                      (nightly + manual) without needing this tab open. */}
                  <div style={{marginBottom:10,padding:12,background:"#f0f9ff",borderRadius:8,border:"1px solid #bfdbfe"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:6}}>
                      <div style={{fontSize:13,fontWeight:700,color:"#1e3a8a"}}>🤖 Auto-Sync (server-side)</div>
                      <button
                        onClick={()=>triggerServerSync(7)}
                    ...
                    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:6}}>
                      ...
                      {backlog&&<span style={{fontSize:11,color:backlog.running?C.accent:"#475569"}}>
                        {backlog.running?`Pass ${backlog.pass}${backlog.remaining!=null?` · ${backlog.remaining} left`:""} · imported ${backlog.imported}${backlog.queued?` · ${backlog.queued} to review`:""}…`
                          :`${backlog.note||"Done"} — imported ${backlog.imported}${backlog.queued?`, ${backlog.queued} queued for review`:""}`}
                      </span>}
                    </div>
                    <div style={{fontSize:11,color:"#475569",lineHeight:1.5}}>
                      {syncStatus?(()=>{
                        const last=syncStatus.lastRun?new Date(syncStatus.lastRun).toLocaleString():"never";
                        return <>
                          <div>Last run: <span style={{fontWeight:600}}>{last}</span></div>
                          {syncStatus.message&&<div style={{marginTop:2,color:syncStatus.message.startsWith("✓")?C.green:syncStatus.message.startsWith("✗")?C.red:"#475569"}}>{syncStatus.message}</div>}
                          {/* v2.17.0: the queue is durable server-side — surface its depth so a backlog is visible even between runs */}
                          {typeof syncStatus.remaining==="number"&&syncStatus.remaining>0&&!backlog?.running&&<div style={{marginTop:2,color:C.accent,fontWeight:600}}>📬 {syncStatus.remaining} attachment(s) waiting in the queue — each ~3-hourly run drains a batch, or Catch Up Backlog clears it now.</div>}
                          {Array.isArray(syncStatus.errors)&&syncStatus.errors.length>0&&<div style={{marginTop:2,color:"#92400e"}}>{syncStatus.errors.slice(0,3).map((e,i)=><div key={i}>· {e}</div>)}</div>}
                          {!syncStatus.tokenStored&&gmailConn&&<div style={{marginTop:2,color:C.accent}}>⚠ Server doesn't have Gmail token yet — will save on next manual sync.</div>}
                          {!gmailConn&&<div style={{marginTop:2,color:C.accent}}>⚠ Gmail not connected — scroll down to connect.</div>}
                        </>;
                      })():<div>Loading status…</div>}
                      <div style={{marginTop:4,fontSize:10,color:"#94a3b8"}}>Runs automatically every ~3 hours and keeps going until caught up · imports confident invoices on its own, queues only the uncertain ones for review below.</div>
                    </div>
                  </div>
                  <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",marginBottom:8}}>
                    <button
                      onClick={()=>scanAllNewInvoices(effectiveDaysBack,effectiveBeforeDate)}
                      disabled={autoScanPhase!==null||gmailLoading||scanning}
                      style={{
                        flex:"1 1 240px",padding:"12px 16px",fontSize:14,fontWeight:800,
                        background:autoScanPhase?C.accent:C.brand,color:"#fff",
                        border:"none",borderRadius:8,cursor:autoScanPhase?"wait":"pointer",
                        opacity:(autoScanPhase||gmailLoading||scanning)?0.7:1,
                        boxShadow:"0 2px 4px rgba(0,0,0,0.08)",
                      }}
                    >
                      {autoScanPhase==="searching"?"🔍 Searching Gmail..."
                        :autoScanPhase==="queueing"?`📥 Queueing ${autoScanProgress.done}/${autoScanProgress.total}...`
                        :scanning?"🤖 AI scanning queue..."
                        :`🚀 Scan All New Invoices (${dateLabel})`}
                    </button>
                    <select
                      value={autoScanDaysBack}
                      onChange={e=>{
                        const v=e.target.value;
                        setAutoScanDaysBack(v==="custom"?"custom":Number(v));
                      }}
                      disabled={autoScanPhase!==null||scanning}
                      style={{padding:"10px 8px",fontSize:12,fontWeight:600,border:"1px solid #cbd5e1",borderRadius:6,background:"#fff",color:"#1e293b",outline:"none"}}
                    >
                  ...
                  {autoScanDaysBack==="custom"&&<div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:8,padding:"8px 10px",background:"#f8fafc",borderRadius:6,border:"1px solid #e2e8f0"}}>
                    <div style={{display:"flex",gap:4,alignItems:"center",flex:"1 1 150px"}}>
                      <label style={{fontSize:11,fontWeight:600,color:"#6b7785",minWidth:40}}>From:</label>
                      <input
                        type="date"
                        value={autoScanCustomSince}
                        max={autoScanCustomUntil||todayYMD()}
                        onChange={e=>setAutoScanCustomSince(e.target.value)}
                        disabled={autoScanPhase!==null||scanning}
                        style={{padding:"6px 8px",fontSize:12,fontWeight:600,border:"1px solid #cbd5e1",borderRadius:4,background:"#fff",color:"#1e293b",outline:"none",flex:1}}
                      />
                    </div>
                    <div style={{display:"flex",gap:4,alignItems:"center",flex:"1 1 150px"}}>
                      <label style={{fontSize:11,fontWeight:600,color:"#6b7785",minWidth:40}}>To:</label>
                      <input
                        type="date"
                        value={autoScanCustomUntil}
                        min={autoScanCustomSince}
                        max={todayYMD()}
                        onChange={e=>setAutoScanCustomUntil(e.target.value)}
                        disabled={autoScanPhase!==null||scanning}
                        style={{padding:"6px 8px",fontSize:12,fontWeight:600,border:"1px solid #cbd5e1",borderRadius:4,background:"#fff",color:"#1e293b",outline:"none",flex:1}}
                      />
                    </div>
                    <span style={{fontSize:10,color:"#94a3b8",width:"100%",textAlign:"center"}}>
                      {effectiveBeforeDate
                        ?`range: ${Math.ceil((new Date(autoScanCustomUntil)-new Date(autoScanCustomSince))/86400000)+1} days (inclusive)`
                        :`${effectiveDaysBack} days back (to today)`}
                    </span>
                  </div>}
                  <div style={{fontSize:10,color:"#94a3b8",marginBottom:10}}>
                    Fetches all {knownVendors.length} vendors, skips already-imported emails, auto-scans with AI. One click.
                  </div>

                  {/* Advanced: per-vendor search (for finding a specific old invoice) */}
                  <details style={{marginTop:4}}>
                    <summary style={{fontSize:11,fontWeight:600,color:"#6b7785",cursor:"pointer",padding:"4px 0"}}>Advanced — search one vendor at a time</summary>
                    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:6}}>
                      {knownVendors.map(v=><button key={v.name} onClick={()=>fetchGmailInvoices(v.name.toLowerCase(),effectiveDaysBack,effectiveBeforeDate)} disabled={gmailLoading} style={{...s.saveBtn,opacity:gmailLoading?0.5:1,fontSize:11,padding:"5px 10px",background:"#fff",color:C.brand,border:`1px solid ${C.brand}`,fontWeight:600}}>{gmailLoading?"Searching...":"📧 "+v.name}</button>)}
                    </div>
                  </details>
                </>;
              })()}
          ...
          <div style={{...s.addForm,marginBottom:16}}>
            ...
            {scanQueue.length>0&&(()=>{
              ...
              <div style={{display:"flex",gap:8,marginTop:10,marginBottom:8,flexWrap:"wrap",position:"sticky",top:0,zIndex:2,background:"#fff",paddingBottom:4}}>
                ...
                <button onClick={async()=>{if(await uiConfirm("Clear the entire scan queue? This removes all queued and parsed items that haven't been saved yet."))setScanQueue([]);}} style={s.canBtn}>Clear Queue</button>
              </div>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      {q.status==="parsed"&&q.parsed&&!q.parsed.error&&<div style={{fontSize:10,color:isDup?C.accent:C.green,marginTop:2}}>
                        {isDup?"⚠ DUPLICATE — ":""}{q.parsed.vendor} · ${parseMoney(q.parsed.total).toLocaleString()} · {q.parsed.category}{q.parsed.invoiceNum&&` · Inv #${q.parsed.invoiceNum}`}
          ...
          {(inventoryAnalytics.inv.length>0||cores.length>0)&&<div>
            <div style={{...s.secT,marginTop:20}}>📦 Parts Inventory Tracking</div>
            {inventoryAnalytics.inv.length>0&&<>
            <div style={s.statGrid}>
            ...
            {/* v2.10.16: CORES (relocated from standalone tab) */}
            {cores.length>0&&(()=>{
              ...
              return <div style={{marginTop:inventoryAnalytics.inv.length>0?24:0,paddingTop:inventoryAnalytics.inv.length>0?20:0,borderTop:inventoryAnalytics.inv.length>0?"1px solid #e2e8f0":"none"}}>
                <div style={{...s.secT,marginTop:0,marginBottom:8,fontSize:15}}>🔧 Cores</div>
                <div style={{marginBottom:12}}>
                  <div style={{fontSize:11,color:"#6b7785",textTransform:"uppercase",letterSpacing:.5,marginBottom:4}}>Cores Outstanding</div>
                  <div style={{fontSize:22,fontWeight:700,color:outstanding.length>0?C.red:C.green}}>{outstanding.length} cores · ${totalOutstanding.toFixed(2)}</div>
                ...
                <div style={{marginBottom:20}}>
                  <div style={{fontSize:13,fontWeight:700,color:C.red,display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                    <span>Outstanding ({outstanding.length})</span>
                  </div>
                  {outstanding.length===0?(
                    <div style={{padding:"12px",background:"#f0fdf4",borderRadius:8,color:C.green,fontSize:12,textAlign:"center"}}>
                            ...
                            <div style={{flex:1,minWidth:200}}>
                              ...
                              <div style={{fontSize:11,color:"#6b7785",marginTop:4}}>
                                {c.date} · {c.vendor} · Inv {c.invoiceNum
                                  ?<button onClick={e=>{e.stopPropagation();openInvoice(c,"core","Inv");}} style={{background:"none",border:"none",color:C.brand,textDecoration:"underline",fontWeight:600,cursor:"pointer",padding:0,font:"inherit"}} title={findInvoiceSourceUrl(c)?"View invoice PDF":"Click to attach PDF"}>{c.invoiceNum}{!findInvoiceSourceUrl(c)&&" 📎"}</button>
                                  :"—"}
                              </div>
                ...
                {unmatched.length>0&&<div style={{marginBottom:20}}>
                  <div style={{fontSize:13,fontWeight:700,color:C.accent,marginBottom:8}}>⚠️ Return Credits Without Matching Core ({unmatched.length})</div>
                      ...
                      <div key={c.id} style={{background:"#fffbeb",border:`1px solid ${C.accent}55`,borderRadius:8,padding:"10px 12px"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,flexWrap:"wrap"}}>
                          <div style={{flex:1,minWidth:200}}>
                            ...
                            <div style={{fontSize:11,color:"#6b7785",marginTop:2}}>{c.date} · {c.vendor} · Credit inv {(c.returnedCreditInvoice||c.invoiceNum)
                              ?<button onClick={e=>{e.stopPropagation();openInvoice(c,"core","Credit");}} style={{background:"none",border:"none",color:C.brand,textDecoration:"underline",fontWeight:600,cursor:"pointer",padding:0,font:"inherit"}} title={findInvoiceSourceUrl(c)?"View credit invoice PDF":"Click to attach PDF"}>{c.returnedCreditInvoice||c.invoiceNum}{!findInvoiceSourceUrl(c)&&" 📎"}</button>
                              :"—"}</div>
                          </div>
                          <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                {returned.length>0&&<details style={{marginTop:12}}>
                  <summary style={{fontSize:13,fontWeight:700,color:C.green,cursor:"pointer",padding:"6px 0"}}>✓ Returned ({returned.length}) — ${returned.reduce((acc,c)=>acc+(c.amount||0),0).toFixed(2)} recovered</summary>
                      ...
                      <div key={c.id} style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:6,padding:"8px 12px"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                          <div style={{flex:1,minWidth:200}}>
                            ...
                            <div style={{fontSize:10,color:"#6b7785"}}>Charged {c.date} · Returned {c.returnedDate||"—"} · Credit {c.returnedCreditInvoice
                              ?<button onClick={e=>{e.stopPropagation();openInvoice(c,"core","Credit");}} style={{background:"none",border:"none",color:C.brand,textDecoration:"underline",fontWeight:600,cursor:"pointer",padding:0,font:"inherit"}} title={findInvoiceSourceUrl(c)?"View credit invoice PDF":"Click to attach PDF"}>{c.returnedCreditInvoice}{!findInvoiceSourceUrl(c)&&" 📎"}</button>
                              :"—"}</div>
                          </div>
          ...
          {/* v2.10.45: Attach-PDF modal — for invoices with no stored file yet.
              Drop or pick a PDF/image, upload to Netlify Blobs, save fileUrl on
              the record, then auto-open the preview with the just-uploaded file. */}
          {attachInvoice&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1100,padding:16}} onClick={()=>{if(!attachStatus.startsWith("Uploading"))setAttachInvoice(null);}}>
            <div style={{background:"#fff",borderRadius:10,width:"100%",maxWidth:520,padding:20}} onClick={e=>e.stopPropagation()}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:16,fontWeight:700,color:"#1e293b"}}>Attach Invoice PDF</div>
                  <div style={{fontSize:11,color:"#6b7785",marginTop:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{attachInvoice.label}</div>
                </div>
                <button onClick={()=>setAttachInvoice(null)} disabled={attachStatus.startsWith("Uploading")} style={{padding:"4px 8px",fontSize:14,background:"#fff",color:"#1e293b",border:"1px solid #e2e8f0",borderRadius:4,cursor:attachStatus.startsWith("Uploading")?"wait":"pointer"}}>✕</button>
              </div>
              <div style={{fontSize:12,color:"#6b7785",marginBottom:10,lineHeight:1.5}}>No PDF was stored for this invoice (added manually or pre-storage). Pick a PDF or image (max 5MB) to attach. The file will be saved in the app and viewable here forever.</div>
              <label htmlFor="attach-pdf-input" style={{display:"block",border:`2px dashed ${C.brand}66`,borderRadius:8,padding:"24px 16px",textAlign:"center",cursor:"pointer",background:"#f8fafc",marginBottom:10}}
                ...
                onDrop={e=>{e.preventDefault();e.currentTarget.style.background="#f8fafc";const f=e.dataTransfer.files[0];if(f)uploadInvoiceAttachment(f);}}>
                <div style={{fontSize:24,marginBottom:6}}>📎</div>
                <div style={{fontSize:13,fontWeight:600,color:C.brand}}>Drop file here or click to pick</div>
                <div style={{fontSize:11,color:"#94a3b8",marginTop:4}}>PDF · JPG · PNG · max 5MB</div>
                <input id="attach-pdf-input" type="file" accept=".pdf,image/*" style={{display:"none"}} onChange={e=>{const f=e.target.files[0];if(f)uploadInvoiceAttachment(f);}}/>
              </label>
              {attachStatus&&<div style={{padding:"8px 12px",borderRadius:6,fontSize:12,fontFamily:"monospace",background:attachStatus.startsWith("✓")?"#f0fdf4":attachStatus.startsWith("✗")?"#fef2f2":"#f1f5f9",color:attachStatus.startsWith("✓")?C.green:attachStatus.startsWith("✗")?C.red:"#475569"}}>{attachStatus}</div>}
            </div>
          </div>}
          {/* v2.10.44: In-app PDF preview modal — invoices open here, never in a new tab */}
          {pdfPreview&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1100,padding:16}} onClick={()=>setPdfPreview(null)}>
            <div style={{background:"#fff",borderRadius:10,width:"100%",maxWidth:1100,height:"94vh",display:"flex",flexDirection:"column",overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
              <div style={{padding:"12px 16px",borderBottom:"1px solid #e2e8f0",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:14,fontWeight:700,color:"#1e293b",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{pdfPreview.label||"Invoice PDF"}</div>
                  <div style={{fontSize:10,color:"#94a3b8",marginTop:2}}>Stored in app · Click outside to close</div>
                </div>
                <div style={{display:"flex",gap:8,flexShrink:0}}>
                  <a href={pdfPreview.url} download style={{padding:"6px 12px",fontSize:12,fontWeight:600,background:"#f1f5f9",color:"#1e293b",border:"1px solid #e2e8f0",borderRadius:6,textDecoration:"none",cursor:"pointer"}}>⬇ Download</a>
                  <button onClick={()=>setPdfPreview(null)} style={{padding:"6px 12px",fontSize:12,fontWeight:600,background:"#fff",color:"#1e293b",border:"1px solid #e2e8f0",borderRadius:6,cursor:"pointer"}}>✕ Close</button>
                </div>
              </div>
              <iframe src={pdfPreview.url} style={{flex:1,width:"100%",border:"none",background:"#f1f5f9"}} title="Invoice PDF"/>
            </div>
          </div>}
            ...
            <div style={{background:"#fff",borderRadius:10,padding:0,maxWidth:600,width:"100%",maxHeight:"90vh",overflow:"auto"}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{padding:"16px 20px"}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:16,paddingTop:12,borderTop:"1px solid #e2e8f0"}}>
                  {showCostDetail.fileUrl
                    ?<button onClick={()=>setPdfPreview({url:showCostDetail.fileUrl,label:`Inv ${showCostDetail.invoiceNum||""} — ${showCostDetail.vendor||""}`})} style={{flex:1,textAlign:"center",padding:"10px",background:C.brand,color:"#fff",border:"none",borderRadius:6,fontWeight:700,fontSize:13,cursor:"pointer"}}>📄 View PDF</button>
                    :<button onClick={()=>{setAttachInvoice({record:showCostDetail,recordType:"cost",label:`Inv ${showCostDetail.invoiceNum||""} — ${showCostDetail.vendor||""}`});setAttachStatus("");}} style={{flex:1,padding:"10px",background:"#f8fafc",color:C.brand,border:`1px dashed ${C.brand}66`,borderRadius:6,textAlign:"center",fontSize:12,fontWeight:600,cursor:"pointer"}}>📎 Attach PDF</button>}
                  ...
                  <button onClick={async()=>{if(await uiConfirm("Delete this invoice?")){tombstoneGmailRef(costEntries.find(c=>c.id===showCostDetail.id));saveCosts(costEntries.filter(c=>c.id!==showCostDetail.id));setShowCostDetail(null);}}} style={{padding:"10px 14px",background:"#fff",color:C.red,border:`1px solid ${C.red}`,borderRadius:6,cursor:"pointer",fontWeight:700,fontSize:13}}>Delete</button>
                </div>
              </div>
            </div>
          </div>}
          ...
          {showQuickFuelReport&&quickFuelAnalytics&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:8}} onClick={()=>setShowQuickFuelReport(false)}>
            <div style={{background:"#fff",borderRadius:10,maxWidth:900,width:"100%",maxHeight:"92vh",overflow:"hidden",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{padding:16,overflowY:"auto",flex:1}}>
                ...
                <div style={{fontSize:11,color:"#6b7785",marginBottom:10}}>Drivers matched to each purchase via the Driver Board assignment for that week. Purchases with multiple drivers on the same truck that week are split evenly.</div>
              ...
              <div style={{padding:"12px 16px",borderTop:"1px solid #e2e8f0",display:"flex",justifyContent:"flex-end",gap:8,flexShrink:0}}>
                <button onClick={()=>{
                  ...
                  a.href=url;a.download=`quick-fuel-report-${todayYMD()}.csv`;a.click();
          ...
          {/* ═══ FUEL COMPARISON REPORT (v2.10.1) ═══ */}
          {showFuelCompareReport&&fuelComparison&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:8}} onClick={()=>setShowFuelCompareReport(false)}>
            <div style={{background:"#fff",borderRadius:10,maxWidth:1100,width:"100%",maxHeight:"94vh",overflow:"hidden",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
              {/* Header */}
              <div style={{padding:"16px 20px",borderBottom:"1px solid #e2e8f0",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0,background:C.purple,color:"#fff"}}>
                <div>
                  <div style={{fontSize:16,fontWeight:800}}>🆚 Quick Fuel vs FuelFox Analysis</div>
                  <div style={{fontSize:11,opacity:0.9,marginTop:2}}>
                    {fuelComparison.qfEntries.length} QF · {fuelComparison.ffEntries.length} FF · true cost incl. taxes + fees
                  </div>
                </div>
                <button onClick={()=>setShowFuelCompareReport(false)} style={{fontSize:20,background:"none",border:"none",color:"#fff",cursor:"pointer",padding:"0 8px",fontWeight:700}}>×</button>
              </div>

              {/* Scrollable body */}
              <div style={{padding:16,overflowY:"auto",flex:1}}>
                {/* ── Warning if FuelFox overhead not redistributed ── */}
                {fuelComparison.ffMissingOverhead>0&&<div style={{padding:12,background:"#fffbeb",border:`1px solid ${C.accent}`,borderRadius:8,marginBottom:16}}>
                  <div style={{fontSize:12,fontWeight:700,color:C.accent}}>⚠️ {fuelComparison.ffMissingOverhead} FuelFox entr{fuelComparison.ffMissingOverhead>1?"ies have":"y has"} not been redistributed</div>
                  <div style={{fontSize:11,color:"#6b7785",marginTop:4}}>These entries are excluded from the comparison because they show BASE price only (no tax/delivery fee). Run "Redistribute Overhead" in the Costs tab to include them.</div>
                </div>}

                {/* ── COMPARISON SUMMARY (top) ── */}
                <div style={{marginBottom:16}}>
                  <div style={{fontSize:13,fontWeight:700,color:"#1e293b",marginBottom:8}}>Summary</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                    {/* QF summary */}
                    <div style={{padding:12,background:C.light,borderRadius:8,border:`2px solid ${C.brand}`}}>
                      <div style={{fontSize:11,fontWeight:700,color:C.brand,textTransform:"uppercase",marginBottom:6}}>Quick Fuel</div>
                      <div style={{fontSize:20,fontWeight:800,color:C.brand}}>${fuelComparison.qfRoll.gwAvg.toFixed(4)}/gal</div>
                      <div style={{fontSize:10,color:"#6b7785",marginTop:2}}>gallons-weighted avg</div>
                      <div style={{fontSize:10,color:"#94a3b8",marginTop:6,lineHeight:1.5}}>
                        Simple avg: ${fuelComparison.qfRoll.simpleAvg.toFixed(4)}/gal<br/>
                        {fuelComparison.qfRoll.gallons.toFixed(1)} gal · ${fuelComparison.qfRoll.cost.toFixed(2)}<br/>
                        {fuelComparison.qfRoll.count} invoice{fuelComparison.qfRoll.count!==1?"s":""}
                      </div>
                    </div>
                    {/* FF summary */}
                    <div style={{padding:12,background:"#fef3c7",borderRadius:8,border:`2px solid ${C.accent}`}}>
                      <div style={{fontSize:11,fontWeight:700,color:C.accent,textTransform:"uppercase",marginBottom:6}}>FuelFox Atlanta</div>
                      <div style={{fontSize:20,fontWeight:800,color:C.accent}}>${fuelComparison.ffRoll.gwAvg.toFixed(4)}/gal</div>
                      <div style={{fontSize:10,color:"#6b7785",marginTop:2}}>gallons-weighted avg</div>
                      <div style={{fontSize:10,color:"#94a3b8",marginTop:6,lineHeight:1.5}}>
                        Simple avg: ${fuelComparison.ffRoll.simpleAvg.toFixed(4)}/gal<br/>
                        {fuelComparison.ffRoll.gallons.toFixed(1)} gal · ${fuelComparison.ffRoll.cost.toFixed(2)}<br/>
                        {fuelComparison.ffRoll.count} invoice{fuelComparison.ffRoll.count!==1?"s":""}
                      </div>
                    </div>
                  </div>
                  {/* Delta callout */}
                  {fuelComparison.qfRoll.count>0&&fuelComparison.ffRoll.count>0&&(()=>{
                    const delta=fuelComparison.ffRoll.gwAvg-fuelComparison.qfRoll.gwAvg;
                    const pct=fuelComparison.qfRoll.gwAvg>0?((delta/fuelComparison.qfRoll.gwAvg)*100):0;
                    const ffCheaper=delta<0;
                    return <div style={{marginTop:10,padding:12,background:ffCheaper?"#f0fdf4":"#fef2f2",borderRadius:8,border:`1px solid ${ffCheaper?C.green:C.red}`,textAlign:"center"}}>
                      <div style={{fontSize:12,color:"#6b7785",marginBottom:4}}>Overall difference</div>
                      <div style={{fontSize:18,fontWeight:800,color:ffCheaper?C.green:C.red}}>
                        FuelFox is {ffCheaper?"$":"+$"}{Math.abs(delta).toFixed(4)}/gal {ffCheaper?"cheaper":"more expensive"} ({pct>0?"+":""}{pct.toFixed(1)}%)
                      </div>
                      <div style={{fontSize:10,color:"#94a3b8",marginTop:4}}>
                        Over the whole period tracked. Monthly/weekly deltas below may vary with diesel market swings.
                      </div>
                    </div>;
                  })()}
                </div>
                ...
                {/* ── MONTHLY COMPARISON ── */}
                <div style={{marginBottom:16}}>
                  <div style={{fontSize:13,fontWeight:700,color:"#1e293b",marginBottom:6}}>Monthly averages (gallons-weighted)</div>
                  <div style={{overflowX:"auto",border:"1px solid #e2e8f0",borderRadius:6}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                      <thead>
                        <tr style={{background:"#f8fafc"}}>
                          <th style={{padding:"6px 8px",textAlign:"left",color:"#6b7785",fontWeight:600,fontSize:10,textTransform:"uppercase"}}>Month</th>
                          <th style={{padding:"6px 8px",textAlign:"right",color:C.brand,fontWeight:700,fontSize:10,textTransform:"uppercase"}}>QF $/gal</th>
                          <th style={{padding:"6px 8px",textAlign:"right",color:"#94a3b8",fontWeight:500,fontSize:10}}>QF gal</th>
                          <th style={{padding:"6px 8px",textAlign:"right",color:C.accent,fontWeight:700,fontSize:10,textTransform:"uppercase"}}>FF $/gal</th>
                          <th style={{padding:"6px 8px",textAlign:"right",color:"#94a3b8",fontWeight:500,fontSize:10}}>FF gal</th>
                          <th style={{padding:"6px 8px",textAlign:"right",color:"#6b7785",fontWeight:600,fontSize:10,textTransform:"uppercase"}}>Δ FF-QF</th>
                        </tr>
                      </thead>
                      <tbody>
                        {fuelComparison.monthCompare.map(row=>(
                          <tr key={row.month} style={{borderTop:"1px solid #f1f5f9"}}>
                            <td style={{padding:"5px 8px",fontWeight:600}}>{row.month}</td>
                            <td style={{padding:"5px 8px",textAlign:"right",color:C.brand,fontWeight:600}}>{row.qf?`$${row.qf.gwAvg.toFixed(4)}`:"—"}</td>
                            <td style={{padding:"5px 8px",textAlign:"right",color:"#94a3b8"}}>{row.qf?row.qf.gallons.toFixed(0):"—"}</td>
                            <td style={{padding:"5px 8px",textAlign:"right",color:C.accent,fontWeight:600}}>{row.ff?`$${row.ff.gwAvg.toFixed(4)}`:"—"}</td>
                            <td style={{padding:"5px 8px",textAlign:"right",color:"#94a3b8"}}>{row.ff?row.ff.gallons.toFixed(0):"—"}</td>
                            <td style={{padding:"5px 8px",textAlign:"right",fontWeight:700,color:row.delta==null?"#94a3b8":(row.delta<0?C.green:C.red)}}>
                ...
                {/* ── WEEKLY COMPARISON ── */}
                <div style={{marginBottom:16}}>
                  <div style={{fontSize:13,fontWeight:700,color:"#1e293b",marginBottom:6}}>Weekly averages (gallons-weighted) — latest 20</div>
                  <div style={{overflowX:"auto",border:"1px solid #e2e8f0",borderRadius:6}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                      <thead>
                        <tr style={{background:"#f8fafc"}}>
                          <th style={{padding:"6px 8px",textAlign:"left",color:"#6b7785",fontWeight:600,fontSize:10,textTransform:"uppercase"}}>Week of</th>
                          <th style={{padding:"6px 8px",textAlign:"right",color:C.brand,fontWeight:700,fontSize:10}}>QF $/gal</th>
                          <th style={{padding:"6px 8px",textAlign:"right",color:"#94a3b8",fontWeight:500,fontSize:10}}>QF gal</th>
                          <th style={{padding:"6px 8px",textAlign:"right",color:C.accent,fontWeight:700,fontSize:10}}>FF $/gal</th>
                          <th style={{padding:"6px 8px",textAlign:"right",color:"#94a3b8",fontWeight:500,fontSize:10}}>FF gal</th>
                          <th style={{padding:"6px 8px",textAlign:"right",color:"#6b7785",fontWeight:600,fontSize:10,textTransform:"uppercase"}}>Δ</th>
                        </tr>
                      </thead>
                      <tbody>
                        {fuelComparison.weekCompare.slice(0,20).map(row=>(
                          <tr key={row.week} style={{borderTop:"1px solid #f1f5f9"}}>
                            <td style={{padding:"5px 8px",fontWeight:600}}>{row.week}</td>
                            <td style={{padding:"5px 8px",textAlign:"right",color:C.brand,fontWeight:600}}>{row.qf?`$${row.qf.gwAvg.toFixed(4)}`:"—"}</td>
                            <td style={{padding:"5px 8px",textAlign:"right",color:"#94a3b8"}}>{row.qf?row.qf.gallons.toFixed(0):"—"}</td>
                            <td style={{padding:"5px 8px",textAlign:"right",color:C.accent,fontWeight:600}}>{row.ff?`$${row.ff.gwAvg.toFixed(4)}`:"—"}</td>
                            <td style={{padding:"5px 8px",textAlign:"right",color:"#94a3b8"}}>{row.ff?row.ff.gallons.toFixed(0):"—"}</td>
                  ...
                  {fuelComparison.weekCompare.length>20&&<div style={{fontSize:10,color:"#94a3b8",marginTop:4,textAlign:"center"}}>Showing latest 20 of {fuelComparison.weekCompare.length} weeks. Export CSV for full history.</div>}
                </div>

                {/* ── SIDE-BY-SIDE INVOICE DETAIL TABLES ── */}
                <div style={{fontSize:13,fontWeight:700,color:"#1e293b",marginBottom:6}}>Invoice detail — newest first</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  {/* Quick Fuel invoices (LEFT) */}
                  <div>
                    <div style={{padding:"6px 10px",background:C.brand,color:"#fff",borderRadius:"6px 6px 0 0",fontSize:11,fontWeight:700}}>
                      Quick Fuel — {fuelComparison.qfEntries.length} entries
                    </div>
                    <div style={{border:`1px solid ${C.brand}33`,borderTop:"none",borderRadius:"0 0 6px 6px",maxHeight:360,overflowY:"auto"}}>
                      {fuelComparison.qfEntries.length===0?(
                        <div style={{padding:12,color:"#94a3b8",fontSize:11,textAlign:"center"}}>No entries</div>
                      ):(
                        <table style={{width:"100%",borderCollapse:"collapse",fontSize:10}}>
                          <thead style={{position:"sticky",top:0,background:"#f8fafc"}}>
                            <tr>
                              <th style={{padding:"5px 6px",textAlign:"left",color:"#6b7785",fontWeight:600,fontSize:9,textTransform:"uppercase",borderBottom:"1px solid #e2e8f0"}}>Date</th>
                              <th style={{padding:"5px 6px",textAlign:"left",color:"#6b7785",fontWeight:600,fontSize:9,textTransform:"uppercase",borderBottom:"1px solid #e2e8f0"}}>Truck</th>
                              <th style={{padding:"5px 6px",textAlign:"right",color:"#6b7785",fontWeight:600,fontSize:9,textTransform:"uppercase",borderBottom:"1px solid #e2e8f0"}}>Gal</th>
                              <th style={{padding:"5px 6px",textAlign:"right",color:"#6b7785",fontWeight:600,fontSize:9,textTransform:"uppercase",borderBottom:"1px solid #e2e8f0"}}>$/gal</th>
                              <th style={{padding:"5px 6px",textAlign:"right",color:"#6b7785",fontWeight:600,fontSize:9,textTransform:"uppercase",borderBottom:"1px solid #e2e8f0"}}>Total</th>
                            </tr>
                          </thead>
                          <tbody>
                            {fuelComparison.qfEntries.map(e=>(
                              <tr key={e.id} style={{borderTop:"1px solid #f1f5f9"}}>
                                <td style={{padding:"4px 6px"}}>{e.date||"—"}</td>
                                <td style={{padding:"4px 6px",fontWeight:600}}>{e.truckId||"—"}</td>
                                <td style={{padding:"4px 6px",textAlign:"right"}}>{e._gallons.toFixed(1)}</td>
                                <td style={{padding:"4px 6px",textAlign:"right",fontWeight:700,color:C.brand}}>${e._ppg.toFixed(4)}</td>
                                <td style={{padding:"4px 6px",textAlign:"right"}}>${e._cost.toFixed(2)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                  ...
                  {/* FuelFox invoices (RIGHT) */}
                  <div>
                    <div style={{padding:"6px 10px",background:C.accent,color:"#fff",borderRadius:"6px 6px 0 0",fontSize:11,fontWeight:700}}>
                      FuelFox Atlanta — {fuelComparison.ffEntries.length} entries
                    </div>
                    <div style={{border:`1px solid ${C.accent}33`,borderTop:"none",borderRadius:"0 0 6px 6px",maxHeight:360,overflowY:"auto"}}>
                      {fuelComparison.ffEntries.length===0?(
                        <div style={{padding:12,color:"#94a3b8",fontSize:11,textAlign:"center"}}>
                          {fuelComparison.ffMissingOverhead>0?"No entries with overhead applied yet. Run Redistribute Overhead first.":"No entries"}
                        </div>
                      ):(
                        <table style={{width:"100%",borderCollapse:"collapse",fontSize:10}}>
                          <thead style={{position:"sticky",top:0,background:"#f8fafc"}}>
                          ...
                          <tbody>
                            {fuelComparison.ffEntries.map(e=>(
                              <tr key={e.id} style={{borderTop:"1px solid #f1f5f9"}}>
                                <td style={{padding:"4px 6px"}}>{e.date||"—"}</td>
                                <td style={{padding:"4px 6px",fontWeight:600}}>{e.truckId||"—"}</td>
                                <td style={{padding:"4px 6px",textAlign:"right"}}>{e._gallons.toFixed(1)}</td>
                                <td style={{padding:"4px 6px",textAlign:"right",fontWeight:700,color:C.accent}}>${e._ppg.toFixed(4)}</td>
                                <td style={{padding:"4px 6px",textAlign:"right"}}>${e._cost.toFixed(2)}</td>
                              </tr>
                            ))}
                          </tbody>
              ...
              {/* Footer */}
              <div style={{padding:"12px 16px",borderTop:"1px solid #e2e8f0",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,flexWrap:"wrap",flexShrink:0,background:"#f8fafc"}}>
                <div style={{fontSize:10,color:"#6b7785",flex:1}}>
                  Gallons-weighted avg = total$ ÷ total gallons (the true unit cost). Simple avg = unweighted mean of line-level $/gal values.
                </div>
                <button onClick={()=>{
                  // Build CSV combining both vendors with vendor column
                  const rows=[["Date","Vendor","Truck","Gallons","PricePerGal","Total","InvoiceNum"]];
                  [...fuelComparison.qfEntries,...fuelComparison.ffEntries].forEach(e=>{
                    rows.push([e.date||"",e._vendor,e.truckId||"",e._gallons.toFixed(2),e._ppg.toFixed(4),e._cost.toFixed(2),(e.invoiceNum||"").replace(/,/g,"")]);
                  });
                  // Then append monthly + weekly aggregates
                  rows.push([]);
                  rows.push(["MONTHLY AVERAGES (gallons-weighted)"]);
                  rows.push(["Month","QF $/gal","QF gallons","FF $/gal","FF gallons","Delta"]);
                  fuelComparison.monthCompare.forEach(m=>{
                    rows.push([m.month,m.qf?m.qf.gwAvg.toFixed(4):"",m.qf?m.qf.gallons.toFixed(2):"",m.ff?m.ff.gwAvg.toFixed(4):"",m.ff?m.ff.gallons.toFixed(2):"",m.delta!=null?m.delta.toFixed(4):""]);
                  });
                  rows.push([]);
                  rows.push(["WEEKLY AVERAGES (gallons-weighted)"]);
                  rows.push(["Week of","QF $/gal","QF gallons","FF $/gal","FF gallons","Delta"]);
                  fuelComparison.weekCompare.forEach(w=>{
                    rows.push([w.week,w.qf?w.qf.gwAvg.toFixed(4):"",w.qf?w.qf.gallons.toFixed(2):"",w.ff?w.ff.gwAvg.toFixed(4):"",w.ff?w.ff.gallons.toFixed(2):"",w.delta!=null?w.delta.toFixed(4):""]);
                  });
                  const csv=rows.map(r=>r.map(c=>`"${String(c).replace(/"/g,'""')}"`).join(",")).join("\n");
                  const blob=new Blob([csv],{type:"text/csv"});
                  const url=URL.createObjectURL(blob);
                  const a=document.createElement("a");
                  a.href=url;a.download=`fuel-compare-${todayYMD()}.csv`;a.click();
                  URL.revokeObjectURL(url);
                }} style={{padding:"8px 14px",background:C.green,color:"#fff",border:"none",borderRadius:6,cursor:"pointer",fontWeight:700,fontSize:12}}>📥 Export CSV</button>
                <button onClick={()=>setShowFuelCompareReport(false)} style={{padding:"8px 14px",background:"#fff",color:"#6b7785",border:"1px solid #cbd5e1",borderRadius:6,cursor:"pointer",fontWeight:700,fontSize:12}}>Close</button>
              </div>
            </div>
          </div>}
          ...
          {costEntries.length>0&&<div>
            <div style={{...s.secT,marginTop:20,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                ...
                {fuelComparison&&(fuelComparison.qfEntries.length>0||fuelComparison.ffEntries.length>0)&&<button onClick={()=>setShowFuelCompareReport(true)} style={{padding:"6px 12px",background:C.purple,color:"#fff",fontSize:12,fontWeight:700,border:"none",borderRadius:6,cursor:"pointer"}}>🆚 Compare QF vs FF</button>}
                {quickFuelAnalytics&&<button onClick={async()=>{
                  const qfCount=quickFuelAnalytics.qfEntries.length;
                  const qfTotal=quickFuelAnalytics.totalCost;
                  if(!await uiConfirm(`Delete all ${qfCount} Quick Fuel entries ($${qfTotal.toFixed(2)})?\n\nThis will let you re-scan the invoices cleanly. Entries with vendor "Quick Fuel" will be removed. Other vendors are unaffected.\n\nThis cannot be undone.`))return;
                  if(!await uiConfirm(`Really delete ${qfCount} Quick Fuel entries worth $${qfTotal.toFixed(2)}?`))return;
                  const remaining=costEntries.filter(c=>(c.vendor||"").toLowerCase()!=="quick fuel");
                  saveCosts(remaining);
                  toast(`Deleted ${qfCount} Quick Fuel entries ($${qfTotal.toFixed(2)}). Re-scan from Gmail to re-import cleanly.`);
                ...
                {(()=>{
                  ...
                  return <button onClick={async()=>{
                    if(!await uiConfirm(`Delete all ${ffEntries.length} FuelFox entries ($${ffTotal.toFixed(2)})?\n\nThis will let you re-scan the invoices cleanly. Both Service Log and Invoice (DDxxx) entries will be removed. Other vendors are unaffected.\n\nThis cannot be undone.`))return;
                    if(!await uiConfirm(`Really delete ${ffEntries.length} FuelFox entries worth $${ffTotal.toFixed(2)}?`))return;
                    const remaining=costEntries.filter(c=>{const v=(c.vendor||"").toLowerCase();return v!=="fuelfox atlanta"&&v!=="fuelfox";});
                    saveCosts(remaining);
                    toast(`Deleted ${ffEntries.length} FuelFox entries ($${ffTotal.toFixed(2)}). Re-scan from Gmail to re-import cleanly.`);
          ...
          {/* v2.10.48: Review Queue — uncertain auto-sync items awaiting human approval */}
          {reviewQueue.length>0&&<div style={{marginTop:20,padding:14,background:"#fef3c7",borderRadius:8,border:"1px solid #fcd34d"}}>
            <div style={{fontSize:14,fontWeight:800,color:"#78350f",marginBottom:8,display:"flex",alignItems:"center",gap:8}}>
              ⚠ Review Queue ({reviewQueue.length})
              <span style={{fontSize:11,fontWeight:600,color:"#92400e"}}>Auto-sync flagged these as uncertain — please review</span>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {reviewQueue.map(item=>{
                const totalSum=(item.parsed||[]).reduce((s,e)=>s+(Number(e.total)||0),0);
                return <div key={item.id} style={{padding:10,background:"#fff",borderRadius:6,border:"1px solid #fcd34d"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,flexWrap:"wrap"}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:13,fontWeight:700,color:"#1e293b"}}>{item.vendor} · {item.parsed?.[0]?.invoiceNum||"(no invoice #)"}</div>
                      <div style={{fontSize:11,color:"#78350f",marginTop:2}}>⚠ {item.confidenceReason||"AI flagged uncertain"}</div>
                      <div style={{fontSize:11,color:"#475569",marginTop:4}}>{item.parsed?.length||0} row(s) · Total ${totalSum.toFixed(2)} · {item.filename}</div>
                      {item.parsed&&item.parsed.length>0&&<div style={{fontSize:10,color:"#6b7785",marginTop:6,paddingLeft:8,borderLeft:`2px solid ${C.brand}33`}}>
                        {item.parsed.slice(0,3).map((e,i)=><div key={i}>Truck #{e.truckId} · {e.date} · ${(Number(e.total)||0).toFixed(2)} · {e.category}</div>)}
          ...
          <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
            <input
              type="text"
              placeholder="🔎 Search truck #, vendor, invoice #, or notes..."
              value={costSearch}
              onChange={e=>setCostSearch(e.target.value)}
              style={{flex:"1 1 280px",minWidth:200,padding:"8px 12px",fontSize:13,border:"1px solid #e2e8f0",borderRadius:6,outline:"none",background:"#fff"}}
            />
            ...
            {(costSearch||costFilter!=="all"||costCatFilter!=="all")&&<button onClick={()=>{setCostSearch("");setCostFilter("all");setCostCatFilter("all");}} style={{padding:"6px 10px",fontSize:11,color:"#6b7785",background:"#f1f5f9",border:"1px solid #e2e8f0",borderRadius:4,cursor:"pointer"}}>Clear</button>}
          </div>
          {(()=>{
            // v2.10.42: filtered results with all three filters combined
            const q=costSearch.trim().toLowerCase();
            const filteredCosts=costEntries.filter(c=>{
              if(costFilter!=="all"&&c.truckId!==costFilter)return false;
              if(costCatFilter!=="all"&&c.category!==costCatFilter)return false;
              if(!q)return true;
              return (
                (c.truckId||"").toLowerCase().includes(q)||
                (c.vendor||"").toLowerCase().includes(q)||
                (c.invoiceNum||"").toLowerCase().includes(q)||
                (c.notes||"").toLowerCase().includes(q)||
                (c.category||"").toLowerCase().includes(q)
              );
            }).sort((a,b)=>b.date?.localeCompare(a.date));
            return <>
              {costSearch&&<div style={{fontSize:11,color:"#6b7785",marginBottom:8,marginTop:-4}}>{filteredCosts.length} match{filteredCosts.length===1?"":"es"}</div>}
              <div style={s.tWrap}><table style={{...s.table,fontSize:12}}><thead><tr>
                ...
                <td style={{...s.ltd,color:"#94a3b8"}} onClick={(e)=>e.stopPropagation()}>{c.invoiceNum
                  ?<button onClick={()=>openInvoice(c,"cost","Inv")} style={{background:"none",border:"none",color:C.brand,textDecoration:"underline",fontWeight:600,cursor:"pointer",padding:0,font:"inherit"}} title={findInvoiceSourceUrl(c)?"View invoice PDF":"Click to attach PDF"}>{c.invoiceNum}{!findInvoiceSourceUrl(c)&&" 📎"}</button>
                  :"—"}</td>
                <td style={{...s.ltd,maxWidth:180,overflow:"hidden",textOverflow:"ellipsis"}}>{c.notes||"—"}</td>
                <td style={s.ltd} onClick={(e)=>e.stopPropagation()}>{c.fileUrl
                  ?<button onClick={()=>setPdfPreview({url:c.fileUrl,label:`Inv ${c.invoiceNum||""} — ${c.vendor||""}`})} style={{background:"none",border:"none",color:C.brand,fontSize:16,cursor:"pointer",padding:0}} title="View PDF in app">📄</button>
                  :<button onClick={()=>{setAttachInvoice({record:c,recordType:"cost",label:`Inv ${c.invoiceNum||""} — ${c.vendor||""}`});setAttachStatus("");}} style={{background:"none",border:"none",color:"#94a3b8",fontSize:14,cursor:"pointer",padding:0}} title="Attach PDF">📎</button>}</td>
                <td style={s.ltd} onClick={(e)=>e.stopPropagation()}><button style={s.xBtn} title="Delete this cost entry" onClick={()=>deleteCost(c.id)}>×</button></td>
              </tr>
            )}
            {filteredCosts.length===0&&
              <tr><td colSpan={11} style={{...s.ltd,textAlign:"center",color:"#94a3b8",padding:20}}>{costSearch?`No matches for "${costSearch}".`:"No cost entries yet. Upload invoices or add manually."}</td></tr>
            }
          </tbody></table></div>
            </>;
          })()}
      ...
      {historyTruck&&(()=>{
        ...
        return <div style={s.modal} onClick={()=>{setTruckEdit(null);setHistoryTruck(null);}}><div style={{...s.modalBox,maxWidth:520}} onClick={e=>e.stopPropagation()}>
          {/* Header */}
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:6,alignItems:"center"}}>
              {t&&!truckEdit&&<button onClick={()=>setTruckEdit({orig:t.id,id:t.id,mk:normalizeMake(t.mk)||"Freightliner",md:t.md||"",tr:t.tr==="M"?"M":"A",ax:t.ax==="Tandem"?"Tandem":"Single",del:!!t.del})} style={{padding:"5px 10px",fontSize:11,fontWeight:700,background:"#fff",color:C.brand,border:`1px solid ${C.brand}`,borderRadius:5,cursor:"pointer",whiteSpace:"nowrap"}}>✏️ Edit</button>}
              <button onClick={()=>{setTruckEdit(null);setHistoryTruck(null);}} style={s.xBtn}>×</button>
            </div>
          </div>

          {/* v2.16.17: inline editor for truck # / make / model. Renumbering carries
              the truck's whole history with it — see commitTruckEdit. */}
          {t&&truckEdit&&<div style={{background:"#fffbeb",borderRadius:8,padding:12,marginBottom:12,border:`1px solid ${C.accent}66`}}>
            <div style={{fontSize:11,fontWeight:700,color:C.accent,marginBottom:8}}>EDIT TRUCK</div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              <label style={{flex:"1 1 110px",minWidth:100}}>
                <div style={{fontSize:10,color:"#6b7785",marginBottom:3}}>Truck #</div>
                <input style={{...s.fInp,width:"100%",boxSizing:"border-box",fontFamily:"monospace",fontWeight:700}} value={truckEdit.id} onChange={e=>setTruckEdit({...truckEdit,id:e.target.value})} onKeyDown={e=>{if(e.key==="Enter")commitTruckEdit();if(e.key==="Escape")setTruckEdit(null);}} autoFocus/>
              </label>
              <label style={{flex:"1 1 140px",minWidth:120}}>
                <div style={{fontSize:10,color:"#6b7785",marginBottom:3}}>Make</div>
                <select style={{...s.fInp,width:"100%",boxSizing:"border-box"}} value={MAKES.includes(truckEdit.mk)?truckEdit.mk:"Other"} onChange={e=>setTruckEdit({...truckEdit,mk:e.target.value})}>{MAKES.map(m=><option key={m} value={m}>{m}</option>)}</select>
              </label>
              <label style={{flex:"1 1 140px",minWidth:120}}>
              ...
              {/* v2.18.3: transmission and axle were set once on Add Truck and then
                  frozen — a spec that turns out wrong, or a unit swapped for a tandem,
                  had no way to be corrected. Both drive the Single/Tandem fleet filters
                  and the driver-assignment dropdowns, so a stale value misroutes work. */}
              <label style={{flex:"1 1 120px",minWidth:110}}>
                <div style={{fontSize:10,color:"#6b7785",marginBottom:3}}>Transmission</div>
                <select style={{...s.fInp,width:"100%",boxSizing:"border-box"}} value={truckEdit.tr==="M"?"M":"A"} onChange={e=>setTruckEdit({...truckEdit,tr:e.target.value})}>
                  <option value="A">Auto</option><option value="M">Manual</option>
                </select>
              </label>
              <label style={{flex:"0 0 auto",display:"flex",flexDirection:"column",cursor:"pointer"}}>
                <div style={{fontSize:10,color:"#6b7785",marginBottom:3}}>Deleted</div>
                <input type="checkbox" checked={!!truckEdit.del} onChange={e=>setTruckEdit({...truckEdit,del:e.target.checked})} style={{width:18,height:18,margin:"7px 0 0"}}/>
              </label>
              <label style={{flex:"1 1 120px",minWidth:110}}>
                <div style={{fontSize:10,color:"#6b7785",marginBottom:3}}>Axle</div>
                <select style={{...s.fInp,width:"100%",boxSizing:"border-box"}} value={truckEdit.ax==="Tandem"?"Tandem":"Single"} onChange={e=>setTruckEdit({...truckEdit,ax:e.target.value})}>
                  <option value="Single">Single</option><option value="Tandem">Tandem</option>
                </select>
              </label>
            </div>
            {String(truckEdit.id||"").trim()&&String(truckEdit.id).trim()!==truckEdit.orig&&<div style={{fontSize:11,color:"#92400e",marginTop:8,lineHeight:1.4}}>
              ⚠ Renumbering #{truckEdit.orig} → #{String(truckEdit.id).trim()} moves its repairs, invoices and every weekly assignment and daily status onto the new number.
            </div>}
            <div style={{display:"flex",gap:8,marginTop:10}}>
              <button style={{...s.saveBtn,opacity:truckEditBusy?0.6:1,cursor:truckEditBusy?"wait":"pointer"}} disabled={truckEditBusy} onClick={commitTruckEdit}>{truckEditBusy?"Saving…":"Save"}</button>
              <button style={s.canBtn} disabled={truckEditBusy} onClick={()=>setTruckEdit(null)}>Cancel</button>
            </div>
          </div>}

          {/* Specs */}
          {t&&!truckEdit&&<div style={{background:"#f8fafc",borderRadius:8,padding:12,marginBottom:12,border:"1px solid #e2e8f0"}}>
            <div style={{display:"flex",gap:16,flexWrap:"wrap",fontSize:12}}>
              <div><span style={{color:"#6b7785"}}>Type:</span> <span style={{fontWeight:700}}>{t.type==="straight"?"Box Truck":"Tractor"}</span></div>
              <div><span style={{color:"#6b7785"}}>Make:</span> <span style={{fontWeight:700}}>{normalizeMake(t.mk)}</span></div>
              {t.md&&<div><span style={{color:"#6b7785"}}>Model:</span> <span style={{fontWeight:700}}>{t.md}</span></div>}
              ...
              {t.del&&<div><span style={{color:"#6b7785"}}>Deleted:</span> <span style={{fontWeight:700}}>Yes</span></div>}
          ...
          {tCosts.map(c=>
            <div key={c.id} style={{padding:"8px 0",borderBottom:"1px solid #f1f5f9"}}>
              ...
              <div style={{fontSize:10,color:"#94a3b8"}}>
                ...
                {c.fileUrl&&<> · <button onClick={()=>setPdfPreview({url:c.fileUrl,label:`Inv ${c.invoiceNum||""} — ${c.vendor||""}`})} style={{background:"none",border:"none",color:C.brand,fontWeight:600,cursor:"pointer",padding:0,font:"inherit"}}>📄 View PDF</button></>}
              </div>
            </div>
          )}
          ...
          <button onClick={()=>{setTruckEdit(null);setHistoryTruck(null);}} style={{...s.canBtn,marginTop:12,width:"100%"}}>Close Report</button>
        </div></div>;
      })()}
          ...
          {truckHist.map(th=>
            <div key={th.truckId} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid #f1f5f9"}}>
              <div>
                ...
                <span style={{fontSize:10,color:"#94a3b8",marginLeft:6}}>{truckDesc(th)} · {th.tr==="A"?"Auto":"Man"}</span>
              </div>
      ...
      {/* Toast notifications */}
      {toasts.length>0&&<div style={{position:"fixed",bottom:16,left:"50%",transform:"translateX(-50%)",zIndex:9999,display:"flex",flexDirection:"column",gap:8,width:"min(92vw,440px)",pointerEvents:"none"}}>
        {toasts.map(t=>{const c=t.type==="error"?C.red:t.type==="success"?C.green:C.brand;const bg=t.type==="error"?"#fef2f2":t.type==="success"?"#f0fdf4":"#eff6ff";
          return <div key={t.id} onClick={()=>dismissToast(t.id)} style={{background:bg,border:`1px solid ${c}55`,borderLeft:`4px solid ${c}`,borderRadius:8,padding:"10px 12px",fontSize:12,color:"#1e293b",boxShadow:"0 6px 24px rgba(0,0,0,0.15)",cursor:"pointer",whiteSpace:"pre-wrap",lineHeight:1.45,display:"flex",gap:8,alignItems:"flex-start",pointerEvents:"auto"}}>
            <span style={{color:c,fontWeight:800,fontSize:13,flexShrink:0}}>{t.type==="error"?"⚠":t.type==="success"?"✓":"ℹ"}</span>
      ...
      {dialog&&<div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:10000,padding:16}} onClick={()=>closeDialog(dialog.kind==="prompt"?null:false)}>
        <div style={{background:"#fff",borderRadius:12,padding:20,width:"100%",maxWidth:400,boxShadow:"0 20px 60px rgba(0,0,0,0.25)",display:"flex",flexDirection:"column",gap:14}} onClick={e=>e.stopPropagation()}>
          <div style={{fontSize:13,color:"#1e293b",whiteSpace:"pre-wrap",lineHeight:1.5}}>{dialog.message}</div>
          {dialog.kind==="prompt"&&<input autoFocus value={dialog.value} placeholder={dialog.placeholder} onChange={e=>setDialog(d=>({...d,value:e.target.value}))} onKeyDown={e=>{if(e.key==="Enter")closeDialog(dialog.value);if(e.key==="Escape")closeDialog(null);}} style={{fontSize:13,padding:"8px 10px",border:`1px solid ${C.brand}66`,borderRadius:6,outline:"none",width:"100%"}}/>}
          <div style={{display:"flex",justifyContent:"flex-end",gap:8}}>
            <button onClick={()=>closeDialog(dialog.kind==="prompt"?null:false)} style={{padding:"8px 16px",fontSize:12,fontWeight:600,background:"#fff",color:"#475569",border:"1px solid #cbd5e1",borderRadius:6,cursor:"pointer"}}>{dialog.cancelLabel||"Cancel"}</button>
            <button autoFocus={dialog.kind!=="prompt"} onClick={()=>closeDialog(dialog.kind==="prompt"?dialog.value:true)} style={{padding:"8px 16px",fontSize:12,fontWeight:700,background:dialog.danger?C.red:C.brand,color:"#fff",border:"none",borderRadius:6,cursor:"pointer"}}>{dialog.okLabel}</button>
          </div>
        </div>
      </div>}
    </div>
      ...
      <div style={{overflowX:"auto",maxHeight:440,overflowY:"auto",border:"1px solid #e2e8f0",borderRadius:8}}>
        ...
        {rows.slice().sort((a,b)=>(b.rate||-1)-(a.rate||-1)).map(p=>{
          ...
          return <tr key={p.truckId} style={f.length?{background:"#fdf6f5"}:null}>
            ...
            <td style={{...td,fontSize:11,color:"#6b7785"}}>{p.t?truckDesc(p.t):"—"}</td>
...
function TSRow({t,tStat,ec,ev,sev,se,ce,gs,onOOS}){
  ...
  return <tr style={s.tr}><td style={{...s.td,fontWeight:700,fontFamily:"monospace",color:"#1e293b",position:"sticky",left:0,background:"#fff",zIndex:1}}>{t.id}</td><td style={{...s.td,fontSize:11}}>{truckDesc(t)}</td>
...
function Stat({l,v,sub,c,onClick}){
  ...
  return <div onClick={onClick} onMouseEnter={()=>clickable&&setH(true)} onMouseLeave={()=>setH(false)}
    title={clickable?`View ${l}`:undefined}
    style={{...s.statCard,position:"relative",...(clickable?{cursor:"pointer"}:{}),...(clickable&&h?{boxShadow:"0 4px 14px rgba(0,0,0,0.10)",transform:"translateY(-1px)",borderColor:c+"66"}:{})}}>
    <div style={{fontSize:28,fontWeight:800,color:c}}>{v}</div>
    <div style={{fontSize:12,color:"#6b7785",marginTop:4}}>{l}</div>
    {sub&&<div style={{fontSize:10,color:"#94a3b8",marginTop:2}}>{sub}</div>}
    {clickable&&<div style={{position:"absolute",top:8,right:10,fontSize:14,lineHeight:1,color:h?c:"#cbd5e1"}}>›</div>}
  </div>;}
...
function ManualCostForm({trucks,cats,onAdd}){
  ...
  const[f,setF]=useState({truckId:"",vendor:"",date:todayYMD(),invoiceNum:"",total:"",category:"Parts",notes:""});
  const submit=()=>{
    if(!f.truckId||!f.total)return;
    onAdd({...f,total:Number(f.total),lineItems:[]});
    setF({truckId:"",vendor:"",date:todayYMD(),invoiceNum:"",total:"",category:"Parts",notes:""});
    setShow(false);
  };
  ...
  return <div style={{...s.addForm,marginBottom:16}}>
    ...
    <div style={{display:"flex",gap:6}}>
      <select style={s.fInp} value={f.truckId} onChange={e=>setF({...f,truckId:e.target.value})}>
        ...
        {trucks.map(t=><option key={t.id} value={t.id}>#{t.id} — {truckDesc(t)}</option>)}
      </select>
