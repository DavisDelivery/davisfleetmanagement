          ...
          <details style={{marginBottom:12,padding:"8px 12px",background:"#fffbeb",border:`1px solid ${C.accent}55`,borderRadius:6}}>
            ...
            <div style={{marginTop:10,fontSize:11,fontFamily:"monospace",color:"#1e293b"}}>
              ...
              <div style={{marginTop:10,display:"flex",gap:6,flexWrap:"wrap"}}>
                ...
                <button onClick={async()=>{
                  try{
                    const r=await window.storage.get(`fl-asgn-${wk}`);
                    if(!r){alert(`No data found at fl-asgn-${wk}`);return;}
                    const data=JSON.parse(r.value);
                    const entries=Object.entries(data);
                    if(entries.length===0){alert(`fl-asgn-${wk} exists but is empty {}`);return;}
                    const sample=entries.slice(0,10).map(([k,v])=>`  ${k}: ${v}`).join("\n");
                    alert(`fl-asgn-${wk}\n${entries.length} entries.\n\nFirst 10:\n${sample}`);
                  }catch(e){alert("Fetch failed: "+e.message);}
                }} style={{padding:"6px 12px",fontSize:11,fontWeight:700,background:C.green,color:"#fff",border:"none",borderRadius:4,cursor:"pointer"}}>🔎 Inspect current week</button>
              </div>
              <div style={{marginTop:14,paddingTop:10,borderTop:`1px solid ${C.accent}55`}}>
                <div style={{fontSize:11,fontWeight:700,color:C.red,marginBottom:6}}>⚠ RECOVERY — restore historical data from DVIR backup</div>
                <div style={{fontSize:10,color:"#475569",marginBottom:8}}>The /dvir_history.json file deployed with the app contains 68 weeks (Dec 2024 – Apr 13 2026) of historical assignment + truck-status data. This button deletes the 'already-imported' flag and reloads the page. The app will then re-import the data into Firebase on next mount. Safe to run multiple times — uses merge, not overwrite. Will NOT recover the most recent ~3 weeks (Apr 20 onward).</div>
                <button onClick={async()=>{
                  if(!confirm("RESTORE HISTORICAL DATA?\n\nThis will:\n1. Delete the fl-dvir-imported flag from Firebase\n2. Reload the page\n3. The app will automatically re-import 68 weeks of historical data on startup\n\nProceed?"))return;
                  try{
                    await window.storage.delete("fl-dvir-imported");
                    alert("Flag deleted. Reloading page now — wait 30-60 seconds after reload for the import to finish, then check the Weekly Board.");
                    window.location.reload();
                  }catch(e){alert("Failed: "+e.message);}
                }} style={{padding:"8px 16px",fontSize:12,fontWeight:700,background:C.red,color:"#fff",border:"none",borderRadius:6,cursor:"pointer"}}>🔄 Restore from DVIR backup (68 weeks)</button>
              </div>
            </div>
          </details>
