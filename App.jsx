...
function App(){
  ...
  const[autoScanDaysBack,setAutoScanDaysBack]=useState(30); // v2.10.3: date window for Scan All New
        ...
        {tab==="costs"&&<div>
          ...
          <div style={{...s.addForm,marginBottom:16}}>
            {!gmailConn?<div>
              ...
              {/* v2.10.3: primary one-click action */}
              <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",marginBottom:8}}>
                <button
                  onClick={()=>scanAllNewInvoices(autoScanDaysBack)}
                ...
                >
                  {autoScanPhase==="searching"?"🔍 Searching Gmail..."
                    ...
                    :`🚀 Scan All New Invoices (last ${autoScanDaysBack}d)`}
                </button>
                <select
                  value={autoScanDaysBack}
                  onChange={e=>setAutoScanDaysBack(Number(e.target.value))}
              ...
              <details style={{marginTop:4}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:6}}>
                  {knownVendors.map(v=><button key={v.name} onClick={()=>fetchGmailInvoices(v.name.toLowerCase(),autoScanDaysBack)} disabled={gmailLoading} style={{...s.saveBtn,opacity:gmailLoading?0.5:1,fontSize:11,padding:"5px 10px",background:"#fff",color:C.brand,border:`1px solid ${C.brand}`,fontWeight:600}}>{gmailLoading?"Searching...":"📧 "+v.name}</button>)}
                </div>
              </details>
            </div>}
