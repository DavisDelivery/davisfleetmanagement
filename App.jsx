    ...
    <div style={s.wrap}>
      ...
      <div style={s.body}>
        ...
        {tab==="fleet"&&<div>
          ...
          {fleetView==="list"&&<div style={s.tWrap}><table style={{...s.table,fontSize:12}}><thead><tr>{["Truck #","Type","Make","Model","Trans","Axle","Status","Driver","Repairs",""].map(h=><th key={h} style={{...s.th,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>
            {filteredTrucks.map(t=>{const st2=gTS(t.id,dk);const dr=findDriverForTruck(t.id);const openR=repairs.filter(r=>r.truckId===t.id&&r.status==="open").length;
              ...
              return <tr key={t.id} style={s.tr}>
                ...
                {/* v2.10.28: editable Make dropdown */}
                <td style={s.ltd}>
                  <select value={t.mk} onChange={e=>updateTruck(t.id,{mk:e.target.value})}
                    style={{fontSize:11,padding:"2px 4px",border:"1px solid #e2e8f0",borderRadius:4,background:"#fff",outline:"none",cursor:"pointer",maxWidth:90}}>
                    {makeOpts.map(m=><option key={m} value={m}>{m}</option>)}
                    {!makeOpts.includes(t.mk)&&<option value={t.mk}>{t.mk}</option>}
                  </select>
                </td>
                {/* v2.10.28: editable Model text field */}
                <td style={s.ltd}>
                  <input defaultValue={t.model||""} placeholder="Model..."
                    onBlur={e=>{if(e.target.value!==(t.model||""))updateTruck(t.id,{model:e.target.value});}}
                    onKeyDown={e=>{if(e.key==="Enter")e.target.blur();}}
                    style={{fontSize:11,padding:"2px 6px",border:"1px solid #e2e8f0",borderRadius:4,background:"#fff",outline:"none",width:80}}/>
                </td>
                ...
                <td style={{...s.ltd,color:drDown?"#94a3b8":C.brand,cursor:dr?"pointer":"default",textDecoration:dr?"underline":"none",fontStyle:drDown?"italic":"normal"}} onClick={()=>{if(dr)setDriverReport(dr.name);}} title={drDown?"Truck is OOS — driver needs reassignment":""}>{dr?(dr.name+(drDown?" (reassign)":"")):"—"}</td>
