...
function App(){
  ...
  const[autoScanCustomUntil,setAutoScanCustomUntil]=useState(()=>new Date().toISOString().split("T")[0]);
  ...
  useEffect(()=>{(async()=>{
    try{
      const[tR,dR,aR,sR,rR,cR,rtR,coresR,rqR,rdR,mmR]=await Promise.all([
      ...
      if(mmR){try{const m=JSON.parse(mmR.value);setMotiveMap({vehicles:m.vehicles||{},drivers:m.drivers||{}});}catch(e){}}
  ...
  const[motiveDriverLocs,setMotiveDriverLocs]=useState([]); // driver_locations: each driver + LIVE current_vehicle
  ...
  const loadMotiveData=async(baseMap)=>{
    ...
    try{
      const[vR,dR,lR]=await Promise.all([
        fetch("/api/motive?action=vehicles"),
        fetch("/api/motive?action=drivers"),
        fetch("/api/motive?action=driver_locations"),
      ]);
      ...
      const locs=(lJson.users||[]).map(x=>x.user||x);
      setMotiveVehicles(vehicles);setMotiveDrivers(driversList);setMotiveDriverLocs(locs);
  ...
  // Live "who is on which truck in Motive" keyed by Motive vehicle id, derived
  // from driver_locations (the driver→current_vehicle direction that matches
  // Motive's driver page). This is authoritative; vehicle.current_driver lags.
  const motiveDriverByVehId=useMemo(()=>{
    const m={};
    motiveDriverLocs.forEach(u=>{const cv=u.current_vehicle;if(cv&&cv.id!=null)m[String(cv.id)]={name:`${u.first_name||""} ${u.last_name||""}`.trim(),id:String(u.id)};});
    return m;
  },[motiveDriverLocs]);
  // Read-only audit: for the given day compare who the board has on each mapped
  // truck vs the driver Motive currently reports on it (live, from driver_locations),
  // and flag the differences. Matches by mapped Motive id, falling back to name.
  const buildMotiveComparison=(day)=>{
    ...
    trucks.forEach(t=>{
      ...
      const md=motiveDriverByVehId[String(vid)]||null;
      const motiveId=md?md.id:null;
      const motiveName=md?md.name:null;
      const drv=drivers.find(d=>asgn[`${d.name}-${day}`]===t.id);
  ...
  // Check one or more trucks against Motive right now and record flags. Uses
  // driver_locations (live driver→vehicle) so we see who's actually on each truck,
  // not the stale vehicle.current_driver field. One API call covers all trucks.
  const liveCheckTrucks=useCallback(async(truckIds,day,asgnMap)=>{
    ...
    try{
      const r=await fetch("/api/motive?action=driver_locations");
      const j=await r.json().catch(()=>({}));
      if(!r.ok)throw new Error(j.error||`HTTP ${r.status}`);
      const locs=(j.users||[]).map(x=>x.user||x);
      const byVeh={};locs.forEach(u=>{if(u.current_vehicle&&u.current_vehicle.id!=null)byVeh[String(u.current_vehicle.id)]=u;});
      const ts=Date.now();
      setMotiveLiveFlags(f=>{const n={...f};mapped.forEach(tid=>{
        const vid=motiveMap.vehicles[tid];
        const onTruck=byVeh[String(vid)]||null;
        const motiveName=onTruck?`${onTruck.first_name||""} ${onTruck.last_name||""}`.trim():null;
        const motiveId=onTruck&&onTruck.id!=null?String(onTruck.id):null;
        ...
        else if(boardName)status="board_only"; // assigned on board, driver not logged into the truck in Motive yet
        else if(motiveId)status="motive_only"; // Motive shows a driver the board no longer has on this truck
        else status="clear";
  ...
  return(
    <div style={s.wrap}>
      ...
      <div style={s.body}>
        ...
        {/* Live Motive conflict banner — only fires on a REAL conflict (Motive shows a
            different driver than the board). "Assigned but not logged into Motive yet"
            is normal until shifts start, so it's a quiet footnote, never an alarm. */}
        {(()=>{
          ...
          return <div style={{margin:"0 0 12px",background:"#fef2f2",border:`1px solid ${C.red}55`,borderRadius:8,padding:"10px 12px"}}>
            ...
            {pending>0&&<div style={{fontSize:10,color:"#94a3b8",marginTop:6}}>+ {pending} more assigned but not yet logged into Motive (normal until shifts start)</div>}
          </div>;
        })()}
            ...
            <div style={s.modal} onClick={()=>setShowMotiveSync(false)}>
              <div style={{...s.modalBox,maxWidth:580}} onClick={e=>e.stopPropagation()}>
                ...
                {(()=>{
                  ...
                  const pending=rows.filter(r=>r.status==="board_only");   // assigned, not logged into Motive yet
                  const matches=rows.filter(r=>r.status==="match").length;
                  const isToday=motiveSyncDay===DAYS[Math.min(todayDI(),4)]&&wK(weekDate)===wK(new Date());
                  ...
                  return <div style={{marginTop:4}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                      ...
                      {rows.length>0&&<span style={{fontSize:10,color:"#94a3b8"}}>{matches} match{pending.length?` · ${pending.length} not logged in`:""}</span>}
                    </div>
                    {!isToday&&rows.length>0&&<div style={{fontSize:10,color:C.accent,marginTop:2}}>Motive only reflects today's live driver — comparing {motiveSyncDay} is informational.</div>}
                    <div style={{maxHeight:230,overflowY:"auto",marginTop:6,display:"flex",flexDirection:"column",gap:4}}>
                      ...
                      {motiveVehicles.length>0&&conflicts.length===0&&rows.length>0&&<div style={{fontSize:11,color:C.green,fontWeight:600}}>✓ No conflicts — Motive matches the board wherever a driver is logged in.</div>}
                      ...
                      {pending.length>0&&<details style={{marginTop:2}}>
                        <summary style={{fontSize:10,color:"#94a3b8",cursor:"pointer"}}>{pending.length} assigned but not logged into Motive yet</summary>
            ...
            {filteredRG.map(g=>{
              ...
              return[
                ...
                ...gd.map(dr=><tr key={dr.name} style={s.tr}><td style={{...s.td,...s.tdN,cursor:"pointer",color:C.brand,textDecoration:"underline",textDecorationColor:C.brand+"44"}} onClick={()=>setDriverReport(dr.name)}>{drvIcon(dr.role)} {dr.name}</td>
                  {DAYS.map(day=>{const ck=`${dr.name}-${day}`;const val=asgn[ck]||"";const isE=editCell===ck;const isOff=OFF_OPTS.includes(val);const ti=val&&!isOff?trucks.find(t=>t.id===val):null;const dup=val&&!isOff&&dupByDay[day]&&dupByDay[day].has(val);
                    ...
                    return <td key={day} style={dup?{...s.td,background:"#fef2f2",boxShadow:`inset 0 0 0 1.5px ${C.red}`}:s.td} title={dup?`Truck ${val} is double-booked on ${day}`:undefined} onClick={()=>startEdit(ck,val)}>{val?<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
                      <span style={{fontWeight:700,fontFamily:isOff?"inherit":"monospace",fontSize:isOff?9:12,color:dup?C.red:(isOff?C.accent:"#1e293b")}}>{dup&&"⚠ "}{val}</span>
                      {ti&&<span style={{fontSize:8,color:dup?C.red:"#94a3b8"}}>{dup?"double-booked":`${ti.mk} ${ti.tr}`}</span>}
                  ...
                  {autoScanDaysBack==="custom"&&<div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:8,padding:"8px 10px",background:"#f8fafc",borderRadius:6,border:"1px solid #e2e8f0"}}>
                    <div style={{display:"flex",gap:4,alignItems:"center",flex:"1 1 150px"}}>
                      ...
                      <input
                        ...
                        max={autoScanCustomUntil||new Date().toISOString().split("T")[0]}
            ...
            <div style={{background:"#fff",borderRadius:10,maxWidth:1100,width:"100%",maxHeight:"94vh",overflow:"hidden",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{padding:"12px 16px",borderTop:"1px solid #e2e8f0",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,flexWrap:"wrap",flexShrink:0,background:"#f8fafc"}}>
                ...
                <button onClick={()=>{
                  ...
                  const url=URL.createObjectURL(blob);
                  const a=document.createElement("a");
                  a.href=url;a.download=`fuel-compare-${new Date().toISOString().split("T")[0]}.csv`;a.click();
                  URL.revokeObjectURL(url);
                }} style={{padding:"8px 14px",background:C.green,color:"#fff",border:"none",borderRadius:6,cursor:"pointer",fontWeight:700,fontSize:12}}>📥 Export CSV</button>
