...
Format your response as clear sections with headers using ** for bold. Use specific numbers, names, and amounts. Be direct and actionable — this is for the owner who makes operational decisions daily. Keep it concise but thorough.`}
  ...
  const quickAddTruckAndClearQueue=async(missingId)=>{
    const stillBlocked=(row,fleetIds)=>{
    ...
    const touchesMissing=item=>(item.parsed||[]).some(r=>r&&String(r.truckId)===String(missingId));
    const simFleetIds=new Set([...trucks.map(t=>t.id),String(missingId),...retiredTrucks.map(t=>t.id)]);
    const wouldClear=reviewQueue.filter(item=>touchesMissing(item)&&(item.parsed||[]).every(r=>!stillBlocked(r,simFleetIds)));
    const extraCount=wouldClear.length-1;
    ...
    const clearable=reviewQueue.filter(item=>touchesMissing(item)&&(item.parsed||[]).every(r=>!stillBlocked(r,fleetIds)));
    if(!clearable.length){toast(`Added truck #${missingId} to the fleet.`,"success");return;}
    ...
    saveCosts([...costEntries,...approved]);
    saveReviewQueue(reviewQueue.filter(q=>!clearIds.has(q.id)));
    const stillStuck=reviewQueue.filter(item=>touchesMissing(item)&&!clearIds.has(item.id)).length;
    toast(`Added truck #${missingId} and imported ${clearable.length} invoice${clearable.length===1?"":"s"}.`+(stillStuck?` ${stillStuck} more mentioning #${missingId} still need review for a different reason.`:""),"success");
  };
