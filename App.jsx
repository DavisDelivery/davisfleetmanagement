          ...
          <details style={{marginBottom:12,padding:"8px 12px",background:"#fffbeb",border:`1px solid ${C.accent}55`,borderRadius:6}}>
            ...
            <div style={{marginTop:10,fontSize:11,fontFamily:"monospace",color:"#1e293b"}}>
              ...
              <div style={{marginTop:10,display:"flex",gap:6,flexWrap:"wrap"}}>
                ...
                <button onClick={async()=>{
                  const out=document.getElementById("test-write-out");
                  out.style.display="block";
                  out.textContent="Testing write to fl-asgn-test-2099-12-31...";
                  out.style.color="#1e293b";
                  window.__lastStorageError=null;
                  const result=await window.storage.set("fl-asgn-test-2099-12-31",JSON.stringify({"TestDriver-Mon":"TEST"}));
                  if(result){
                    out.textContent="✓ Write succeeded. fl-asgn-* writes are NOT blocked. The recovery should work.";
                    out.style.color="#16a34a";
                    await window.storage.delete("fl-asgn-test-2099-12-31").catch(()=>{});
                  }else{
                    const err=window.__lastStorageError||"(no error captured — check console)";
                    out.innerHTML=`✗ Write FAILED.<br>Error: <b>${err}</b><br><br>${/permission|denied|insufficient/i.test(err)?"This is a Firestore security rules block. Go to Firebase Console → Firestore → Rules and check what rules exist for the kv collection.":"Check browser console (F12) for full error details."}`;
                    out.style.color="#dc2626";
                  }
                }} style={{padding:"6px 12px",fontSize:11,fontWeight:700,background:"#f59e0b",color:"#fff",border:"none",borderRadius:4,cursor:"pointer"}}>🧪 Test write to fl-asgn-* (1 doc)</button>
                <div id="test-write-out" style={{display:"none",marginTop:6,padding:"6px 10px",background:"#f1f5f9",borderRadius:4,fontSize:11,fontFamily:"monospace",lineHeight:1.5}}></div>
              </div>
