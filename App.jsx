    ...
    <div style={s.wrap}>
      ...
      <div style={s.body}>
        ...
        {tab==="dispatch"&&(()=>{
          ...
          const useYesterdaySnapshot=todayAssigned<3&&di>0;
          // For Monday (di===0) fall back to last week's Friday if needed
          const viewDay=useYesterdaySnapshot?(di>0?DAYS[di-1]:null):today;
          // viewAsgn: use same asgn for this week; last week's data not loaded here
          // so Monday fallback only works if user is still on same week's data.
          // For the typical use-case (Mon-Fri, same week), di>0 covers Tue-Fri.
          const getViewVal=(driverName)=>{
            if(!useYesterdaySnapshot||!viewDay)return asgn[`${driverName}-${today}`]||"";
            // Yesterday snapshot: read from yesterday first, then fall back to today
            return asgn[`${driverName}-${viewDay}`]||asgn[`${driverName}-${today}`]||"";
          };
          const viewTStat=(truckId)=>{
            const key=useYesterdaySnapshot&&viewDay?`${truckId}-${viewDay}`:`${truckId}-${today}`;
            return tStat[key]||"";
          };
          ...
          return <div>
            ...
            {useYesterdaySnapshot&&<div style={{background:"#fffbeb",border:`1px solid ${C.accent}55`,borderRadius:8,padding:"10px 14px",marginBottom:16,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
              <div>
                <div style={{fontSize:12,fontWeight:700,color:C.accent}}>📋 Showing {["","Monday","Tuesday","Wednesday","Thursday","Friday"][di-1] || "yesterday"}'s assignments as today's snapshot</div>
            ...
            {/* Working drivers */}
            <div style={{fontSize:13,fontWeight:700,color:C.green,marginBottom:6}}>ON THE ROAD — {working.length} Davis + {ownerWorking.length} Owner{useYesterdaySnapshot?<span style={{fontSize:10,fontWeight:400,color:C.accent,marginLeft:8}}>(from {["","Mon","Tue","Wed","Thu","Fri"][di-1]||"yesterday"})</span>:null}</div>
