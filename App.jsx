...
function App(){
  ...
  // Tries in order: 1) direct fileUrl (PDF on Drive), 2) Gmail message link from gmailRef,
  // 3) fallback — find a cost entry with matching invoiceNum and use ITS fileUrl/gmailRef.
  // v2.10.42: 4) Final fallback — Gmail search URL for the invoice number. This handles
  // orphan records (cores with no matching cost entry, manually-entered invoices, etc).
  // Clicking always does *something* useful even when no exact source is tracked.
  // Returns a URL string, or null if no invoice number at all.
  const findInvoiceSourceUrl=(record)=>{
    if(!record)return null;
    const tryRec=(r)=>{
      if(!r)return null;
      if(r.fileUrl)return r.fileUrl;
      if(r.gmailRef&&typeof r.gmailRef==="string"&&r.gmailRef.startsWith("gmail:")){
        const msgId=r.gmailRef.split(":")[1];
        if(msgId)return `https://mail.google.com/mail/u/0/#inbox/${msgId}`;
      }
      return null;
    };
    const direct=tryRec(record);
    if(direct)return direct;
    // Fallback: look up cost entry by invoice number (covers cores, which don't have gmailRef)
    const invNum=record.returnedCreditInvoice||record.invoiceNum;
    if(!invNum)return null;
    const matchingEntry=costEntries.find(c=>c.invoiceNum===invNum);
    const viaMatching=tryRec(matchingEntry);
    if(viaMatching)return viaMatching;
    // Final fallback: Gmail search for the invoice number. Strip any -CORE / -truckId
    // suffixes we add internally (so the search hits the real invoice). Encode for URL.
    const cleanInv=invNum.replace(/-(CORE|CORERETURN)$/i,"").replace(/-\d+$/,"");
    return `https://mail.google.com/mail/u/0/#search/${encodeURIComponent(cleanInv)}`;
  };
                            ...
                            <div style={{flex:1,minWidth:200}}>
                              ...
                              <div style={{fontSize:11,color:"#6b7785",marginTop:4}}>
                                {c.date} · {c.vendor} · Inv {(()=>{
                                  ...
                                  return url
                                    ?<a href={url} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} style={{color:C.brand,textDecoration:"underline",fontWeight:600}} title="Open invoice source">{inv}</a>
                                    :inv;
                                })()}
                              </div>
            ...
            return <>
              ...
              <div style={s.tWrap}><table style={{...s.table,fontSize:12}}><thead><tr>
                ...
                <td style={{...s.ltd,color:"#94a3b8"}} onClick={(e)=>e.stopPropagation()}>{(()=>{
                  ...
                  const url=findInvoiceSourceUrl(c);
                  return url
                    ?<a href={url} target="_blank" rel="noopener noreferrer" style={{color:C.brand,textDecoration:"underline",fontWeight:600}} title="Open invoice source">{c.invoiceNum}</a>
                    :c.invoiceNum;
                })()}</td>
