...
Always match to the closest fleet number. Use the TOTAL line (including tax) for the total amount. For the date, use DATE INVOICE (not DATE SHIPPED). Return ONLY the JSON array.`}
  ...
  const closeRepair=(id,cost)=>{
    ...
    const next=repairs.map(r=>r.id===id?{...r,status:"closed",dateClosed:new Date().toISOString(),cost:cost||r.cost,...(deviceUser?{closedBy:deviceUser}:{})}:r);
    saveRepairs(next);
      ...
      <div style={s.body}>
        ...
        {tab==="maint"&&(()=>{
          ...
          {(()=>{
            ...
            const noteRefs={};
            const addNoteToRepair=(id)=>{
              const inp=noteRefs[id];
              if(!inp)return;
              const text=(inp.value||"").trim();
              if(!text)return;
              const entry={ts:new Date().toISOString(),text,...(deviceUser?{by:deviceUser}:{})};
