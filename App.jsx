...
Format your response as clear sections with headers using ** for bold. Use specific numbers, names, and amounts. Be direct and actionable — this is for the owner who makes operational decisions daily. Keep it concise but thorough.`}
  ...
  const fetchGmailInvoices=async(vendor)=>{
    ...
    try{
      ...
      const resp=await fetch("/api/fetch-invoices",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({vendor:vendor||"peach state",afterDate})});
      const data=await resp.json();
      const toStr=(v)=>{if(v==null)return"";if(typeof v==="string")return v;if(typeof v==="object")return JSON.stringify(v).substring(0,300);return String(v);};
      const setSafe=(val)=>{
        if(!val){setGmailResults([{error:"Empty response"}]);return;}
        if(Array.isArray(val))setGmailResults(val.filter(x=>x&&typeof x==="object"));
        else if(typeof val==="object")setGmailResults([val]);
        else setGmailResults([{error:toStr(val).substring(0,200)}]);
      };
      if(data.error){setGmailResults([{error:toStr(data.error).substring(0,300)}]);setGmailLoading(false);return;}
      if(data.results&&Array.isArray(data.results)){
        if(data.results.length===0)setGmailResults([{error:"No invoices found in the last 90 days"}]);
        else setSafe(data.results);
      }
      else if(data.message){setGmailResults([{error:toStr(data.message).substring(0,300)}]);}
      else if(data.rawText){
        const m=String(data.rawText).match(/\[[\s\S]*\]/);
        if(m){try{setSafe(JSON.parse(m[0]));}catch(e){setGmailResults([{error:"Found data but could not parse: "+toStr(data.rawText).substring(0,200)}]);}}
        else setGmailResults([{error:"No invoices found. Response: "+toStr(data.rawText).substring(0,200)}]);
      }else{
        setGmailResults([{error:"No results returned. Raw: "+JSON.stringify(data).substring(0,200)}]);
      }
    }catch(err){
              ...
              {gmailResults.map((r,i)=>{
                ...
                return <div key={i} style={{padding:"8px 10px",background:safeR.error?"#fef2f2":"#f0f9ff",borderRadius:6,marginBottom:4,border:"1px solid #e2e8f0"}}>
                  {safeR.error?<div style={{fontSize:11,color:C.red,wordBreak:"break-word"}}>{typeof safeR.error==="string"?safeR.error:JSON.stringify(safeR.error).substring(0,500)}</div>:<div>
                    ...
                    {atts.length>0&&<div style={{fontSize:10,color:C.brand,marginTop:2}}>📎 {atts.map(a=>a&&a.filename?a.filename:"file").join(", ")}</div>}
                    {safeR.invoiceHints&&<div style={{fontSize:10,color:C.green,marginTop:2}}>{safeR.invoiceHints}</div>}
                  </div>}
                </div>;
              })}
