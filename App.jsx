          ...
          {inventoryAnalytics.inv.length>0&&<div>
            ...
            {inventoryAnalytics.unallocated.length>0&&<div style={{marginTop:12}}>
              ...
              <div style={{maxHeight:240,overflowY:"auto",border:"1px solid #e2e8f0",borderRadius:6}}>
                {inventoryAnalytics.unallocated.sort((a,b)=>(b.date||"").localeCompare(a.date||"")).map(c=><div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px",borderBottom:"1px solid #f1f5f9",background:"#fff"}}>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:11,fontWeight:600,color:"#1e293b"}}>{c.date} · {c.invoiceNum||"no inv#"} · ${(c.total||0).toFixed(2)}</div>
                    <div style={{fontSize:10,color:"#6b7785",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.vendor}{c.notes?" — "+c.notes:""}</div>
                  </div>
                  <button onClick={()=>setAllocModal(c)} style={{fontSize:10,padding:"4px 8px",background:C.brand,color:"#fff",border:"none",borderRadius:4,cursor:"pointer",fontWeight:600}}>Allocate</button>
                </div>)}
              </div>
            </div>}
            ...
            {inventoryAnalytics.allocated.length>0&&<div style={{marginTop:12}}>
              ...
              <div style={{maxHeight:160,overflowY:"auto",border:"1px solid #e2e8f0",borderRadius:6}}>
                {inventoryAnalytics.allocated.sort((a,b)=>(b.allocatedDate||"").localeCompare(a.allocatedDate||"")).map(c=><div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px",borderBottom:"1px solid #f1f5f9",background:"#fff"}}>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:11,fontWeight:600,color:"#1e293b"}}>Truck {c.allocatedToTruck} ← {c.invoiceNum||"no inv#"} · ${(c.total||0).toFixed(2)}</div>
                    <div style={{fontSize:10,color:"#6b7785"}}>Purchased {c.date} · Installed {c.allocatedDate}</div>
                  </div>
                  <button onClick={()=>unallocateInventory(c.id)} style={{fontSize:10,padding:"4px 8px",background:"#fff",color:C.red,border:`1px solid ${C.red}`,borderRadius:4,cursor:"pointer",fontWeight:600}}>Undo</button>
                </div>)}
              </div>
            </div>}
          </div>}
