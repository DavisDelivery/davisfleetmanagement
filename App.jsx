...
Format your response as clear sections with headers using ** for bold. Use specific numbers, names, and amounts. Be direct and actionable — this is for the owner who makes operational decisions daily. Keep it concise but thorough.`}
  ...
  const catchUpBacklog=async(daysBack=365)=>{
    ...
    if(!await uiConfirm(`Catch up the invoice backlog?\n\nScans about the last ${months} months of vendor email and imports anything not already in the ledger. Confident invoices import automatically; uncertain ones go to the Review Queue.\n\nIt runs in passes and can take several minutes — the counter shows how many attachments are left. Already-imported invoices are skipped for free; only genuinely new ones use AI credits. You can stop at any time and it resumes where it left off.`))return;
    ...
    while(pass<MAX_PASSES){
      if(backlogStopRef.current){setBacklog({running:false,pass,imported,queued,scanned,remaining,note:"Stopped — the queue is saved and will resume where it left off."});toast(`Backlog sweep stopped — imported ${imported}, queued ${queued}.`);return;}
      ...
      /* v2.17.0: another run (usually the 3-hourly schedule) holds the single-flight
         lock — wait a few seconds and try again rather than counting it as a pass. */
      if(data&&data.busy){
        if(++busyWaits>20){setBacklog({running:false,pass,imported,queued,scanned,remaining,note:"Another sync kept the lock — try again in a minute."});return;}
      ...
      pass++;busyWaits=0;
      imported+=data.imported||0;queued+=data.queued||0;scanned+=data.processed||0;stuck=data.stuck||0;
    ...
    setBacklog({running:false,pass,imported,queued,scanned,remaining:hitCap?remaining:0,note:(hitCap?`Paused after ${MAX_PASSES} passes — run again to continue.`:"Caught up ✓")+stuckNote});
    toast((hitCap
                ...
                return <>
                  ...
                  <div style={{marginBottom:10,padding:12,background:"#f0f9ff",borderRadius:8,border:"1px solid #bfdbfe"}}>
                    ...
                    <div style={{fontSize:11,color:"#475569",lineHeight:1.5}}>
                      ...
                      <div style={{marginTop:4,fontSize:10,color:"#94a3b8"}}>Runs automatically every ~3 hours · imports confident invoices on its own, queues only the uncertain ones for review below.</div>
                    </div>
                  </div>
