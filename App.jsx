...
Format your response as clear sections with headers using ** for bold. Use specific numbers, names, and amounts. Be direct and actionable — this is for the owner who makes operational decisions daily. Keep it concise but thorough.`}
  ...
  const fetchVendorEmails=async(vendor,daysBack=30)=>{
    ...
    try{
      const resp=await fetch("/api/gmail-search",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({refresh_token:gmailConn.refresh_token,vendor,afterDate})});
      const data=await resp.json();
  ...
  const fetchGmailInvoices=async(vendor,daysBack=30)=>{
    ...
    setGmailResults(null);
    const results=await fetchVendorEmails(vendor||"peach state",daysBack);
    if(results.length===0){
      setGmailResults([{error:`No matching emails found in the last ${daysBack} days.`}]);
    }else{
  ...
  const scanAllNewInvoices=async(daysBack=30)=>{
    if(!gmailConn||!gmailConn.refresh_token){
    ...
    const perVendor=await Promise.all(vendorNames.map(v=>fetchVendorEmails(v,daysBack)));
    const allResults=perVendor.flat();

    // Show them in the UI immediately so user sees what we found
    setGmailResults(allResults.length>0?allResults:[{error:`No matching emails found in the last ${daysBack} days across any vendor.`}]);
              ...
              {(()=>{
                ...
                const dateLabel=(()=>{
                  if(autoScanDaysBack==="custom"){
                    const d=new Date(autoScanCustomSince);
                ...
                return <>
                  <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",marginBottom:8}}>
                    <button
                      onClick={()=>scanAllNewInvoices(effectiveDaysBack)}
                  ...
                  {autoScanDaysBack==="custom"&&<div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",marginBottom:8,padding:"8px 10px",background:"#f8fafc",borderRadius:6,border:"1px solid #e2e8f0"}}>
                    <label style={{fontSize:11,fontWeight:600,color:"#6b7785"}}>Since:</label>
                    <input
                      type="date"
                      ...
                      disabled={autoScanPhase!==null||scanning}
                      style={{padding:"6px 8px",fontSize:12,fontWeight:600,border:"1px solid #cbd5e1",borderRadius:4,background:"#fff",color:"#1e293b",outline:"none",flex:"1 1 140px"}}
                    />
                    <span style={{fontSize:10,color:"#94a3b8"}}>({effectiveDaysBack} days)</span>
                  </div>}
                  ...
                  <details style={{marginTop:4}}>
                    ...
                    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:6}}>
                      {knownVendors.map(v=><button key={v.name} onClick={()=>fetchGmailInvoices(v.name.toLowerCase(),effectiveDaysBack)} disabled={gmailLoading} style={{...s.saveBtn,opacity:gmailLoading?0.5:1,fontSize:11,padding:"5px 10px",background:"#fff",color:C.brand,border:`1px solid ${C.brand}`,fontWeight:600}}>{gmailLoading?"Searching...":"📧 "+v.name}</button>)}
                    </div>
                  </details>
                </>;
              })()}
