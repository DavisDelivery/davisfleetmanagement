                          ...
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,flexWrap:"wrap"}}>
                            <div style={{flex:1,minWidth:200}}>
                              ...
                              <div style={{fontSize:11,color:"#6b7785",marginTop:4}}>
                                {c.date} · {c.vendor} · Inv {(()=>{
                                  const url=findInvoiceSourceUrl(c);
                                  const inv=c.invoiceNum||"—";
                                  return url
                                    ?<button onClick={e=>{e.stopPropagation();setPdfPreview({url,label:`Inv ${inv} — ${c.vendor}`});}} style={{background:"none",border:"none",color:C.brand,textDecoration:"underline",fontWeight:600,cursor:"pointer",padding:0,font:"inherit"}} title="View invoice PDF in app">{inv}</button>
                                    :inv;
                                })()}
                              </div>
                      ...
                      <div key={c.id} style={{background:"#fffbeb",border:`1px solid ${C.accent}55`,borderRadius:8,padding:"10px 12px"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,flexWrap:"wrap"}}>
                          <div style={{flex:1,minWidth:200}}>
                            ...
                            <div style={{fontSize:11,color:"#6b7785",marginTop:2}}>{c.date} · {c.vendor} · Credit inv {(()=>{
                              const url=findInvoiceSourceUrl(c);
                              const inv=c.returnedCreditInvoice||c.invoiceNum||"—";
                              return url
                                ?<button onClick={e=>{e.stopPropagation();setPdfPreview({url,label:`Credit ${inv} — ${c.vendor}`});}} style={{background:"none",border:"none",color:C.brand,textDecoration:"underline",fontWeight:600,cursor:"pointer",padding:0,font:"inherit"}} title="View credit invoice PDF in app">{inv}</button>
                                :<span title="No PDF stored for this invoice — was likely added manually or before file storage existed">{inv}</span>;
                            })()}</div>
                          </div>
                          <div style={{display:"flex",alignItems:"center",gap:8}}>
                      ...
                      <div key={c.id} style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:6,padding:"8px 12px"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                          <div style={{flex:1,minWidth:200}}>
                            ...
                            <div style={{fontSize:10,color:"#6b7785"}}>Charged {c.date} · Returned {c.returnedDate||"—"} · Credit {(()=>{
                              const url=findInvoiceSourceUrl(c);
                              const inv=c.returnedCreditInvoice||"—";
                              return url
                                ?<button onClick={e=>{e.stopPropagation();setPdfPreview({url,label:`Credit ${inv} — ${c.vendor}`});}} style={{background:"none",border:"none",color:C.brand,textDecoration:"underline",fontWeight:600,cursor:"pointer",padding:0,font:"inherit"}} title="View credit invoice PDF in app">{inv}</button>
                                :inv;
                            })()}</div>
            ...
            <div style={{background:"#fff",borderRadius:10,padding:0,maxWidth:600,width:"100%",maxHeight:"90vh",overflow:"auto"}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{padding:"16px 20px"}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:16,paddingTop:12,borderTop:"1px solid #e2e8f0"}}>
                  {showCostDetail.fileUrl?<button onClick={()=>setPdfPreview({url:showCostDetail.fileUrl,label:`Inv ${showCostDetail.invoiceNum||""} — ${showCostDetail.vendor||""}`})} style={{flex:1,textAlign:"center",padding:"10px",background:C.brand,color:"#fff",border:"none",borderRadius:6,fontWeight:700,fontSize:13,cursor:"pointer"}}>📄 View PDF</button>:<div style={{flex:1,padding:"10px",background:"#f8fafc",color:"#94a3b8",borderRadius:6,textAlign:"center",fontSize:11,fontStyle:"italic"}}>No PDF attached (pre-v2.6.4 or bulk-imported)</div>}
          ...
          {(()=>{
            ...
            return <>
              ...
              <div style={s.tWrap}><table style={{...s.table,fontSize:12}}><thead><tr>
                ...
                <td style={{...s.ltd,color:"#94a3b8"}} onClick={(e)=>e.stopPropagation()}>{(()=>{
                  if(!c.invoiceNum)return "—";
                  const url=findInvoiceSourceUrl(c);
                  return url
                    ?<button onClick={()=>setPdfPreview({url,label:`Inv ${c.invoiceNum} — ${c.vendor||""}`})} style={{background:"none",border:"none",color:C.brand,textDecoration:"underline",fontWeight:600,cursor:"pointer",padding:0,font:"inherit"}} title="View invoice PDF in app">{c.invoiceNum}</button>
                    :c.invoiceNum;
                })()}</td>
                <td style={{...s.ltd,maxWidth:180,overflow:"hidden",textOverflow:"ellipsis"}}>{c.notes||"—"}</td>
                <td style={s.ltd} onClick={(e)=>e.stopPropagation()}>{c.fileUrl?<button onClick={()=>setPdfPreview({url:c.fileUrl,label:`Inv ${c.invoiceNum||""} — ${c.vendor||""}`})} style={{background:"none",border:"none",color:C.brand,fontSize:16,cursor:"pointer",padding:0}} title="View PDF in app">📄</button>:<span style={{color:"#cbd5e1"}}>—</span>}</td>
