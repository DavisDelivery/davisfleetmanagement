...
Format your response as clear sections with headers using ** for bold. Use specific numbers, names, and amounts. Be direct and actionable — this is for the owner who makes operational decisions daily. Keep it concise but thorough.`}
  ...
  const processGmailAttachment=async(emailResult,att)=>{
    ...
    try{
      // Fetch the attachment bytes — retry once on network failure
      let data;
      let attempt=0;
      while(attempt<2){
        try{
      ...
      const b64=data.data;
      if(!b64||b64.length===0)throw new Error("Gmail returned empty attachment data");
      ...
      if(isPdf){
        ...
        for(let p=1;p<=pdf.numPages;p++){
          ...
          // Compute scale so longest side doesn't exceed 1600px (Anthropic API has a 5MB base64 image limit).
          const rawVp=page.getViewport({scale:1});
          const maxSide=Math.max(rawVp.width,rawVp.height);
          const targetScale=Math.min(2,1600/maxSide);
          const vp=page.getViewport({scale:targetScale});
          const canvas=document.createElement("canvas");
          canvas.width=vp.width;canvas.height=vp.height;
          await page.render({canvasContext:canvas.getContext("2d"),viewport:vp}).promise;
          // JPEG at 85% quality is far smaller than PNG for scanned docs and stays under API limits.
  ...
  const handleFileUpload=async(e)=>{
    ...
    for(const file of files){
      if(file.type==="application/pdf"){
        ...
        for(let p=1;p<=pdf.numPages;p++){
          const page=await pdf.getPage(p);
          // Cap longest side at 1600px to avoid Anthropic 5MB image limit, use JPEG for smaller payload.
          const rawVp=page.getViewport({scale:1});
          const maxSide=Math.max(rawVp.width,rawVp.height);
          const scale=Math.min(2,1600/maxSide);
...
SPECIAL RULE FOR FUELFOX ATLANTA (vendor "FuelFox" / from "accounting@fuelfox.net"):
- FuelFox sends TWO DOCUMENTS PER DELIVERY, each handled separately:
  (A) SERVICE LOG — has per-truck breakdown (Unit Number, Gallons, Price Per Gallon, Total Charge). Return ONE ENTRY PER TRUCK in the service log. For each truck row set:
     - truckId: Unit Number (keep leading zeros, e.g. "0424")
     - vendor: "FuelFox Atlanta"
     - category: "Fuel"
     - total: Total Charge column (base price × gallons — does NOT include tax/delivery yet)
     - gallons: gallons for this truck
     - pricePerGallon: Price Per Gallon shown (this is the BASE price)
     - date: Service Date from header (YYYY-MM-DD)
     - invoiceNum: "FUELFOX-SL-YYYY-MM-DD-truck#" using the service date (so each truck row gets a unique invoice# per delivery, e.g. "FUELFOX-SL-2026-04-16-0424")
     - lineItems: [{"desc": "X.X gallons diesel @ $Y.YYYY/gal (base price)", "amount": total}]
     - notes: "FuelFox Service Log - Truck X (Y gal base price) - true cost includes tax+fee from matching Invoice DDxxx"
  (B) INVOICE (e.g. "Invoice DD404") — has overhead only: Diesel Taxes + Delivery Fee. Return ONE ENTRY with:
     - truckId: "INVENTORY"
     - vendor: "FuelFox Atlanta"
     - category: "Fuel"
     - total: the Diesel Taxes amount PLUS the Delivery Fee amount (e.g. 343.08 + 150.00 = 493.08) — NOT the balance due, just the overhead portion
     - invoiceNum: the invoice # (e.g. "DD404")
     - date: the Invoice DATE
     - lineItems: [{"desc": "Diesel Taxes", "amount": X}, {"desc": "Delivery Fee", "amount": Y}]
     - notes: "FuelFox Invoice DDxxx overhead (taxes + delivery fee). Distribute across trucks from matching Service Log for true $/gal."
